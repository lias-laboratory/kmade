...
public final class MediaAdaptator {
	...
	private static final ImageIcon START_MARK_SLIDER_IMAGE = new ImageIcon(KMADEEditorMultimediaDialog.class.getResource(KMADEConstant.START_MARK_SLIDER_IMAGE));
	
	private static final ImageIcon FINISH_MARK_SLIDER_IMAGE = new ImageIcon(KMADEEditorMultimediaDialog.class.getResource(KMADEConstant.FINISH_MARK_SLIDER_IMAGE));
    ...
    public static void openMediaEditor(Object ref) {
    	...
    	if (!KMADeMain.isAcceptedMedia()) {
        	GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().hideNoCodecMoviePanel();
        	return;
        }
    ...
    public static void openMovie(QTFile qtf) {
    	try {
	        ...
	        GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().openNewMovie(qc, duration , getTimeWithFormat(duration / currentMovie.getTimeScale()), mediaFilePath + File.separator + mediaFileName,qtf.length() / 1024);
	        if (startMark != -1) {
	        ...
	        GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().hideMoviePanel();
	    } catch (Exception e) {
	        GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().hideMoviePanel();
	    } finally {            
    ...
    private static void openFromModel() {
    	...
    	GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().showMovieInformationPanel();
    ...
    public static void enableMousePressed() {
		...
		try {
			...
			if (isPlayingWithMark) {
				isPlaying = true;
				GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setPlayMarkButtonImage();
				GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setPauseButtonImage();
    ...
    public static void playMovie() {
		try {
			...
			if (isPlaying) {
				currentMovie.stop();
				GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setPlayButtonImage();
				isPlaying = false;
			} else {
				if (isFinished) {
    ...
    public static void playMarkMovie() {
    	try {
			...
			if (isPlayingWithMark) {
				...
				currentMovie.start();
				GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setPauseMarkButtonImage();
    ...
    private static void startMarkMovie() {
		...
		labelTable.put( Integer.valueOf( startMark ), new JLabel(START_MARK_SLIDER_IMAGE) );
    	
		if (finishMark != -1) {
			if (startMark >= finishMark) {
				finishMark = -1;
				GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setFinishMarkTextField("");
				GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setDisableFinishMarkInit();
			} else {
				labelTable.put( Integer.valueOf( finishMark ), new JLabel(FINISH_MARK_SLIDER_IMAGE) );
			}
    	}
    	
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setEnableStartMarkInit();
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setEnablePlayMarkButton(labelTable);
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setStartMarkTextField(getTimeWithFormat((getRealCurrentTime(startMark))));   	
    }
    ...
    public static void initStartMarkMovie() {
		...
		if (labelTable.size() == 0) {
			GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setDisablePlayMarkButton();
		}
		else {
			GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setEnablePlayMarkButton(labelTable);			
		}
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setStartMarkTextField("");
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setDisableStartMarkInit();
    }
    ...
    private static void finishMarkMovie() {
		...
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setEnableFinishMarkInit();
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setEnablePlayMarkButton(labelTable);
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setFinishMarkTextField(getTimeWithFormat((getRealCurrentTime(finishMark))));
    }
    ...
    public static void timerProgess() {
		...
		try {
			...
			GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setTimeLabelContent(getTimeWithFormat(totalSeconds));
			if (!(mousePressed && keyPressed)) {
				GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setTimeSliderContent(currentMovie.getTime());
			}			
		} catch (QTException qte) {
			qte.printStackTrace();
		}
	...
	private static void processInitPlayer() {
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().initPlayerController();
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().hideMoviePanel();
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().hideMovieInformationPanel();
		isPlayingWithMark = false;
		isPlaying = false;
		isFinished = false;
		duration = 0;
