...
public final class KMADeAdaptator {
    ...
    public static void initLaunchKMADe() {
        ...
        ExpressPostcondition.addObserver(new Observer() {
            public void update(Observable o, Object argv) {
                if (updateThread) {
                    InterfaceExpressJava.setBddSetOn();
                    CoherenceAdaptator.checkTaskModel();
...
//        GraphicEditorAdaptator.getMainFrame().showEmptyPanel();      
    ...
    public static void openSimulationDialog() {
        ...
        if (erreur != 0) {
            ...
            JOptionPane.showMessageDialog(GraphicEditorAdaptator.getMainFrame(),"L'outil de cohérence a détecté " + erreur + " erreur" + (erreur > 1 ? "s" : "") + " et " + warning + " warning" + (warning > 1 ? "s" : ""), "Simulation refusée ...",JOptionPane.WARNING_MESSAGE, new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.INFO_DIALOG_IMAGE)));
    ...
    public static void showAtFirst() {
        ...
        ref.showInfoAbout("KMADe is under development ...", KMADEConstant.UNDER_DEVELOPMENT_START_HTML);
    }
