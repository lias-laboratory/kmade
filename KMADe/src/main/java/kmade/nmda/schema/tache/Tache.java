...
public class Tache implements Entity {
    ...
    // Ces trois attributs sont réservés pour la copie
    ...
    /**
     * Création d'une tâche avec des valeurs par défaut.
     */
    public Tache() {
    ...
    public Tache(
            ...
            ArrayList<Tache> fils, Point point, String prec, String post, String it, Oid oid) {
        this.name = name;
        this.but = but;
        this.ressources = res;
        this.feed = f;
        ...
        this.executant = Executant.getValue(exe);
        this.imp = Importance.getValue(imp);
        this.modal = Modalite.getValue(mod);
        this.frequence = Frequence.getValue(freq);
        this.compFreq = comp;
        this.setEvents(events);
        this.setDeclencheur(dec);
        if (facultatif == null)
        		this.facultatif = false;
        else
        ...
        for (int i = 0; i < fils.size(); i++) {
            Tache tache = fils.get(i);
            addSubTask(tache);
        }
        
        this.point = point;
        
        // Le media pour rester compatible avec les anciennes versions qui exploitent SPF.	
		idMedia  = null;
    ...
    /**
     * @param sensOrCogn : 0 pas de modalit�, 1 Sensori-motrice, 2 Cognitive
     */
        ...
        if (nodeList.getLength() != 0) {
            ...
            for (int i = 0 ; i < nodeListEvents.getLength() ; i++) {
                if (nodeListEvents.item(i).getNodeType() == Element.ELEMENT_NODE) {
                    if (InterfaceExpressJava.bdd.prendre(new Oid(((Element)nodeListEvents.item(i)).getTextContent())) == null) {
                        return true;
                    }
                }
            }
        ...
        if (nodeList.getLength() != 0) {
            ...
            for (int i = 0 ; i < nodeListSubTasks.getLength() ; i++) {
                if (nodeListSubTasks.item(i).getNodeType() == Element.ELEMENT_NODE) {
                    if (InterfaceExpressJava.bdd.prendre(new Oid(((Element)nodeListSubTasks.item(i)).getTextContent())) == null) {
                        return true;
                    }
                }
            }
    ...
    public void createObjectFromXMLElement(org.w3c.dom.Element p) {
        ...
        nodeList = p.getElementsByTagName("id-task-media");
        if (nodeList.item(0) != null) {
        	this.idMedia = (Media)InterfaceExpressJava.bdd.prendre(new Oid(nodeList.item(0).getTextContent()));
        } 
        ...
        if (nodeList.item(0) != null) {
        	Evenement event = (Evenement)InterfaceExpressJava.bdd.prendre(new Oid(nodeList.item(0).getTextContent()));
        ...
        if (nodeList.getLength() != 0) {
            ...
            for (int i = 0 ; i < nodeListEvent.getLength() ; i++) {
                if (nodeListEvent.item(i).getNodeType() == Element.ELEMENT_NODE) {
                	Evenement event = (Evenement)InterfaceExpressJava.bdd.prendre(new Oid(nodeListEvent.item(i).getTextContent()));
        ...
        if (nodeList.getLength() != 0) {
            ...
            for (int i = 0 ; i < nodeListActors.getLength() ; i++) {
                if (nodeListActors.item(i).getNodeType() == Element.ELEMENT_NODE) {
                	Acteur acteur = (Acteur)InterfaceExpressJava.bdd.prendre(new Oid(nodeListActors.item(i).getTextContent()));
        ...
        if (nodeList.getLength() != 0) {
            ...
            for (int i = 0 ; i < nodeListSubTasks.getLength() ; i++) {
                if (nodeListSubTasks.item(i).getNodeType() == Element.ELEMENT_NODE) {
                	Tache tache = (Tache)InterfaceExpressJava.bdd.prendre(new Oid(nodeListSubTasks.item(i).getTextContent()));
        ...
        this.iteExpression = new IterExpression(nodeList.item(0).getTextContent());
        
        // Iteration Description
        nodeList = p.getElementsByTagName("task-descriptioniteration");
        if (nodeList.item(0) != null) {
        		this.iteExpression.setDescription(nodeList.item(0).getTextContent());
        }
    }
     ...
     */
    public String toSPF() {
        String SPF = oid.get() + "=" + "Tache" + "(" + "'" + name.replaceAll("'","\\\\'") + "'" + ","
                + "'" + but + "'" + "," + "'" + ressources + "'" + "," + "'"
                + feed + "'" + "," + "'" + duree + "'" + "," + "'"
                + observation + "'" + "," + executant.toSPF() + ","
                + frequence.toSPF() + ",'" + compFreq + "'," + imp.toSPF()
                + "," + modal.toSPF() + ",";
        if (declencheur != null)
    ...
    /**
     * @param taskCloned The taskCloned to set.
     */
