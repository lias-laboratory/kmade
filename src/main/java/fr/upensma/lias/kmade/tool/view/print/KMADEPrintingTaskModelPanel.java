...
public class KMADEPrintingTaskModelPanel extends JPanel {
	...
	private Image down = new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.DOWN_ARROW_IMAGE))
			.getImage();

	private Image left = new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.LEFT_ARROW_IMAGE))
			.getImage();

	private Image right = new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.RIGHT_ARROW_IMAGE))
			.getImage();
	...
	class JGraphExtendedView extends JGraph {
		...
		public void paint(Graphics g) {
			...
			Rectangle myGraphBounds = KMADEPrintingTaskModelPanel.getRealJGraphScaleSize(jgraphSize, myGraph,
					scaleGraph);
			...
			for (int i = 0; i < nh; i++) {
				for (int j = 0; j < nw; j++) {
					...
					g2d.drawString(namePage + text, (int) (currentx + wp - (width + textHGap)),
					...
					if (j + 1 < nw) {
						...
						g2d.drawString(namePage + text, (int) (currentx + wp) - (width + rightHGap),
								(int) (currenty + (hp / 2)) + (heigh / 2) - rightVGap);
						g2d.drawImage(right, (int) (currentx + wp) - widthIcon,
								(int) (currenty + (hp / 2)) - (heightIcon / 2), widthIcon, heightIcon, null);
					}
					...
					if (i + 1 < nh) {
						...
						bounds = g2d.getFont().getStringBounds(text, frc);
						width = (int) bounds.getWidth();
						g2d.drawString(namePage + text, (int) (currentx + (wp / 2) - (width + bottomHGap)),
								(int) (currenty + hp) - (heigh + bottomVGap));
						g2d.drawImage(down, (int) (currentx + wp / 2) - (widthIcon / 2),
								(int) (currenty + hp) - (heightIcon), widthIcon, heightIcon, null);
					}
					...
					if (i != 0) {
						...
						bounds = g2d.getFont().getStringBounds(text, frc);
						width = (int) bounds.getWidth();
						g2d.drawString(namePage + text, (int) (currentx + (wp / 2) - (width + topHGap)),
								(int) (currenty) + (heigh + topVGap));
						g2d.drawImage(up, (int) (currentx + wp / 2) - (widthIcon / 2), (int) (currenty), widthIcon,
								heightIcon, null);
					}
					...
					if (j != 0) {
						...
						bounds = g2d.getFont().getStringBounds(text, frc);
						width = (int) bounds.getWidth();
						g2d.drawString(namePage + text, (int) currentx + leftHGap,
								(int) (currenty + (hp / 2)) + (heigh / 2) - leftVGap);
						g2d.drawImage(left, (int) currentx, (int) (currenty + (hp / 2)) - (heightIcon / 2), widthIcon,
								heightIcon, null);
					}
	...
	class BookComponentPrintable implements Printable {
		...
		public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
			...
			AffineTransform at = AffineTransform.getTranslateInstance(pageFormat.getImageableX() - x - 2,
