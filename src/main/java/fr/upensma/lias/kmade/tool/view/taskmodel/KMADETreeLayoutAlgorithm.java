...
public class KMADETreeLayoutAlgorithm extends KMADEGraphLayoutAlgorithm {
	...
	public void setPosition(java.awt.Point point, int i) {
		...
		java.awt.Rectangle rectangle1 = new java.awt.Rectangle(
			(int) rectangle2d.getX(), (int) rectangle2d.getY(),
		...
		if (centerRoot) {
		    ...
		    ((KMADEDefaultGraphCell) obj1).setExpressPoint(
			    rectangle1.getBounds().x, rectangle1.getBounds().y);
		    Map map1 = GraphConstants.createAttributes(obj1, "bounds",
			    rectangle1);
		    graph.getGraphLayoutCache().edit(map1, null, null, null);
		    if (orientation == 7 || orientation == 3) {
			graph.setPreferredSize(new java.awt.Dimension(k1
				+ insets.left + insets.right, l + j1
				+ insets.top + insets.bottom));
		    } else {
			graph.setPreferredSize(new java.awt.Dimension(l + j1
				+ insets.left + insets.right, k1 + insets.top
				+ insets.bottom));
		    }
		...
		for (Iterator iterator1 = children.iterator(); iterator1
			.hasNext(); ((TreeNode) iterator1.next()).setPosition(
			point, j)) {
		}
		return;
	    }
	    ...
	    ((KMADEDefaultGraphCell) obj).setExpressPoint(
		    rectangle.getBounds().x, rectangle.getBounds().y);
	    if (orientation == 1 || orientation == 7) {
		y = point.y + levelDistance + i;
	    } else {
		y = point.y - levelDistance - i;
	    }
	    for (Iterator iterator2 = children.iterator(); iterator2.hasNext(); ((TreeNode) iterator2
		    .next()).setPosition(new java.awt.Point(x + point.x, y), j)) {
	    }
	}
	...
	TreeNode treenode1;
	for (Iterator iterator = list.iterator(); iterator.hasNext(); treenode.children
		.add(treenode1)) {
	...
	ArrayList<KMADEDefaultGraphCell> myList = ((KMADEDefaultGraphCell) cellview
		.getCell()).getDirectSubCells();
	ArrayList<CellView> myViewList = new ArrayList<CellView>();
	if (cell.isSonExpanded()) {
	    for (KMADEDefaultGraphCell current : myList) {
		myViewList.add(graph.getGraphLayoutCache().getMapping(current,
			false));
	    }
	} else {
	...
	for (Iterator iterator = list.iterator(); iterator.hasNext(); layout((TreeNode) iterator
		.next())) {
	}
    }

    protected void layout(TreeNode treenode) {
	KMADEDefaultGraphCell cell = (KMADEDefaultGraphCell) treenode.view
		.getCell();
	...
	if (treenode.children.size() != 0 && cell.isSonExpanded()) {
		...
		TreeNode treenode2;
		for (Iterator iterator = treenode.children.iterator(); iterator
			.hasNext(); layout(treenode2)) {
	...
	for (int j = 0; j < treenode.children.size(); j++) {
		...
		int j1 = distance(treenode1.rightContour, treenode3.leftContour)
			/ (l - j);
		i = Math.max(i, j1);
	    }
	...
	for (int k1 = 1; k1 < treenode.children.size(); k1++) {
	    ...
	    merge(treenode.leftContour.next, treenode5.leftContour, k1 * i
		    + treenode.width);
	}

	for (int l1 = treenode.children.size() - 2; l1 >= 0; l1--) {
	    TreeNode treenode6 = (TreeNode) treenode.children.get(l1);
	    merge(treenode.rightContour.next, treenode6.rightContour, l1 * i
		    + treenode.width);
	}
	...
	return treenode1 == null ? (TreeNode) treenode.children.get(0)
		: treenode1;
    }
	...
	for (Iterator iterator = list.iterator(); iterator.hasNext(); ((TreeNode) iterator
		.next()).setPosition(null, 0)) {
	}
    }
