...
public class KMADERecordingSimulationPanel extends JPanel {
    ...
    public static final ImageIcon GO_SIMULATION_SCENARIO_IMAGE = new ImageIcon(
	    KMADETaskModelToolBar.class
		    .getResource(KMADEConstant.GO_SIMULATION_SCENARIO_IMAGE));

    public static final ImageIcon PLAYER_ACTION_IMAGE = new ImageIcon(
	    KMADETaskModelToolBar.class
		    .getResource(KMADEConstant.PLAYER_ACTION_IMAGE));
	...
	this.myListActivableBorder = new TitledBorder(null,
	...
	availableTaskList.getSelectionModel().addListSelectionListener(
	...
	availableTaskList.addMouseListener(new MouseAdapter() {
		...
		SimulationAdaptator
			.goToTask((TokenSimulation) availableTaskList
				.getSelectedValue());
		if (e.getClickCount() == 2) {
		    if (availableTaskList.getSelectedValue() != null) {
			SimulationAdaptator
				.launchActionFromRecord((TokenSimulation) availableTaskList
					.getSelectedValue());
		    }
		}
	    }
	...
	this.myScrollListScenarioBorder = new TitledBorder(null,
	...
	JButton saveScenario = new JButton(
		KMADEReplaySimulationPanel.SAVEAS_SCENARIO_IMAGE);
	...
	JButton redoButton = new JButton(
		KMADEReplaySimulationPanel.PLAYER_FWD_IMAGE);
	...
	if (this.getScenarioListModel().size() > 0) {
	    this.getScenarioListModel().remove(
		    this.getScenarioListModel().size() - 1);
	}
    }
	...
	for (int i = 0; i < this.getScenarioListModel().size(); i++) {
	    myList.add((TokenRecordScenarioSimulation) this
		    .getScenarioListModel().get(i));
	}
	return myList;
    }
}
