...
public final class GraphicEditorAdaptator {
	...
	private static final KMADETaskPropertiesPanel TASK_PROPERTIES_PANEL = KMADEMainFrame.getProjectPanel()
	...
	private static final KMADEReadWriteEventObjectPanel EVENT_OBJECT_PANEL = KMADEMainFrame.getProjectPanel()
			.tableEvent();

	private static final KMADEReadWriteConcreteObjectPanel CONCRETE_OBJECT_PANEL = KMADEMainFrame.getProjectPanel()
			.getPanelCreationObjConc();
	...
	// KMADEHistoryMessageManager.printMessage("Non correspondance entre le modèle
	// Express et les vues JGraph : un conseil recharger le fichier");
	// return;
	// } else {
	...
	public static void setSelectionTask(Task myTask) {
		GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().setSelectionCell(myTask.getRefJTask());
	}
	...
	private static Point centeredTask(Point p) {
		 ...
		 * SwingUtilities.computeStringWidth(fname, ExpressConstant.NEW_NAME_TASK) + 10;
		 */
	...
	public static void addNewUnknownTask(Point p) {
		int mx = (int) (p.getX() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		int my = (int) (p.getY() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		GraphicEditorAdaptator.addNewTask(Executor.INCONNU, centeredTask(new Point(mx, my)));
	}

	public static void addNewAbstractTask(Point p) {
		int mx = (int) (p.getX() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		int my = (int) (p.getY() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		GraphicEditorAdaptator.addNewTask(Executor.ABS, centeredTask(new Point(mx, my)));
	}

	public static void addNewUserTask(Point p) {
		int mx = (int) (p.getX() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		int my = (int) (p.getY() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		GraphicEditorAdaptator.addNewTask(Executor.USER, centeredTask(new Point(mx, my)));
	}

	public static void addNewFeedbackTask(Point p) {
		int mx = (int) (p.getX() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		int my = (int) (p.getY() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		GraphicEditorAdaptator.addNewTask(Executor.SYS, centeredTask(new Point(mx, my)));
	}

	public static void addNewInteractionTask(Point p) {
		int mx = (int) (p.getX() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		int my = (int) (p.getY() / GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getScale());
		GraphicEditorAdaptator.addNewTask(Executor.INT, centeredTask(new Point(mx, my)));
	}
	 ...
	 * l'utilisateur et � la cr�ation d'une t�che par l'application (chargement).
	 * 
	 * @param pTask
	 */
	...
	public static void addNewTaskIntoClipBoard(Task pTask) {
		...
		// KMADEHistoryMessageManager.printMessage("Nom "+tempTask.getName()+"
		// "+tempTask.getExecutant()+" "+tempTask.getDecomposition());
	...
	/**
	 * Cette methode permet d'effectuer un lien graphique entre deux taches dans le
	 * presse-papier.
	 * 
	 * @param oidMere
	 * @param oidFils
	...
	public static void addNewEdgeIntoClipBoard(String oidMere, String oidFils) {
		...
		MAIN_FRAME.getClipBoardDialog().getMyGraph().getGraphLayoutCache().insertEdge(edge, mere.getMotherPort(),
				fils.getSonPort());
	}
	...
	private static void addNewEdge(String oidMere, String oidFils) {
		...
		KMADEDefaultEdge edge = new KMADEDefaultEdge();
		GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getGraphLayoutCache().insertEdge(edge, mere.getMotherPort(),
				fils.getSonPort());
	}
		...
		if (source != null && target != null) {
			if ((source.getParent() instanceof KMADEDefaultGraphCell)
				...
				if (source.getParent() == target.getParent()) {
					...
					// Cr�ation de la liaison dans la partie graphique.
					KMADEDefaultEdge edge = new KMADEDefaultEdge();
					GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getGraphLayoutCache().insertEdge(edge, source,
							target);

					/* AG */
					/*
					 * Récuperer l'etat actuel est le mettre dans la pile pour le Undo
					 */
					Vector<Object> vec = new Vector<Object>();
					vec.add(edge);
					vec.add(sourceCell);
					vec.add(targetCell);
					GraphicEditorAdaptator.getTaskModelPanel().getUndoRedoManager().whatUndo.push("Edge");
					GraphicEditorAdaptator.getTaskModelPanel().getUndoRedoManager().getObjToUndo().push(edge);
					/**/
		...
		for (int i = 0; i < cells.length; i++) {
			if (cells[i] instanceof KMADEDefaultPort) {
				...
				while (iterator.hasNext()) {
					...
					GraphicEditorAdaptator.getCurrentGraphLayoutCache().remove(tempo);
					GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getModel().remove(tempo);
				}
			}
		}
		// La t�che en elle-m�me.
		GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getModel().remove(cells);
	}
	...
	private static void removeEdge(KMADEDefaultEdge currentEdge) {
		KMADEDefaultGraphCell motherCell = ((KMADEDefaultEdge) currentEdge).getMotherCell();
		...
		GraphicEditorAdaptator.getTaskModelPanel().getUndoRedoManager().getObjToUndo().push(edgeToRemove);
		/**/

		// Suppression dans Express du lien.
		ExpressTask.removeEdge(motherCell.getTask(), ((KMADEDefaultEdge) currentEdge).getSonCell().getTask());

		// Suppression dans la partie graphique.
		GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getGraphLayoutCache().remove(new Object[] { currentEdge });
		GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getModel().remove(new Object[] { currentEdge });

		/* AG */
		GraphicEditorAdaptator.getTaskModelPanel().getUndoRedoManager().whatUndo.push("EdgeRemove");
		/**/
	}
	...
	public static void removeTaskAndEdge() {
		...
		if (cellSelected.length == 0) {
			...
			if (cellSelected[0] instanceof KMADEDefaultGraphCell) {
				...
				GraphicEditorAdaptator.removeEdge((KMADEDefaultEdge) cellSelected[0]);
			}
		} else if (cellSelected.length > 1) {
			...
			for (int i = 0; i < cellSelected.length; i++) {
				if (cellSelected[i] instanceof KMADEDefaultGraphCell) {
					GraphicEditorAdaptator.removeTask((KMADEDefaultGraphCell) cellSelected[i]);
				} else {
					if (((KMADEDefaultEdge) cellSelected[i]).getMotherCell() != null) {
						GraphicEditorAdaptator.removeEdge((KMADEDefaultEdge) cellSelected[i]);
					}
				}
			}
		}
	...
	/**
	 * Cette m�thode permet de mettre � jour les points des t�ches dans l'express
	 */
		...
		if (GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getSelectionCount() == 1) {
			...
			for (int i = 0; i < cellSelected.length; i++) {
				if (cellSelected[i] instanceof KMADEDefaultGraphCell) {
					...
					if (monPoint != null) {
						ExpressTask.setTaskPoint((int) monPoint.getX(), (int) monPoint.getY(),
								((KMADEDefaultGraphCell) cellSelected[i]).getTask());
					}
				}
			}
	...
	public static void setFewAttributesSelectedTask() {
		GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().startEditingAtCell(myDefaultGraphCellRef);
	}
	...
	public static JPopupMenu showPopupMenu(Point pt) {
		...
		if (cellSelected.length == 0) {
			return GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().createPopupMenuForAdd(pt);
		}

		// Une tâche de sélectionnée.
		if (cellSelected.length == 1) {
			if (cellSelected[0] instanceof KMADEDefaultEdge) {
				return GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().createPopupMenuForOneEdge(pt);
			} else {
				return GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().createPopupMenuForOneTask(pt);
			}
		}
		...
		if (cellSelected.length > 1) {
			...
			if (existedCell) {
				return GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().createPopupMenuForSeveralTasks(pt);
			} else {
				return GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().createPopupMenuForSeveralEdges(pt);
			}
		}
		return null;
	}
	 ...
	 * 
	 * @param pDefaultGraphCell : t�che s�lectionn�e
	 */
			...
			public Object construct() {
				...
				for (int i = 0; i < tacheToBeCreated.length && !GraphicEditorAdaptator.canceled; i++) {
					...
					if (tacheToBeCreated[i].hasNoPoint()
							&& tacheToBeCreated[i].getNumber().contains(ExpressConstant.ROOT_TASK_NAME)) {
	...
	public static void setGridSize() {
		...
		Object myObject = JOptionPane.showInputDialog(GraphicEditorAdaptator.getMainFrame(),
				KMADEConstant.GRID_SIZE_ACTION_MESSAGE, KMADEConstant.INPUT_GRID_SIZE_MESSAGE,
				JOptionPane.QUESTION_MESSAGE,
				new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.ASK_DIALOG_IMAGE)), null,
	...
	public static void copyAction() {
		...
		Task[] newTasks = ExpressTask.copyElementsIntoClipBoard(taskList, edgeOIDList);
	...
	public static boolean removeSelectedItem(String type, int[] r, ArrayList<Object[]> list, Window myOwner,
			String title) {
	...
	public static void setJustifyTaskModel() {
		...
		KMADEGraphLayoutAlgorithm.applyLayout(GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph(), layout,
				GraphicEditorAdaptator.TASK_MODEL_PANEL.getJGraph().getRoots(), new Object[] { myDefaultGraphCellRef });
	}
