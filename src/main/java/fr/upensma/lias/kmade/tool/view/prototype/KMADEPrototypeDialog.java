...
public class KMADEPrototypeDialog extends JFrame {
	...
	public KMADEPrototypeDialog() {
		...
		KMADEEnhancedSplitPane myCenterPanel = KMADEEnhancedSplitPane
				.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftPanel, rightPanel);
		...
		TitledBorder rightBotTaskPanelTitle = new TitledBorder(null, KMADEConstant.PROTOTYPING_TOOL_CONTROL_TITLE,
		...
		TitledBorder leftTitle = new TitledBorder(BorderFactory.createLineBorder(Color.BLACK, 2),
				KMADEConstant.PROTOTYPING_TOOL_CURRENT_TASK, TitledBorder.LEFT, TitledBorder.TOP,
		 ...
		 * 
		 * @Override public void windowOpened(WindowEvent e) { Date date =
		 * Calendar.getInstance().getTime(); System.err.println(date +
		 * " frame windowOpened"); log.add(date + " frame windowOpened"); }
		 * 
		 * @Override public void windowIconified(WindowEvent e) { Date date =
		 * Calendar.getInstance().getTime(); System.err.println(date +
		 * " frame windowIconified"); log.add(date + " frame windowIconified"); }
		 * 
		 * @Override public void windowDeiconified(WindowEvent e) { Date date =
		 * Calendar.getInstance().getTime(); System.err.println(date +
		 * "frame windowDeiconified"); log.add(date + " frame windowDeiconified"); }
		 * 
		 * @Override public void windowDeactivated(WindowEvent e) { Date date =
		 * Calendar.getInstance().getTime(); System.err.println(date +
		 * "frame windowDeactivated"); log.add(date + "frame windowDeactivated");
		 * ecrirelog();
		 * 
		 * }
		 * 
		 * @Override public void windowClosing(WindowEvent e) { Date date =
		 * Calendar.getInstance().getTime(); System.err.println(date +
		 * "frame windowClosing"); log.add(date + "frame windowClosing"); }
		 * 
		 * @Override public void windowClosed(WindowEvent e) { Date date =
		 * Calendar.getInstance().getTime(); System.err.println(date +
		 * "frame windowClosed"); log.add(date + "frame windowClosed"); }
		 * 
		 * @Override public void windowActivated(WindowEvent e) { Date date =
		 * Calendar.getInstance().getTime(); System.err.println(date +
		 * " frame windowActivated"); log.add(date + "frame windowActivated"); } });
		 */
		/*
		 ...
		 * }
		 * 
		 * @Override public void mouseClicked(MouseEvent arg0) { Date date =
		 * Calendar.getInstance().getTime(); System.err.println(date + "frame clicked");
	...
	public void setCurrentTask(final Task currentTask) {
		...
		case ABS:
			executantImage = new ImageIcon(
					KMADETaskModelToolBar.class.getResource(KMADEConstant.ABSTRACT_TASK_48_IMAGE));
		...
		case INT:
			executantImage = new ImageIcon(
					KMADETaskModelToolBar.class.getResource(KMADEConstant.INTERACTIF_TASK_48_IMAGE));
			executantText += KMADEConstant.PROTOTYPING_TOOL_EXECUTANT_INT;
			break;
		case SYS:
			executantImage = new ImageIcon(
					KMADETaskModelToolBar.class.getResource(KMADEConstant.FEEDBACK_TASK_48_IMAGE));
			executantText += KMADEConstant.PROTOTYPING_TOOL_EXECUTANT_SYS;
			break;
		case USER:
			executantImage = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.USER_TASK_48_IMAGE));
		...
		TitledBorder ordoPanelTitle = new TitledBorder(null,
				KMADEConstant.PROTOTYPING_TOOL_DECOMPOSITION_TITLE + " : " + currentTask.getOrdering().getValue(),
				TitledBorder.LEFT, TitledBorder.TOP, KMADEConstant.TITLE_PROTO_TASK_FONT);
		...
		TitledBorder iterationTitle = new TitledBorder(null, KMADEConstant.PROTOTYPING_TOOL_ITERATION_TOOLTIP + " : ",
				TitledBorder.LEFT, TitledBorder.TOP, KMADEConstant.TITLE_PROTO_TASK_FONT);
		...
		TitledBorder myUserBorder = new TitledBorder(null, KMADEConstant.PROTOTYPING_TOOL_DESCRIPTION_TITLE + " : ",
				TitledBorder.LEFT, TitledBorder.TOP, KMADEConstant.TITLE_PROTO_TASK_FONT);
	 ...
	 */
	public void setExecutableTask(final Task t, boolean accessible, boolean enabled, int number, boolean haveChoice,
			ChoiceEnum choice) {
		...
		if (!t.isLeaf()) {
			...
			toolTip += "<HTML>" + KMADEConstant.PROTOTYPING_TOOL_SUBTASK_DECOMP_TOOLTIP1 + " " + t.getChildren().size()
					+ " " + KMADEConstant.PROTOTYPING_TOOL_SUBTASK_DECOMP_TOOLTIP2;
		} else {
			toolTip += "<HTML>" + KMADEConstant.PROTOTYPING_TOOL_SUBTASK_TERMINAL_TOOLTIP;
		}
		if (t.getDescription() != null && !t.getDescription().equals("")) {
			toolTip += "<BR> " + KMADEConstant.PROTOTYPING_TOOL_SUBTASK_DESCRIPTION_TOOLTIP + " : "
					+ t.getDescription();
		}
		toolTip += "</HTML>";
		...
		case ABS: /*
					 * executantImage =new ImageIcon(KMADETaskModelToolBar.class.getResource
					 * (KMADEConstant.ABSTRACT_TASK_16_IMAGE))
					 */
			;
			break;
		case INCONNU:/*
						 * executantImage =new ImageIcon(KMADETaskModelToolBar.class
		...
		case SYS:
			...
			break;
		case USER:
			executantImage = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.USER_TASK_16_IMAGE));
			break;
		}
		...
		if (t.getPreExpression().getDescription() != null && !t.getPreExpression().getDescription().equals("")) {
			String precond = KMADEConstant.PROTOTYPING_TOOL_SUBTASK_PRECONDITION + " : "
	...
	public void setEnabledEnd(final Task t, boolean b) {
		...
		if (t != null) {
			...
			rightBotTaskPanel.setBorder(new TitledBorder(null, KMADEConstant.PROTOTYPING_TOOL_CONTROL_TITLE,
					TitledBorder.LEFT, TitledBorder.TOP, KMADEConstant.TITLE_PROTO_TASK_FONT));
		}
	...
	public void setIterationEnabled(Task t, boolean existed, ChoiceEnum enabled) {
		...
		if (existed) {
			groupButton = new trueFalseIndeterminateGroupButton(t.getIterExpression().getDescription(), enabled);
	...
	public void ecrirelog() {
		/*
		 * Date date = Calendar.getInstance().getTime(); String name =
		 * ExpressTask.getRootTasks().get(0).getName().replaceAll("\\s", "");
		 * 
		 * @SuppressWarnings("deprecation") String adressedufichier =
		 * System.getProperty("user.dir") + "/"+"KMC"+
		 * name+date.getHours()+"h"+date.getMinutes()+"min.txt"; FileWriter fw; try { fw
		 * = new FileWriter(adressedufichier, true); BufferedWriter output = new
		 * BufferedWriter(fw); for (String str : log) { output.write(str); } for(String
		 * str : KMADEHistoricPanel.log){ output.write(str); } output.flush();
		 * output.close(); log.clear(); KMADEHistoricPanel.log.clear(); } catch
