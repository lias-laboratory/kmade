...
public final class ExpressSimulation {
	...
	ArrayList<TokenSimulation> toto = ExpressSimulation
		.searchActivableTask(currentTask);
	return toto;
    }
	...
	if (myTache.isInterruptible()
		&& (!myTache.getStateSimulation().isTerminalState())) {
	    if (!myTache.getStateSimulation().isSuspended()) {
		if (myTache.getStateSimulation().isPassive()
			|| myTache.getStateSimulation().isActive()) {
		    myTacheList.add(new TokenSimulation(myTache,
			    TokenSimulation.INTERROMPRE));
		}
	    } else {
		if (myTache.getExecutor().equals(Executor.SYS)) {
		    // Un exécutant système est interrompue automatiquement
		    // Vérifie avec la tache mere s'il existe une sous-tache de
		    // plus haute priorite
			...
			// Etat Terminal
			if (!myMother.getChildren().get(i).getStateSimulation()
				.isTerminalState()) {
			    etat = myTache.getImportance().isMoreImportant(
				    myMother.getChildren().get(i)
					    .getImportance());
			}
		    }
		    if (!etat) {
			myTacheList.add(new TokenSimulation(myTache,
				TokenSimulation.REPRENDRE));
		    }
		} else {
		    // Les autres exécutants peuvent reprendre l'exécution d'une
		    // tâche.
		    if (myTache.isOptional()) {
			myTacheList.add(new TokenSimulation(myTache,
				TokenSimulation.ABANDONNER));
		    }
		    myTacheList.add(new TokenSimulation(myTache,
			    TokenSimulation.REPRENDRE));
		}
	...
	if (myTache.getOrdering().equals((Decomposition.ELE))) {
	    if (myTache.getStateSimulation().isPassive()
		    || myTache.getStateSimulation().isActive()) {
		myTacheList.add(new TokenSimulation(myTache,
			TokenSimulation.EXECUTER));
	    }
	    if (myTache.getStateSimulation().isPassive()
		    && myTache.isOptional()) {
		if (!myTache.isRoot()
			&& !myTache.getMother().getOrdering()
				.equals(Decomposition.ALT)
			&& !myTache.getExecutor().equals(Executor.SYS)) {
		    myTacheList.add(new TokenSimulation(myTache,
			    TokenSimulation.PASSER));
		}
	    }
	    return myTacheList;
	}
	...
	for (Task current : myTache.getChildren()) {
	    if (!myTache.getStateSimulation().isSuspended()) {
		ArrayList<TokenSimulation> myList = ExpressSimulation
			.searchActivableTask(current);
    ...
    public static boolean isExecutableTask(boolean exe, boolean event,
	    boolean pre, int value, Task myCurrentTask) {
	// Cette méthode s'occupe de gérer la première catégorie
	KMADEHistoryMessageManager
		.printlnMessage(KMADEConstant.EXECUTE_TASK_TRAITEMENT_MESSAGE
			+ " : " + myCurrentTask.getName());
	KMADEHistoryMessageManager.printlnMessage(" * "
		+ KMADEConstant.EXECUTION_CONSTRAINT_MESSAGE);

	// Est-ce le bon utilisateur qui exécute.
	KMADEHistoryMessageManager.printMessage("  - "
		+ KMADEConstant.USER_EXECUTION_CONSTRAINT_MESSAGE + " : ");
	if (exe) {
		...
		if (trouve) {
		    KMADEHistoryMessageManager.printlnMessage(selectUser
			    .getName()
			    + " "
			    + KMADEConstant.CAN_EXECUTE_TASK_MESSAGE
			    + " "
			    + myCurrentTask.getName());
		} else {
		    KMADEHistoryMessageManager.printlnMessage(selectUser
			    .getName()
			    + " "
			    + KMADEConstant.CAN_NOT_EXECUTE_TASK_MESSAGE
			    + " "
			    + myCurrentTask.getName());
		    KMADEHistoryMessageManager.printlnMessage("");
		    return false;
		...
		KMADEHistoryMessageManager
			.printlnMessage(KMADEConstant.AUTHORIZED_EXECUTER_USER_MESSAGE);
	    }
	} else {
	    KMADEHistoryMessageManager
		    .printlnMessage(KMADEConstant.DISABLED_CONSTRAINT_MESSAGE);
	}

	// Est-ce le bon événement déclencheur.
	KMADEHistoryMessageManager.printMessage("  - "
		+ KMADEConstant.EVENT_TRIGGER_CONSTRAINT_MESSAGE + " : ");
	if (event) {
	    Event eventTask = myCurrentTask.getRaisingEvent();
	    if (eventTask == null) {
		KMADEHistoryMessageManager
			.printlnMessage(KMADEConstant.NO_TRIGGER_EVENT_CONSTRAINT_MESSAGE);
	    } else {
		if (ExpressEvent.isExistingEvent(eventTask)) {
		    KMADEHistoryMessageManager.printlnMessage(eventTask
			    .getName()
			    + " "
			    + KMADEConstant.CAN_TRIGGER_TASK_MESSAGE
			    + " "
			    + myCurrentTask.getName());
		} else {
		    KMADEHistoryMessageManager.printlnMessage(eventTask
			    .getName()
			    + " "
			    + KMADEConstant.CAN_NOT_TRIGGER_CONSTRAINT_MESSAGE
			    + " " + myCurrentTask.getName());
		}
	    }
	} else {
	...
	KMADEHistoryMessageManager.printMessage("  - "
		+ KMADEConstant.PRECONDITION_CONSTRAINT_MESSAGE + " : ");
	if (pre) {
	    try {
		if (myCurrentTask.getPreExpression().getNodeExpression() == null) {
		    KMADEHistoryMessageManager.printMessage(myCurrentTask
			    .getPreExpression().getFormalText());
		} else {
		    KMADEHistoryMessageManager.printMessage(ExpressSimulation
			    .getLinearExpressionWithUserValues(myCurrentTask
				    .getPreExpression().getNodeExpression()
				    .getLinearExpression()));
		}

		myCurrentTask.getPreExpression().getNodeExpression()
			.evaluateNode(null);
		if ((Boolean) myCurrentTask.getPreExpression()
			.getNodeExpression().getNodeValue()) {
		    KMADEHistoryMessageManager
			    .printlnMessage(" -> "
				    + KMADEConstant.PRECONDITION_CONSTRAINTS_RESPECTED_STATE_MESSAGE);
		} else {
		    KMADEHistoryMessageManager
			    .printlnMessage(" -> "
				    + KMADEConstant.PRECONDITION_CONSTRAINTS_NO_RESPECTED_STATE_MESSAGE);
		    KMADEHistoryMessageManager.printlnMessage("");
		    return false;
		}
    ...
    public static boolean setExecuter(Task myTask, boolean post, boolean iter,
	    boolean trigevent, boolean event) {
	// Faire le traitement. Première catégorie, seconde catégorie et
	// troisième catégorie.

	if (ExpressIteration.isFinished(myTask)) {
	    myTask.getStateSimulation().setFinished();
	    KMADEHistoryMessageManager.printlnMessage(" * "
		    + KMADEConstant.ACTION_CONSTRAINT_MESSAGE);
	    KMADEHistoryMessageManager.printlnMessage("  - "
		    + KMADEConstant.ITERATION_FINISH_NO_ACTION);
	    return true;
	} else {
	    ...
	    KMADEHistoryMessageManager.printMessage("  - "
		    + KMADEConstant.GENERATE_EVENTS_CONSTRAINT_MESSAGE + " : ");
	    if (trigevent) {
		String temp = ExpressEvent.generateFiringEvent(myTask);
		if (temp.length() == 0) {
		    KMADEHistoryMessageManager
			    .printlnMessage(KMADEConstant.NO_GENERATED_EVENTS_CONSTRAINT_MESSAGE);
		} else {
		...
		KMADEHistoryMessageManager
			.printMessage("  - "
				+ KMADEConstant.EFFETSDEBORD_CONSTRAINT_MESSAGE
				+ " : ");

		if (myTask.getEffetsDeBordExpression().getNodeExpression() == null) {
		    KMADEHistoryMessageManager.printMessage(myTask
			    .getEffetsDeBordExpression().getFormalText());
		} else {
		    KMADEHistoryMessageManager
			    .printMessage(ExpressSimulation
				    .getLinearExpressionWithUserValues(myTask
		...
		try {
		    myTask.getEffetsDeBordExpression().getNodeExpression()
			    .evaluateNode(null);
		    KMADEHistoryMessageManager
			    .printlnMessage(" -> "
				    + KMADEConstant.EFFETSDEBORD_EXECUTED_STATE_MESSAGE);
		} catch (SemanticErrorException e) {
		    KMADEHistoryMessageManager
			    .printlnMessage(" -> "
				    + KMADEConstant.EFFETSDEBORD_NO_EXECUTED_STATE_MESSAGE
				    + " , " + e.getMessage());
		    KMADEHistoryMessageManager.printlnMessage("");
		    return false;
		} catch (SemanticUnknownException e) {
		...
		KMADEHistoryMessageManager.printlnMessage("  - "
			+ KMADEConstant.EFFETSDEBORD_CONSTRAINT_MESSAGE + " : "
			+ KMADEConstant.DISABLED_CONSTRAINT_MESSAGE);
	    }
	...
	for (Task current : myTache.getChildren()) {
		...
		// Ne rien faire dans cette sous-tache. Elle est encore
		// suspendue.
	    } else {
	...
	if (refTask.getStateSimulation().isPassive()
		&& refTask.getChildren().get(0).getStateSimulation()
			.isNotAccessible()) {
	...
	for (Task current : refTask.getChildren()) {
	    ...
	    if ((finishedOne || passeeOne || noresumedOne)
		    && current.getStateSimulation().isNotAccessible()) {
		// La tâche précédente est une tâche terminée et celle-ci est
		// non accessible
		refTask.getStateSimulation().setActive();
	    ...
	    if (current.getStateSimulation().isPassive()
		    || current.getStateSimulation().isActive()) {
	...
	if ((refTask.getStateSimulation().isPassive() || refTask
		.getStateSimulation().isActive())
		&& refTask.getChildren().get(0).getStateSimulation()
			.isNotAccessible()) {
	    // Pour la verification on a simplement besoin de verifier la
	    // premiere tache a NotAccessible car cet etat
	    // n'est jamais repris
	    for (Task current : refTask.getChildren()) {
		current.getStateSimulation().setPassive();
	    }
	...
	for (Task current : refTask.getChildren()) {
	    if (activedOne) {
		// La tache precedente est une tache terminee et celle-ci est
		// obligatoiteremnt en passive
		refTask.getStateSimulation().setActive();
	    ...
	    finishedOne = finishedOne
		    | current.getStateSimulation().isFinished();
	}
    ...
    /**
     * Remarques : - L'interruption est proposée quand une tâche de plus haute
     * priorité est trouvée lors d'une concurrence ou d'une ind�pendance. Si pas
     * d'interruption pour toutes les t�ches
     */
	...
	for (Task current : refTask.getChildren()) {
		...
		temp = Importance.getMoreImportant(temp,
	...
	// Maintenant qu'on a fait le tour, il faut interrompre les taches de
	// priorite plus faible.
	boolean activedOne = false;
	for (Task current : refTask.getChildren()) {
		...
		// Faut stocker une valeur qui permet de dire explicitement que
		// l'on souhaite reprendre.
	...
	for (Task current : refTask.getChildren()) {
	    ...
	    notAccessibleAll = notAccessibleAll
		    | !current.getStateSimulation().isNotAccessible();
	}
	...
	if (activedOne) {
	    // Si la derni�re t�che active => rendre NotAccessibleAll les taches
	    // Passive.
	    for (Task subCurrent : refTask.getChildren()) {
		if (subCurrent.getStateSimulation().isPassive()) {
		    ExpressSimulation.noAccessibledTasks(subCurrent);
	...
	if (finishedOne) {
	    // Si la derni�re t�che finished => rendre Passive les taches
	    // NotAccessible.
	    for (Task subCurrent : refTask.getChildren()) {
		if (subCurrent.getStateSimulation().isNotAccessible()) {
		    subCurrent.getStateSimulation().setPassive();
	...
	for (Object tt : myList) {
		...
		if (((ConcreteObjectType) tt).getUserConcreteObject() != null) {
		    result += ((ConcreteObjectType) tt).getUserConcreteObject()
			    .toString();
		} else {
