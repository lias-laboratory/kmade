...
public final class ExpressSimulation {
	...
	public static ArrayList<TokenSimulation> searchActivableTask(Task myTache) {
				...
				if (myTache.getExecutor().equals(Executor.SYS)) {
					...
					for (int i = 0; i < myMother.getChildren().size() && !etat; i++) {
						...
						if (!myMother.getChildren().get(i).getStateSimulation().isTerminalState()) {
							etat = myTache.getImportance()
									.isMoreImportant(myMother.getChildren().get(i).getImportance());
						}
					}
					if (!etat) {
		...
		if (myTache.getOrdering().equals((Decomposition.ELE))) {
			...
			if (myTache.getStateSimulation().isPassive() && myTache.isOptional()) {
				if (!myTache.isRoot() && !myTache.getMother().getOrdering().equals(Decomposition.ALT)
	...
	public static boolean isExecutableTask(boolean exe, boolean event, boolean pre, int value, Task myCurrentTask) {
		...
		KMADEHistoryMessageManager
				.printlnMessage(KMADEConstant.EXECUTE_TASK_TRAITEMENT_MESSAGE + " : " + myCurrentTask.getName());
		...
		if (exe) {
			...
			if (myCurrentTask.getActors().size() != 0 && !isSystem) {
				...
				if (trouve) {
					KMADEHistoryMessageManager.printlnMessage(selectUser.getName() + " "
							+ KMADEConstant.CAN_EXECUTE_TASK_MESSAGE + " " + myCurrentTask.getName());
				} else {
					KMADEHistoryMessageManager.printlnMessage(selectUser.getName() + " "
							+ KMADEConstant.CAN_NOT_EXECUTE_TASK_MESSAGE + " " + myCurrentTask.getName());
		...
		if (event) {
			...
			if (eventTask == null) {
				...
				if (ExpressEvent.isExistingEvent(eventTask)) {
					KMADEHistoryMessageManager.printlnMessage(eventTask.getName() + " "
							+ KMADEConstant.CAN_TRIGGER_TASK_MESSAGE + " " + myCurrentTask.getName());
				} else {
					KMADEHistoryMessageManager.printlnMessage(eventTask.getName() + " "
							+ KMADEConstant.CAN_NOT_TRIGGER_CONSTRAINT_MESSAGE + " " + myCurrentTask.getName());
				}
			}
		} else {
			KMADEHistoryMessageManager.printlnMessage(KMADEConstant.DISABLED_CONSTRAINT_MESSAGE);
		...
		if (pre) {
			try {
				if (myCurrentTask.getPreExpression().getNodeExpression() == null) {
					...
					KMADEHistoryMessageManager.printMessage(ExpressSimulation.getLinearExpressionWithUserValues(
							myCurrentTask.getPreExpression().getNodeExpression().getLinearExpression()));
				}
				...
				if ((Boolean) myCurrentTask.getPreExpression().getNodeExpression().getNodeValue()) {
					KMADEHistoryMessageManager
							.printlnMessage(" -> " + KMADEConstant.PRECONDITION_CONSTRAINTS_RESPECTED_STATE_MESSAGE);
				} else {
					KMADEHistoryMessageManager
							.printlnMessage(" -> " + KMADEConstant.PRECONDITION_CONSTRAINTS_NO_RESPECTED_STATE_MESSAGE);
					KMADEHistoryMessageManager.printlnMessage("");
					return false;
				}
		...
		if (ExpressIteration.isFinished(myTask)) {
			...
			if (post) {
				...
				if (myTask.getEffetsDeBordExpression().getNodeExpression() == null) {
					...
					KMADEHistoryMessageManager.printMessage(ExpressSimulation.getLinearExpressionWithUserValues(
							myTask.getEffetsDeBordExpression().getNodeExpression().getLinearExpression()));
				}

				try {
					myTask.getEffetsDeBordExpression().getNodeExpression().evaluateNode(null);
					KMADEHistoryMessageManager
							.printlnMessage(" -> " + KMADEConstant.EFFETSDEBORD_EXECUTED_STATE_MESSAGE);
				} catch (SemanticErrorException e) {
					KMADEHistoryMessageManager.printlnMessage(
							" -> " + KMADEConstant.EFFETSDEBORD_NO_EXECUTED_STATE_MESSAGE + " , " + e.getMessage());
					KMADEHistoryMessageManager.printlnMessage("");
					return false;
				} catch (SemanticUnknownException e) {
					KMADEHistoryMessageManager.printlnMessage(
				...
				KMADEHistoryMessageManager.printlnMessage("  - " + KMADEConstant.EFFETSDEBORD_CONSTRAINT_MESSAGE + " : "
	...
	private static void sequenceOperatorEffects(Task refTask) {
		...
		if (refTask.getStateSimulation().isPassive()
				&& refTask.getChildren().get(0).getStateSimulation().isNotAccessible()) {
	...
	private static void choiceOperatorEffects(Task refTask) {
		...
		if ((refTask.getStateSimulation().isPassive() || refTask.getStateSimulation().isActive())
				&& refTask.getChildren().get(0).getStateSimulation().isNotAccessible()) {
	...
	private static void concurencyOperatorEffects(Task refTask) {
		...
		boolean activedOne = false;
		for (Task current : refTask.getChildren()) {
			if (current.isInterruptible() && current.getImportance().isMoreImportant(temp)
					&& !current.getStateSimulation().isSuspended()
					&& !current.getImportance().equals(Importance.INCONNU) && !current.getStateSimulation().resumeTag) {
