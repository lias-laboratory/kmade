 ...
 */
public class ConcreteObjectPanel extends JPanel implements MouseMotionListener, MouseListener {
	 ...
	 * 
	 * @param data base to create and place the objects already created
	 */
	public ConcreteObjectPanel(ExpressDB bdd) {
		...
		// Cell views
		this.concreteObjectGraph.getGraphLayoutCache().setFactory(new DefaultCellViewFactory() {
			...
			public CellView createView(GraphModel gM, Object cell) {
				...
				if (cell instanceof ConcreteObjectCell) {
					return new ConcreteObjectVertexView((ConcreteObjectCell) cell, concreteObjectGraph);
				} else if (cell instanceof GroupDefaultGraphCell) {

					if (cell instanceof SetCell) {
						return new SetVertexView((SetCell) cell, concreteObjectGraph);
					} else if (cell instanceof ListCell) {
						return new ListVertexView((ListCell) cell, concreteObjectGraph);
					} else if (cell instanceof StackCell) {
						return new StackVertexView((StackCell) cell, concreteObjectGraph);
					} else if (cell instanceof ArrayCell) {
						return new ArrayVertexView((ArrayCell) cell, concreteObjectGraph);
					} else if (cell instanceof SingletonCell) {
						return new SingletonVertexView((SingletonCell) cell, concreteObjectGraph);
					} else
					...
					concreteObjectGraph.getGraphLayoutCache().insert(newCell);
					Object[] aux = { cell };
					concreteObjectGraph.getGraphLayoutCache().remove(aux);
					// Select the new cell created
					concreteObjectGraph.clearSelection();
					concreteObjectGraph.setSelectionCell(newCell);
					return new ConcreteObjectVertexView(newCell, concreteObjectGraph);
				} else {
					return super.createView(gM, cell);
				}
			}
			...
			private ConcreteObjectCell createConcreteFromAbstract(AbstractObjectCell cell) {
				ObjetAbstrait ref = ((ObjetAbstrait) ((AbstractObjectCell) cell).getObject());
				Oid oidObjCon = InterfaceExpressJava.createEntity(ExpressConstant.METAOBJECT_PACKAGE,
						ExpressConstant.CONCRETE_OBJECT_CLASS);
				ObjetConcret o = (ObjetConcret) InterfaceExpressJava.bdd.prendre(oidObjCon);
				o.setName(ref.getName() + " concret");
				o.setUtiliseParClass(ref);

				ExpressConcreteAttribut.createConcreteAttribut(o, ref);

				ConcreteObjectCell newCell = new ConcreteObjectCell(o, ((AbstractObjectCell) cell).getX(),
						((AbstractObjectCell) cell).getY());

				Object[] cells = { cell };
				concreteObjectGraph.getGraphLayoutCache().remove(cells);
				concreteObjectCreation.getGraphLayoutCache().remove(cells);

				int y = 0;
				CellView[] allViews = concreteObjectCreation.getGraphLayoutCache().getAllViews();

				for (int i = allViews.length - 1; i >= 0; i--) {
					if (allViews[i].getBounds().contains(0, y)) {
						if (!((AbstractObjectCell) allViews[i].getCell()).getObject().equals(cell.getObject()))

							y += allViews[i].getBounds().getHeight() + 10;
					}
				}
				GraphConstants.setBounds(cell.getAttributes(),
						new Rectangle2D.Double(0, y, cell.getWidth(), cell.getHeight()));
				concreteObjectCreation.getGraphLayoutCache().insert(cell);
		...
		this.concreteObjectCreation.getGraphLayoutCache().setFactory(new DefaultCellViewFactory() {

			private static final long serialVersionUID = 921433043328434236L;

			public CellView createView(GraphModel gM, Object cell) {
				if (cell instanceof AbstractObjectCell) {
					return new AbstractObjectVertexView(cell, concreteObjectCreation);
				} else {
					return super.createView(gM, cell);
				}
			}
		...
		TitledBorder border2 = BorderFactory.createTitledBorder(KMADEConstant.ABSTRACT_OBJECT_TABBEDPANE_TITLE_NAME);
		border2.setTitleJustification(TitledBorder.CENTER);
		east.setBorder(border2);

		splitPane = KMADEEnhancedSplitPane.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT, west, east);
	 ...
	 * 
	 * @param list of abstract objects already created
	 */
	public void drawConcreteObjectsCells(List<ObjetAbstrait> abstracts) {
		if (!abstracts.isEmpty()) {
			...
			for (ObjetAbstrait oA : abstracts) {
				...
				y += GraphConstants.getBounds(newCell.getAttributes()).getHeight() + 10;
				...
				if (!oA.getInverseObjConc().isEmpty()) {
					for (ObjetConcret oC : oA.getInverseObjConc()) {
						if (oC.getAppartientGroupe() == null) {
							if (oC.getPoint() == null) {
								concreteObjectGraph.getGraphLayoutCache().insert(new ConcreteObjectCell(oC, x, y2));
								x += 10;
								y2 += 10;
							} else
								concreteObjectGraph.getGraphLayoutCache()
										.insert(new ConcreteObjectCell(oC, oC.getPoint()));
						}
					}
				}
				// Groups
				if (!oA.getInverseGroupe().isEmpty()) {
					for (Groupe g : oA.getInverseGroupe()) {
						...
						x += GraphConstants.getBounds(group.getAttributes()).getWidth() + 10;
					}
				}
			}
		}
	...
	public void clearGraph() {
		CellView[] cells = concreteObjectGraph.getGraphLayoutCache().getAllViews();
	...
	public void mousePressed(MouseEvent e) {
		selected = concreteObjectGraph.getFirstCellForLocation(e.getX(), e.getY());
	}

	public void mouseDragged(MouseEvent e) {
		if (selected != null && selected instanceof GroupDefaultGraphCell)
			((GroupDefaultVertexView) ((GroupDefaultGraphCell) selected).getInvView()).getHandle().mouseDragged(e);
	}

	public void mouseReleased(MouseEvent e) {
		concreteObjectGraph.setCursor(Cursor.getDefaultCursor());
		if (selected instanceof GroupDefaultGraphCell && !GraphConstants
				.getBounds(((GroupDefaultGraphCell) selected).getAttributes()).contains(e.getPoint())) {

			((GroupDefaultVertexView) ((GroupDefaultGraphCell) selected).getInvView()).getHandle().mouseReleased(e);
			selected = null;
		}
		...
		// Drop in a group
			...
			((ConcreteObjectCell) selected).getObject().getPoint().setX((int) e.getPoint().getX());
			((ConcreteObjectCell) selected).getObject().getPoint().setY((int) e.getPoint().getY());

			Object cell = null;
			Object[] cellViews = concreteObjectGraph.getGraphLayoutCache().getAllViews();
			for (Object view : cellViews) {
				if (view instanceof GroupDefaultVertexView) {
					if (GraphConstants
							.getBounds(
									((GroupDefaultGraphCell) ((GroupDefaultVertexView) view).getCell()).getAttributes())
							.contains(e.getPoint()) && !((CellView) view).getCell().equals(selected))
						cell = ((GroupDefaultVertexView) view).getCell();
				}
			}

			if (cell != null && cell instanceof GroupDefaultGraphCell)
				((GroupDefaultVertexView) ((GroupDefaultGraphCell) cell).getInvView()).getHandle().mouseReleased(e);
		}

		concreteObjectGraph.getGraphLayoutCache()
				.cellViewsChanged(concreteObjectGraph.getGraphLayoutCache().getAllViews());
		ObjectDialogViewAdaptator.checkSaveStatus();
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (selected instanceof GroupDefaultGraphCell && SwingUtilities.isLeftMouseButton(e)) {
			GraphicEditorAdaptator.getMainFrame().getObjectDialogView().getToolBar().setEditGroupEnabled(true);
			((GroupDefaultVertexView) ((GroupDefaultGraphCell) selected).getInvView()).getHandle().mousePressed(e);

			CellView[] views = concreteObjectCreation.getGraphLayoutCache().getAllViews();
			ObjetAbstrait oA = ((GroupDefaultGraphCell) selected).getObject().getContientObj();
			...
			CellView[] views = concreteObjectCreation.getGraphLayoutCache().getAllViews();
			ObjetAbstrait oA = ((ConcreteObjectCell) selected).getObject().getUtiliseParClass();
			for (CellView v : views) {
				if (((AbstractObjectCell) v.getCell()).getObject().equals(oA)) {
					concreteObjectCreation.setSelectionCell(v.getCell());
					return;
