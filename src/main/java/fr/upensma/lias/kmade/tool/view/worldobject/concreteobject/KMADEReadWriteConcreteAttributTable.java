 ...
 */
public class KMADEReadWriteConcreteAttributTable extends JScrollPane implements
	LanguageFactory {
	...
	this.setBorder(new TitledBorder(null,
		KMADEConstant.CONCRETE_ATTRIBUT_TITLE_TABLE + " " + name,
	...
	public Object getValueAt(int row, int col) {
		...
		return ((String) data[row][0])
			+ " ("
			+ TypeStructure
				.getEnumereIntoLocaleTypeStructure(((String) data[row][1]))
			+ ")";
	    }
	...
	public boolean isCellEditable(int row, int col) {
		...
		ArrayList<String> maList = ConcreteAttributAdaptator
			.getTypeRefValues(attributName);

		if (maList.size() != 0) {
		    table.getColumnModel().getColumn(1)
			    .setCellEditor(new MyCustomTableCellEditor(maList));
		} else {
		    if ((data[row][2] == null)) {
			table.getColumnModel()
				.getColumn(1)
				.setCellEditor(
					new MyCustomStringTableCellEditor(
						new JTextField("")));
		    } else
			table.getColumnModel()
				.getColumn(1)
	...
	public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
		...
		if (aValue instanceof String) {
		    boolean isError = ConcreteAttributAdaptator
			    .setConcreteAttributValue(
				    (String) this.getValueAt(rowIndex, 2),
				    (String) aValue);
		    if (isError)
			return;
		}
