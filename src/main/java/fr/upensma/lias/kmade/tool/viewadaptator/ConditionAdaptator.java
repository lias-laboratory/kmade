...
public final class ConditionAdaptator {
    ...
    public static void updateConditionView() {
        ...
        for (ProtoTaskCondition current : myList) {
            KMADEMainFrame.getProjectPanel().getConditionPanel().addCondition(current.getDescription(), current.getDefaultValueText(), current.getOid().get());
        }
    }
	...
	public static void setPreCondition(String oidConditions) {
		ExpressCondition.setPreCondition(GraphicEditorAdaptator.getSelectedGraphicTask().getTask().getOid().get(),oidConditions);
		
	}
	
	public static void setIterCondition(String oidConditions){
		ExpressCondition.setIterCondition(GraphicEditorAdaptator.getSelectedGraphicTask().getTask().getOid().get(),oidConditions);
