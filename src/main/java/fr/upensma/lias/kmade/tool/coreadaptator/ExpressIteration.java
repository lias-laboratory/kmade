...
public final class ExpressIteration {
	...
	public static boolean isFinished(Task myTache) {
		 ...
		 * (myTache.getIteExpression().getIterationVariant() <= 0); } else { return
		 * (Boolean) myTache.getIteExpression().getNodeExpression().getNodeValue(); }
		 */
	}
		...
		SwingWorker worker = new SwingWorker() {
			public Object construct() {
				...
				for (int i = 0; i < tacheToBeCreated.length && !ExpressIteration.isCanceled(); i++) {
					String iteration = tacheToBeCreated[i].getIterExpression().getFormalText();
