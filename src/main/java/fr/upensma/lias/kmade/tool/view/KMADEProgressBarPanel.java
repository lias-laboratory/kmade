			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (ExpressKMADXML.isDone()) {
					...
					KMADEHistoryMessageManager
							.printlnMessage(KMADEConstant.WRITE_EXPRESS_OK_FILE + ExpressKMADXML.getCurrentFileName());
				}
				...
				if (ExpressKMADXML.isError()) {
					...
					KMADEHistoryMessageManager.printlnMessage(
							KMADEConstant.WRITE_EXPRESS_NO_OK_FILE + ExpressKMADXML.getCurrentFileName());
				}
	...
	public void readKMADFromXMLFile() {
		...
		myTimer = new Timer(10, new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (ExpressKMADXML.isDone() && GraphicEditorAdaptator.isBegining() && ExpressPrecondition.isBegining()
						&& ExpressEffetsDeBord.isBegining() && ExpressIteration.isBegining()) {
				...
				if (ExpressKMADXML.isDone() && GraphicEditorAdaptator.isDone() && ExpressPrecondition.isBegining()
						&& ExpressEffetsDeBord.isBegining() && ExpressIteration.isBegining()) {
				...
				if (ExpressKMADXML.isDone() && GraphicEditorAdaptator.isDone() && ExpressPrecondition.isDone()
						&& ExpressEffetsDeBord.isBegining() && ExpressIteration.isBegining()) {
				...
				if (ExpressKMADXML.isDone() && GraphicEditorAdaptator.isDone() && ExpressPrecondition.isDone()
						&& ExpressEffetsDeBord.isDone() && ExpressIteration.isBegining()) {
				...
				if (ExpressIteration.isDone()) {
					...
					ConditionAdaptator.updateConditionView();
					...
					GraphicEditorAdaptator.getMainFrame().activeMessageStream();
					KMADEHistoryMessageManager
							.printlnMessage(KMADEConstant.OPEN_EXPRESS_OK_FILE + ExpressKMADXML.getCurrentFileName());
				}
				...
				if (ExpressKMADXML.isError()) {
					...
					GraphicEditorAdaptator.getMainFrame().activeMessageStream();
					KMADEHistoryMessageManager.printlnMessage(
							KMADEConstant.OPEN_EXPRESS_NO_OK_FILE + ExpressKMADXML.getCurrentFileName());
				}
				...
				if (ExpressKMADXML.isDone() && GraphicEditorAdaptator.isCanceled()
						&& ExpressPrecondition.isBegining()) {
					KMADEHistoryMessageManager
							.printlnMessage(KMADEConstant.EXPLICIT_STOP_LOAD_SPF_FILE_DURING_GRAPHICAL_OBJECT);
					cancelButton.setEnabled(false);
					retourButton.setEnabled(true);
					InterfaceExpressJava.clearCurrentProject();
					KMADeAdaptator.cleanAllAdaptateur();
				...
				if (ExpressKMADXML.isDone() && GraphicEditorAdaptator.isDone() && ExpressPrecondition.isCanceled()
				...
				if (ExpressKMADXML.isDone() && GraphicEditorAdaptator.isDone() && ExpressPrecondition.isDone()
						&& ExpressEffetsDeBord.isCanceled() && ExpressIteration.isBegining()) {
					KMADEHistoryMessageManager
							.printlnMessage(KMADEConstant.EXPLICIT_STOP_LOAD_SPF_FILE_DURING_GRAPHICAL_OBJECT);
					cancelButton.setEnabled(false);
					retourButton.setEnabled(true);
				...
				if (ExpressKMADXML.isDone() && GraphicEditorAdaptator.isDone() && ExpressPrecondition.isDone()
						&& ExpressEffetsDeBord.isDone() && ExpressIteration.isCanceled()) {
					KMADEHistoryMessageManager
							.printlnMessage(KMADEConstant.EXPLICIT_STOP_LOAD_SPF_FILE_DURING_GRAPHICAL_OBJECT);
					cancelButton.setEnabled(false);
					retourButton.setEnabled(true);
		...
		if (ioError) {
			...
			myTimer = new Timer(10, new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					...
					if (InterfaceExpressJava.isDone() && GraphicEditorAdaptator.isBegining()
							&& ExpressPrecondition.isBegining() && ExpressEffetsDeBord.isBegining()
					...
					if (InterfaceExpressJava.isDone() && GraphicEditorAdaptator.isDone()
							&& ExpressPrecondition.isBegining() && ExpressEffetsDeBord.isBegining()
							&& ExpressIteration.isBegining()) {
						myProgressBar.setString(KMADEConstant.PRECONDITION_PROGRESSBAR_MESSAGE);
						ExpressPrecondition.setBegining(false);
					...
					if (InterfaceExpressJava.isDone() && GraphicEditorAdaptator.isDone() && ExpressPrecondition.isDone()
							&& ExpressEffetsDeBord.isBegining() && ExpressIteration.isBegining()) {
						myProgressBar.setString(KMADEConstant.EFFETSDEBORD_PROGRESSBAR_MESSAGE);
						ExpressEffetsDeBord.setBegining(false);
						ExpressEffetsDeBord.makeAndCheckEffetsDeBordOpenSPFFile();
					...
					if (InterfaceExpressJava.isCanceled() && GraphicEditorAdaptator.isBegining()) {
						KMADEHistoryMessageManager
								.printlnMessage(KMADEConstant.EXPLICIT_STOP_LOAD_SPF_FILE_DURING_OBJECT);
						cancelButton.setEnabled(false);
						retourButton.setEnabled(true);
						InterfaceExpressJava.clearCurrentProject();
						KMADeAdaptator.cleanAllAdaptateur();
					...
					if (InterfaceExpressJava.isDone() && GraphicEditorAdaptator.isCanceled()
							&& ExpressPrecondition.isBegining()) {
						KMADEHistoryMessageManager
								.printlnMessage(KMADEConstant.EXPLICIT_STOP_LOAD_SPF_FILE_DURING_GRAPHICAL_OBJECT);
						cancelButton.setEnabled(false);
					...
					if (InterfaceExpressJava.isDone() && GraphicEditorAdaptator.isDone()
							&& ExpressPrecondition.isCanceled() && ExpressEffetsDeBord.isBegining()) {
						KMADEHistoryMessageManager
								.printlnMessage(KMADEConstant.EXPLICIT_STOP_LOAD_SPF_FILE_DURING_GRAPHICAL_OBJECT);
						cancelButton.setEnabled(false);
						retourButton.setEnabled(true);
