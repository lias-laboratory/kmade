...
public class KMADEVertexSimulationView extends KMADEVertexView {
	...
	private static Color PASSIVE_COLOR = new Color(50, 50, 255, (int) (100 * TRANSPARENCY_COEF));

	private static Color ACTIVE_COLOR = new Color(210, 210, 255, (int) (100 * TRANSPARENCY_COEF));

	private static Color FINISHED_COLOR = new Color(153, 153, 153, (int) (100 * TRANSPARENCY_COEF));

	private static Color PASSED_COLOR = new Color(51, 204, 0, (int) (100 * TRANSPARENCY_COEF));

	private static Color SUSPENDED_COLOR = new Color(255, 0, 0, (int) (100 * TRANSPARENCY_COEF));

	private static Color NO_RESUMED_COLOR = new Color(255, 255, 0, (int) (100 * TRANSPARENCY_COEF));
	...
	public String getStateMessage() {
		KMADEDefaultGraphCell myGraphCell = (KMADEDefaultGraphCell) KMADEVertexSimulationView.this.getCell();
		String message = "";
	...
	public Rectangle2D getBounds() {
		...
		if (temp.getWidth() < newWidth) {
			return new Rectangle2D.Double(temp.getX(), temp.getY(), newWidth, temp.getHeight());
		} else {
			return new Rectangle2D.Double(temp.getX(), temp.getY(), temp.getWidth(), temp.getHeight());
		}
	}
	...
	class MyCellHandle2 implements CellHandle {
		...
		public void mousePressed(MouseEvent event) {
			...
			int tolerance = 2;
			int rx = (int) (rec.getX()) + ((int) rec.getWidth() / 2) - IMAGE_32_WIDTH / 2 - IMAGE_16_WIDTH - V_GAP
					- tolerance;
			...
			if (mx >= rx && mx <= rw && my >= ry && my <= rh) {
				...
				if (!myGraphCell.isMotherPortNoEdged()) {
					myGraphCell.setSonSimulationExpanded(!myGraphCell.isSonSimulationExpanded());
					myGraph.repaint();
				}
			}
		}
	...
	protected String getIterationName(KMADEDefaultGraphCell myGraphCell) {
		...
		if (myGraphCell.getTask().getIterExpression().isErrorInExpression()) {
			...
			if (myGraphCell.getTask().getIterExpression().isVariableExpressionNode()) {
				iteration += String.valueOf(myGraphCell.getTask().getIterExpression().getIterationVariant()) + "/"
						+ myGraphCell.getTask().getIterExpression().getNodeExpression().getNodeValue();
			} else {
				if (myGraphCell.getTask().getStateSimulation().isActive()
						|| myGraphCell.getTask().getStateSimulation().isPassive()) {
					try {
						myGraphCell.getTask().getIterExpression().getNodeExpression().evaluateNode(null);
						iteration += myGraphCell.getTask().getIterExpression().getNodeExpression().getNodeValue();
	...
	class MyVertexSimulationRenderer extends MyVertexRenderer {
		...
		/**
		 * permet l'affichage du texte et de la couleur concernant l'état de la tâche
		 * ainsi que les informations d'it�ration
		 * 
		 * @param gr          le graphix
		 ...
		 * @param iter        - la position de la fin de la chaine pour l'it�ration
		 */
		protected void paintStateCaract(Graphics2D gr, int widthString, String numString, String iterString, double num,
				double iter) {
			...
			if (stateTaskColor != null) {
				...
				gr.fillRect(((int) this.getBounds().getWidth() / 2) - 40, 1, 80,
		...
		public void paint(Graphics gr) {
			...
			if (selected) {
				...
				f.drawRect(((int) this.getBounds().getWidth() / 2) - 40, 1, 80 - 1,
						(int) this.getBounds().getHeight() - 1);
			}

			if (!myGraphCell.isMotherPortNoEdged()
					&& (myGraph == GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getGraphSimulation())) {
				...
				if (!preview) {
					myImageIcon.paintIcon(this, gr,
							((int) this.getBounds().getWidth() / 2) - IMAGE_32_WIDTH / 2 - IMAGE_16_WIDTH - V_GAP,
							BEGIN_V_GAP + IMAGE_16_HEIGH / 2);
				}
			}
		}
	}
