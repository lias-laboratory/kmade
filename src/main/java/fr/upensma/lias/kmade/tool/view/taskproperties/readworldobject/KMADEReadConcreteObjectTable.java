...
public class KMADEReadConcreteObjectTable extends JScrollPane {
	...
	public KMADEReadConcreteObjectTable() {
		treeTable = new JTreeTable(
				new ReadConcreteTreeTableModel(new NodeObject(new CurrentAbstractObjectDataBase("Root", null))));
	...
	static class CurrentAbstractObjectDataBase {
		...
		public CurrentAbstractObjectDataBase(String name, ArrayList<ObjetAbstrait> plistObjetAbstrait) {
	...
	static class ReadConcreteTreeTableModel extends AbstractTreeTableModel implements TreeTableModel {
		...
		public Object getValueAt(Object node, int column) {
			...
			if (current.getEntity() instanceof CurrentAbstractObjectDataBase) {
				...
				name = temp.getName();
				type = "";
				valeur = temp.getAbstractObjects().size() != 0
						? temp.getAbstractObjects().size() + " objet(s) abstrait(s)"
						: "Aucun objet abstrait";
			} else if (current.getEntity() instanceof ObjetAbstrait) {
				ObjetAbstrait temp = (ObjetAbstrait) current.getEntity();
				name = temp.getName();
				type = "Objet Abstrait";
				valeur = temp.getInverseGroupe().size() != 0 ? temp.getInverseGroupe().size() + " groupe(s)"
						: "Aucun groupe";
			} else if (current.getEntity() instanceof Groupe) {
				Groupe temp = (Groupe) current.getEntity();
				name = temp.getName();
				type = "Groupe";
				valeur = temp.getEnsemble().getLstObjConcrets().size() != 0
						? temp.getEnsemble().getLstObjConcrets().size() + " objet(s) concret(s)"
						: "Aucun objet concret";
			} else if (current.getEntity() instanceof ObjetConcret) {
				ObjetConcret temp = (ObjetConcret) current.getEntity();
				name = temp.getName();
				type = "Objet Concret";
				valeur = temp.getInverseListAttribut().size() != 0
						? temp.getInverseListAttribut().size() + " attribut(s) concret(s)"
						: "Aucun attribut concret";
