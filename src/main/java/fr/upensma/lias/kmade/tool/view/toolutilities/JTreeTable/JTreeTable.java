...
public class JTreeTable extends KMADEJTable {
	 ...
	 * and editors; overriding setBounds() below is not the right thing to do for an
	 * editor. Returning -1 for the editing row in this case, ensures the editor is
	 * never painted.
	 */
	 ...
	 * receiver is editing a Tree column, the editor's bounds is reset. The reason
	 * we have to do this is because JTable doesn't think the table is being edited,
	 * as <code>getEditingRow</code> returns -1, and therefore doesn't automatically
	...
	public class TreeTableCellRenderer extends JTree implements TableCellRenderer {
		...
		public void paint(Graphics g) {
			...
			if (highlightBorder != null) {
				highlightBorder.paintBorder(this, g, 0, translationOffset, getWidth(),
						// uhhh
	...
	public class TreeTableCellEditor extends DefaultCellEditor {
		...
		/**
		 * Overridden to determine an offset that tree would place the editor at. The
		 * offset is determined from the <code>getRowBounds</code> JTree method, and
		 * additionally from the icon DefaultTreeCellRenderer will use.
		 * <p>
		...
		/**
		 * This is overridden to forward the event to the tree. This will return true if
			...
			if (e instanceof MouseEvent) {
				...
				if (me.getModifiers() == 0 || me.getModifiers() == InputEvent.BUTTON1_MASK) {
					for (int counter = getColumnCount() - 1; counter >= 0; counter--) {
						if (getColumnClass(counter) == TreeTableModel.class) {
							MouseEvent newME = new MouseEvent(JTreeTable.this.tree, me.getID(), me.getWhen(),
				...
				if (e == null || me.getClickCount() >= 2) {
					...
					return JTreeTable.this.getTreeTableModel()
							.isCellTreeEditable(JTreeTable.this.tree.getSelectionPath().getLastPathComponent());
				}
				return false;
			}
			return false;
