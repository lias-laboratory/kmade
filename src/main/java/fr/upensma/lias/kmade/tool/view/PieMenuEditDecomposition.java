...
public class PieMenuEditDecomposition extends Menu {
	...
	public PieMenuEditDecomposition(Canvas c) {
		...
		this.canvas = c;
		String[] items = { ExpressConstant.SEQUENTIEL_LONG_NAME, ExpressConstant.NO_ORDER_LONG_NAME,
				ExpressConstant.PARALLELE_LONG_NAME, ExpressConstant.ALTERNATIF_LONG_NAME,
				ExpressConstant.LEAF_LONG_NAME, ExpressConstant.UNKNOWN_LONG_NAME };
		menuLayout(items);
		hideMenu();
		...
		smText = new JStateMachine() {
			...
			public State initialState = new State() {
				/*
				 * on "press" le bouton de gauche sur la decomposition de la tache pour afficher
				 * le menu
				 */
						...
						if (onJGraph) {
								...
								if (selection.length == 1) {
									...
									for (int i = 0; i < allViews.length; i++) {
										if (!selection[0].getClass().equals(KMADEDefaultEdge.class)
											...
											/*
											 * on v�rifie que l'on est sur la d�composition de la t�che
											 */
											Rectangle2D recDecomposition = ((KMADEVertexView) allViews[i]).getRectOpe();
											if (recDecomposition.contains(getPoint())) {
												return true;
											}
										}
									}
							...
							for (int i = 0; i < allViews.length; i++) {
								if (allViews[i] instanceof KMADEVertexView) {
									Rectangle2D recDecomposition = ((KMADEVertexView) allViews[i]).getRectOpe();
									if (recDecomposition != null && recDecomposition.contains(getPoint())) {
										KMADEGraph g = KMADEMainFrame.getProjectPanel().getTaskModelPanel().getJGraph();
										g.setSelectionCell(allViews[i].getCell());
										return true;
									}
								}
							}
		...
		interaction = new CStateMachine() {
			...
			public State waitClik = new State() {
				...
				Transition raz = new Click(BUTTON1, ">> menuOn") {
					...
					public void action() {
						disarmTimer();
						Point gapGrab = ((JViewport) KMADEMainFrame.getProjectPanel().getTaskModelPanel().getJGraph()
								.getParent().getParent()).getViewPosition();
						pointDisplay = getPoint();
						pointDisplay.setLocation(pointDisplay.getX()/*
																	 * +gap.getX( )- gapGrab .getX()
																	 */,
								pointDisplay.getY()/*
													 * +gap.getY()-gapGrab .getY()
													 */);
