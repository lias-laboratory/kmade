	...
	interaction = new CStateMachine() {
		...
		Transition select = new Release(BUTTON1, ">> menuOff") {
			...
			if (lastItemVisited != null) {
			    ...
			    if (lastItemVisited
				    .equals(ExpressConstant.SEQUENTIEL_LONG_NAME)) {
				ExpressTask.setOperatorTask(cell.getTask(),
					Decomposition.SEQ);
			    } else if (lastItemVisited
				    .equals(ExpressConstant.NO_ORDER_LONG_NAME)) {
				ExpressTask.setOperatorTask(cell.getTask(),
					Decomposition.ET);
			    } else if (lastItemVisited
				    .equals(ExpressConstant.PARALLELE_LONG_NAME)) {
				ExpressTask.setOperatorTask(cell.getTask(),
					Decomposition.PAR);
			    } else if (lastItemVisited
				    .equals(ExpressConstant.ALTERNATIF_LONG_NAME)) {
				ExpressTask.setOperatorTask(cell.getTask(),
					Decomposition.ALT);
			    } else if (lastItemVisited
				    .equals(ExpressConstant.LEAF_LONG_NAME)) {
				ExpressTask.setOperatorTask(cell.getTask(),
					Decomposition.ELE);
			    } else if (lastItemVisited
				    .equals(ExpressConstant.UNKNOWN_LONG_NAME)) {
				ExpressTask.setOperatorTask(cell.getTask(),
					Decomposition.INCONNU);
			    }
