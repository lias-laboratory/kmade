...
public class KMADESimulationDialog extends JFrame {
    ...
    public void setValue(String[] executant, String[] precondition,
	    String[] evenement) {
	...
	mySimulationGraph
		.setSelectionModel(new NMDASimulationGraphSelectionModel(
			mySimulationGraph));
	...
	mySimulationGraph.getGraphLayoutCache().setFactory(
	...
	tabpaneProjet = new JTabbedPane(JTabbedPane.RIGHT) {
		...
		if (tabpaneProjet.getSelectedIndex() != -1) {
		    boolean state = SimulationAdaptator
			    .recordOrReplayMode(tabpaneProjet
				    .getSelectedIndex());
		    if (state) {
	...
	tabpaneProjet.addTab(null, new VerticalTextIcon(
		KMADEConstant.RECORD_TITLE_MESSAGE, false, myRecordingPanel),
		myRecordingPanel);
	tabpaneProjet.addTab(null, new VerticalTextIcon(
		KMADEConstant.REPLAY_TITLE_MESSAGE, false, myReplayPanel),
		myReplayPanel);

	JSplitPane myCentralSplitPane = KMADEEnhancedSplitPane
	...
	JScrollPane myScrollPaneConstraints = new JScrollPane(new JTable(
		refModel));
	JPanel panelRecordEmpty = new JPanel(new BorderLayout());
	panelRecordEmpty.add(new JLabel(
		KMADEConstant.NO_SELECTED_ACTION_FOR_CONSTRAINTS_MESSAGE,
		JLabel.CENTER));
	JPanel panelReplayEmpty = new JPanel(new BorderLayout());
	panelReplayEmpty
		.add(new JLabel(
			KMADEConstant.NO_SELECTED_SCENARIO_ACTION_FOR_CONSTRAINTS_MESSAGE,
			JLabel.CENTER));
	...
	protertiesTable = new JPropertiesTable(KMADEMainFrame.getProjectPanel()
	...
	this.myPropertiesBorder = new TitledBorder(null,
	...
	this.myObjetConcretBorder = new TitledBorder(null,
		KMADEConstant.EFFETSDEBORD_CONCRETE_OBJECT_TITLE_MESSAGE,
	...
	panelCurrentObjectEmpty.add(new JLabel(
		KMADEConstant.NO_CONCRETE_OBJECT, JLabel.CENTER));
	...
	this.myEventBorder = new TitledBorder(null,
		KMADEConstant.EVENT_TITLE_MESSAGE, TitledBorder.CENTER,
		TitledBorder.TOP);
	...
	panelEventEmpty.add(new JLabel(KMADEConstant.NO_FIRING_EVENT_MESSAGE,
		JLabel.CENTER));
	...
	this.myEventList.setCellRenderer(new ListCellRenderer() {
		...
		return new JLabel("E : " + toto.getFirableEvent() + " , T : "
			+ toto.getFiringTask().getName());
	    }
	});
	...
	JSplitPane executantEventConcreteObject = KMADEEnhancedSplitPane
		.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT,
			panelProperties, panelConcreteObject);
	...
	JSplitPane executantEventConcreteObjectContraints = KMADEEnhancedSplitPane
		.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT,
			executantEventConcreteObject, myEventPanel);
	...
	panelEmpty
		.add(new JLabel(KMADEConstant.NO_USER_MESSAGE, JLabel.CENTER));

	myPanelUser.add("COMBO", panelCombo);
	myPanelUser.add("EMPTY", panelEmpty);

	myPanelUser.setPreferredSize(new Dimension(150, 100));
	this.myUserBorder = new TitledBorder(null,
		KMADEConstant.EXECUTING_USER_TITLE_MESSAGE,
		TitledBorder.CENTER, TitledBorder.TOP);
	myPanelUser.setBorder(myUserBorder);

	JSplitPane executantEventConcreteObjectContraintsProperties = KMADEEnhancedSplitPane
		.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT,
			executantEventConcreteObjectContraints, myPanelUser);
	...
	JSplitPane upDownSplitPane = KMADEEnhancedSplitPane
		.createStrippedSplitPane(JSplitPane.VERTICAL_SPLIT,
			executantEventConcreteObjectContraintsProperties,
			myMainSplitPane);
	...
	ERROR_USER_IMAGEICON = new ImageIcon(
		KMADEToolUtilities.getImageThumbnail(
			GraphicEditorAdaptator.class
				.getResource(KMADEConstant.USER_ERROR_IMAGE),
			KMADEConstant.ROW_HEIGHT));
	UNKNOWN_USER_IMAGEICON = new ImageIcon(
		KMADEToolUtilities.getImageThumbnail(
			GraphicEditorAdaptator.class
    ...
    class NMDASimulationGraphSelectionModel extends DefaultGraphSelectionModel
	...
	public void valueChanged(GraphSelectionEvent arg0) {
		...
		if (this.getSelectionCount() == 0) {
		    ...
		    int firstIndex = 0;
		    boolean find = false;
		    for (int i = 0; i < myRecordingPanel.getActionListModel()
			    .getSize(); i++) {
			if (!find
			...
			if (!myRecordingPanel.getActionList()
				.isSelectionEmpty()) {
				...
				myRecordingPanel
					.getActionList()
					.getSelectionModel()
					.setSelectionInterval(firstIndex,
						firstIndex);
				return;
			    }
			} else {
