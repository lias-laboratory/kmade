...
public class KMADEReadWriteConcreteObjectTreeTable extends JScrollPane implements LanguageFactory {
	...
	public void setConcreteObjectNameBorder(String name) {
		...
		if (name.equals(""))
			...
			afficher = " : " + name;
		this.setBorder(new TitledBorder(null, KMADEConstant.CONCRETE_OBJECT_TITLE_TABLE + afficher, TitledBorder.CENTER,
				TitledBorder.DEFAULT_POSITION, KMADEConstant.fontPASSIF));
	}
		...
		public void keyPressed(KeyEvent ke) {
			if (ke.getKeyCode() == KeyEvent.VK_DELETE) {
				...
				if (objetConcret.size() != 0) {
					...
					boolean state = GraphicEditorAdaptator.removeSelectedItem("ConcreteObject", tabSelect, list,
							GraphicEditorAdaptator.getMainFrame(), KMADEConstant.CONCRETE_OBJECT_REMOVE_MESSAGE_TITLE);
		...
		public String toString() {
			if (myEntity == null) {
				...
				String localeAgregat = AgregatStructure
						.getEnumereIntoLocaleAgregatStructure(myGroupe.getEnsemble().getAgregatStructure().getValue());
	...
	class ConcreteObjectTreeTableModel extends DefaultTreeTableModel {
		 ...
		 * 
		 * @param value
		 * @return une chaine vide ou null si le nom est incorrect le nom si le nom est
		 *         correct
		 */
		...
		public void setValueAt(Object aValue, Object node, int column) {
			...
			if (column == 1) {
				if (refNode.getEntity() instanceof ObjetConcret) {
					ConcreteObjectAdaptator.setConcreteObjectDescription((ObjetConcret) (refNode.getEntity()),
							(String) aValue);
				}
			} else if (column == 0) {
				...
				if (refNode.getEntity() instanceof ConcreteObjectNewLineUserObject) {
					...
					ObjetConcret newConcreteObject = ConcreteObjectAdaptator.addNewConcreteObject(
							(Groupe) ((ConcreteObjectNode) refNode.getParent()).getEntity(), name);
