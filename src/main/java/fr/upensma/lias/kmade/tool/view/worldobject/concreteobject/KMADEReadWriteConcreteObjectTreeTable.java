...
import fr.upensma.lias.kmade.tool.view.toolutilities.JListTreeTable;
import fr.upensma.lias.kmade.tool.view.toolutilities.LanguageFactory;
import fr.upensma.lias.kmade.tool.view.toolutilities.JTreeTable.DefaultTreeTableModel;
import fr.upensma.lias.kmade.tool.view.toolutilities.JTreeTable.TreeTableModel;
import fr.upensma.lias.kmade.tool.viewadaptator.ConcreteObjectAdaptator;
import fr.upensma.lias.kmade.tool.viewadaptator.GraphicEditorAdaptator;

/**
 * @author Mickael BARON
 */
public class KMADEReadWriteConcreteObjectTreeTable extends JScrollPane
	...
	listTreeTable = new JListTreeTable(new ConcreteObjectTreeTableModel(
		new ConcreteObjectNode(null)));
	listTreeTable.addAllKeyListener(new MyKeyListener());
	listTreeTable.getTree().getSelectionModel()
		.addTreeSelectionListener(new TreeSelectionListener() {
		    public void valueChanged(TreeSelectionEvent e) {
			if (listTreeTable.getTreeTable().getSelectionModel()
				.isSelectionEmpty()) {
			    ConcreteObjectAdaptator
				    .setNoSelectedConcreteObject();
			} else {
				...
				ConcreteObjectAdaptator
					.setSelectedConcreteObject(temp.get(0));
			    }
			}
		    }
		});
	...
	for (int i = 0; i < toto.length; i++) {
		...
		if (((ConcreteObjectNode) toto[i]).getEntity() instanceof ObjetConcret) {
		    temp.add((ObjetConcret) (((ConcreteObjectNode) toto[i])
			    .getEntity()));
		}
	    }
	}
	...
	this.setBorder(new TitledBorder(null,
		KMADEConstant.CONCRETE_OBJECT_TITLE_TABLE + afficher,
		TitledBorder.CENTER, TitledBorder.DEFAULT_POSITION,
		KMADEConstant.fontPASSIF));
    }

    public void updateDataModel(ObjetAbstrait refObjetAbstrait) {
	refModel = new ConcreteObjectTreeTableModel(new ConcreteObjectNode(
		refObjetAbstrait));
	...
	public void keyPressed(KeyEvent ke) {
		...
		if (objetConcret.size() != 0) {
		    ...
		    if (state) {
			refModel.nodeStructureChanged((TreeNode) refModel
				.getRoot());
			listTreeTable.getTreeTable().getTableTreeModel()
	...
	public ConcreteObjectNode searchEntityObject(Object entity) {
		...
		for (int i = 0; i < children.size(); i++) {
		    toto = ((ConcreteObjectNode) children.get(i))
			    .searchEntityObject(entity);
		    if (toto != null) {
			return toto;
		    }
		}
	...
	public String toString() {
		...
		String localeAgregat = AgregatStructure
			.getEnumereIntoLocaleAgregatStructure(myGroupe
	...
	public boolean isLeaf() {
	    return (myEntity instanceof ObjetConcret)
		    || (myEntity instanceof ConcreteObjectNewLineUserObject);
	}
	...
	protected List<ConcreteObjectNode> getChildren() {
		...
		if (myEntity instanceof Groupe) {
		    ...
		    for (ObjetConcret current : ref.getEnsemble()
			    .getLstObjConcrets()) {
			children.add(new ConcreteObjectNode(current, this));
		    }
		    children.add(new ConcreteObjectNode(
			    new ConcreteObjectNewLineUserObject(), this));
		    return children;
		}
	...
	public void setValueAt(Object aValue, Object node, int column) {
		...
		if (refNode.getEntity() instanceof ObjetConcret) {
		    ConcreteObjectAdaptator.setConcreteObjectDescription(
			    (ObjetConcret) (refNode.getEntity()),
			    (String) aValue);
		}
	    } else if (column == 0) {
		...
		if (refNode.getEntity() instanceof ConcreteObjectNewLineUserObject) {
		    ...
		    ObjetConcret newConcreteObject = ConcreteObjectAdaptator
			    .addNewConcreteObject(
				    (Groupe) ((ConcreteObjectNode) refNode
					    .getParent()).getEntity(), name);
		    ...
		    ConcreteObjectNode newNode = ((ConcreteObjectNode) this.root)
			    .searchEntityObject(newConcreteObject);
		    int t = listTreeTable.getTree().getRowForPath(
			    new TreePath(this.getPathToRoot(newNode)));
		    listTreeTable.getTree().setSelectionPath(
			    new TreePath(this.getPathToRoot(newNode)));
		    listTreeTable.getTreeTable().getSelectionModel()
			    .setSelectionInterval(t, t);
		} else {
		    // Modifier
		    ...
		    ConcreteObjectAdaptator.setConcreteObjectName(
			    (ObjetConcret) (refNode.getEntity()), value);
		    this.nodeStructureChanged(refNode.getParent());
		    listTreeTable.getTreeTable().getTableTreeModel()
	...
	public Object getValueAt(Object node, int column) {
		...
		int groupCount = ((ObjetAbstrait) refNode.getEntity())
			.getInverseGroupe().size();
		designation = groupCount != 0 ? Integer.toString(groupCount)
			: KMADEConstant.NO_CONCRETE_OBJECT_GROUPE_NAME;
		designation += " " + KMADEConstant.CONCRETE_OBJECT_GROUPE_NAME
			+ (groupCount > 1 ? "s" : "");
