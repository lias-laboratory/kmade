...
public class KMADETaskModelPanel extends JPanel {
	...
	public class NMDAMarqueeHandler extends BasicMarqueeHandler {
		...
		private Image img = tk.getImage(KMADEVertexView.class.getResource(KMADEConstant.CURSEUR_IMAGE_GRAB));

		public Cursor CURSEUR_GRAB = tk.createCustomCursor(img, new Point(16, 16), "curseur_grab");

		/*
		 * Cette m�thode permet d'activer ou de d�sactiver le d�placement. Typiquement
		 * la s�lection d'un port retourne vraie.
		 */
		...
		public void overlay(JGraph graph, Graphics g, boolean clear) {
			...
			if (marqueeBounds != null) {
				...
				if (!curseurMove) {
					...
					g.drawRect((int) marqueeBounds.getX(), (int) marqueeBounds.getY(), (int) marqueeBounds.getWidth(),
				...
				if (GraphicEditorAdaptator.addNewTask(new Point(e.getX(), e.getY()))) {
					...
					if (thread == null) {
						...
						if (!interupt) {
							...
							super.setMarqueeBounds(new Rectangle2D.Double(startPoint.getX(), startPoint.getY(), 0, 0));
		...
		public void mouseDragged(MouseEvent e) {
			...
			if (start != null) {
				...
				if (graph.getCursor().getType() == Cursor.MOVE_CURSOR) {
					...
					curseurMove = true;
					if (graph.getSelectionCell() == null || graph.getSelectionCellAt(e.getPoint()) == null) {
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (e != null && port != null && firstPort != null && firstPort != port) {
				...
				boolean successed = GraphicEditorAdaptator.addNewEdge((KMADEDefaultPort) (firstPort.getCell()),
			...
			if (graph.isCellEditable(cell)) {
				...
				if (cellEditor.isCellEditable(event)) {
					...
					Point2D p = getEditorLocation(cell, editorSize,
							graph.toScreen(new Point2D.Double(cellBounds.getCenterX(), cellBounds.getCenterY())));
					int deltax = 0;
					int deltay = 0;
					...
					editingComponent.setBounds((int) p.getX(), (int) p.getY(), (int) editorSize.getWidth(),
					...
					graph.paintImmediately((int) p.getX(), (int) p.getY(),
							(int) (visRect.getWidth() + visRect.getX() - cellBounds.getX()),
							(int) editorSize.getHeight());
					...
					if (event instanceof MouseEvent) {
						/*
						 * Find the component that will get forwarded all the mouse events until
						 * mouseReleased.
						 */
						...
						/*
						 * Create an instance of BasicTreeMouseListener to handle passing the
						 * mouse/motion events to the necessary component.
						 */
		...
		class MyMouseHandler extends MouseHandler {
			...
			public CellView getNextSelectableViewAt(CellView current, double x, double y) {
				CellView[] selectables = getJGraph().getGraphLayoutCache()
						.getMapping(getJGraph().getSelectionModel().getSelectables(), false);
				return getNextViewAt(selectables, current, x, y);
			}

			/**
			 * Returns the next view at the specified location wrt. <code>c</code> in the
			 * specified array of views. The views must be in order, as returned, for
			 * example, by GraphLayoutCache.order(Object[]).
			 */
			...
			public CellView getNextViewAt(CellView[] cells, CellView c, double x, double y, boolean leafsOnly) {
				if (cells != null) {
					Rectangle2D r = getJGraph().fromScreen(
							new Rectangle2D.Double(x - getJGraph().getTolerance(), y - getJGraph().getTolerance(),
									2 * getJGraph().getTolerance(), 2 * getJGraph().getTolerance()));
					...
					for (int i = 0; i < cells.length; i++) {
						if (cells[i] != null && (!leafsOnly || cells[i].isLeaf()) && cells[i].intersects(getJGraph(), r)
								&& isExpanded(cells[i])) {
			...
			public void mousePressed(MouseEvent e) {
				...
				if (!e.isConsumed() && graph.isEnabled()) {
					...
					Rectangle2D r = graph.fromScreen(new Rectangle2D.Double(e.getX() - s, e.getY() - s, 2 * s, 2 * s));
					lastFocus = focus;
					...
					if (!isForceMarqueeEvent(e)) {
						if (e.getClickCount() == graph.getEditClickCount() && focus != null && focus.isLeaf()
								&& focus.getParentView() == null && graph.isCellEditable(focus.getCell())
					...
					// Marquee Selection
					if (!e.isConsumed() && marquee != null && (!isToggleSelectionEvent(e) || focus == null)) {
						marquee.mousePressed(e);
						handler = marquee;
					}
				}
		...
		class MyRootHandle extends RootHandle {
			...
			public void mouseClicked(MouseEvent e) {
				...
				for (int i = 0; i < views.length; i++) {
					if (views[i] instanceof KMADEVertexView) {
						if (views[i].getCell() == graph.getSelectionCellAt(e.getPoint())) {
							((KMADEVertexView.MyCellHandle) ((KMADEVertexView) views[i]).getHandle(context))
									.mouseClicked(e);
						}
					}
				}
			...
			public void mousePressed(MouseEvent event) {
				if (SwingUtilities.isRightMouseButton(event)) {
					JPopupMenu menu = GraphicEditorAdaptator.showPopupMenu(event.getPoint());
					if (menu != null) {
					...
					if (event != null && !event.isConsumed()) {
						...
						/* else */if (start != null) { // Move Cells
							...
							if (isMoving || Math.abs(dx) > thresh || Math.abs(dy) > thresh) {
								...
								Point2D constrainedPosition = constrainDrag(event, totDx, totDy, dx, dy);
								...
								if (dx != 0 || dy != 0) {
									...
									if (!snapLast.equals(snapStart) && (offgraphics != null || !blockPaint)) {
										if (graph.isXorEnabled()) {
											overlay(g);
										}
										overlayed = true;
									...
									isContextVisible = (!event.isControlDown() || !graph.isCloneable())
											&& contextViews != null && (contextViews.length < MAXCELLS);
									blockPaint = false;
									if (constrained && cachedBounds == null) {
										...
										for (int i = 0; i < all.length; i++) {
											...
											all[i].changeAttributes(graph.getGraphLayoutCache(),
									...
									if (cachedBounds != null) {
										...
										cachedBounds.setFrame(cachedBounds.getX() + dx * scale,
												cachedBounds.getY() + dy * scale, cachedBounds.getWidth(),
									...
									Rectangle2D ignoredRegion = (ignoreTargetGroup != null)
											? (Rectangle2D) ignoreTargetGroup.getBounds().clone()
											: null;
									if (targetGroup != null) {
										Rectangle2D tmp = graph.toScreen((Rectangle2D) targetGroup.getBounds().clone());
										if (dirty != null)
											dirty.add(tmp);
										else
											dirty = tmp;
									...
									targetGroup = null;
									if (graph.isMoveIntoGroups() && (ignoredRegion == null
											|| !ignoredRegion.intersects(AbstractCellView.getBounds(views)))) {
										targetGroup = (event.isControlDown()) ? null
												: findUnselectedInnermostGroup(snapCurrent.getX() / scale,
														snapCurrent.getY() / scale);
										if (targetGroup == ignoreTargetGroup)
											targetGroup = null;
									}
									if (!snapCurrent.equals(snapStart) && (offgraphics != null || !blockPaint)
											&& !spread) {
										if (graph.isXorEnabled()) {
											overlay(g);
										}
									...
									if (overlayed && (offgraphics != null || !graph.isXorEnabled())) {
										if (dirty == null) {
										...
										if (isDragging && !DNDPREVIEW) // BUG IN
											...
											return;
										if (offgraphics != null) {
											graph.drawImage((int) sx1, (int) sy1, (int) sx2, (int) sy2, (int) sx1,
						...
						if (event != null && !event.isConsumed()) {
							...
							if (isMoving && !event.getPoint().equals(start)) {
								...
								if (event.isControlDown() && graph.isCloneable()) { // Clone
									...
									cells = graphLayoutCache.insertClones(cells, graph.cloneCells(cells), attributes,
											cs, pm, 0, 0);
									...
									if (targetGroup != null) {
										...
										pm = new ParentMap(context.getCells(), targetGroup.getCell());
											...
											&& (ignoreTargetGroup != null && !ignoreTargetGroup.getBounds()
													.intersects(AbstractCellView.getBounds(views)))) {
