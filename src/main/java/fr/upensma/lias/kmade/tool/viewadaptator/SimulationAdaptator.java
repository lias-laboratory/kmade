...
public final class SimulationAdaptator {
	...
	// Le modèle du JGraph
	DefaultGraphModel myGraphModel = GraphicEditorAdaptator
		.getTaskModelPanel().getGraphModel();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.setModelToSimulation(myGraphModel);
	...
	if (scenarioFileName != null) {
	    ...
	    if (recordOrReplay) {
		isSuccess = ExpressSimulationXML.saveScenario(scenarioFileName,
			GraphicEditorAdaptator.getMainFrame()
				.getSimulationDialog().getRecordingPanel()
				.getRecordScenarioList(),
		...
		isSuccess = ExpressSimulationXML.saveScenario(scenarioFileName,
			GraphicEditorAdaptator.getMainFrame()
				.getSimulationDialog().getReplayPanel()
				.getReplayScenarioList(),
			ExpressSimulation.getCurrentTask());
		if (isSuccess)
	...
	if (currentSelectedTokenSimulation != null) {
	    SimulationAdaptator
		    .isExecutableTask(currentSelectedTokenSimulation);
	}
    }
	...
	Dimension dimension = GraphicEditorAdaptator.getMainFrame()
		.getSimulationDialog().getGraphSimulation().getPreferredSize();
	Dimension dimension1 = GraphicEditorAdaptator.getMainFrame()
		.getSimulationDialog().getGraphSimulation().getBounds()
		.getSize();
	dimension.width = Math.max(dimension.width, dimension1.width);
	dimension1.height = Math.max(dimension.height, dimension1.height);
	double d = GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getGraphSimulation().getScale();
	dimension.setSize((double) (dimension.width * 1) / d,
		(double) (dimension.height * 1) / d);
	Dimension dimension2 = GraphicEditorAdaptator.getMainFrame()
		.getSimulationDialog().getJGraphScrollPaneParent().getSize();
	...
	for (int i = 0; i < userList.size(); i++) {
	    ...
	    if (userList.get(i).getImage() == null
		    || userList.get(i).getImage().equals("")) {
		myCurrent = GraphicEditorAdaptator.getMainFrame()
			.getSimulationDialog().UNKNOWN_USER_IMAGEICON;
	    } else {
		Image tempImage = KMADEToolUtilities.getImageThumbnail(userList
			.get(i).getImage(), KMADEConstant.ROW_HEIGHT);

		if (tempImage == null) {
		    myCurrent = GraphicEditorAdaptator.getMainFrame()
			    .getSimulationDialog().ERROR_USER_IMAGEICON;
		} else {
		    myCurrent = new ImageIcon(tempImage);
		}
	    }

	    Object[] couple = { userList.get(i).getName(),
		    userList.get(i).getImage(), myCurrent };
	    temp[i] = couple;
	}
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.setUserListCombo(temp);
    }

    public static void undoSimulation() {
	ExpressHistory.undoStateHistory();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getGraphSimulation().repaint();
	...
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getRecordingPanel().clearLastActionInScenarioModel();

	if (ExpressHistory.getHistoricUndoLength() <= 0) {
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getRecordingPanel().setDisableUndoButton();
	} else {
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getRecordingPanel().setEnableUndoButton();
	}
    }
	...
	KMADEDefaultGraphCell myCell = (KMADEDefaultGraphCell) (GraphicEditorAdaptator
	...
	if (recordOrReplay) {
	    tokenSimulation = (TokenSimulation) GraphicEditorAdaptator
		    .getMainFrame().getSimulationDialog().getRecordingPanel()
		    .getActionList().getSelectedValue();
	} else {
	    tokenSimulation = (TokenSimulation) GraphicEditorAdaptator
		    .getMainFrame().getSimulationDialog().getReplayPanel()
		    .getScenarioList().getSelectedValue();
	}

	if (tokenSimulation != null) {
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getRecordingPanel().setToActionState();
	    ...
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getRecordingPanel().setToNoActionState();
	}
	...
	if (recordOrReplay) {
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .hideRecordConstraintsExecute();
	} else {
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .hideReplayConstraintsExecute();
	}
    }
	...
	KMADESimulationToolBar toolbar = GraphicEditorAdaptator.getMainFrame()
		.getSimulationDialog().getSimulationToolBar();
	...
	if (isSystem) {
	    ...
	    String acteurList = ExpressActeur.getActorList(tokenSimulation
		    .getTask());
	...
	if (toolbar.isPreSelected()) {
	    ...
	    try {
		tokenSimulation.getTask().getPreExpression()
			.getNodeExpression().evaluateNode(null);
		if ((Boolean) tokenSimulation.getTask().getPreExpression()
	    ...
	    if (tokenSimulation.getTask().getPreExpression()
		    .isAnyUserExpressionInExpression()) {
	...
	SimulationAdaptator.buildAnyUserExpressionOrConcreteObjectType(
		...
		currentUserEditTask.getEffetsDeBordExpression()
			.getObjectValues(), currentUserEditTask
	...
	refInputDialog
		.setLockOrUnlockPreconditionUserValuesAndConcreteObjects(currentUserEditTask
			.getPreExpression().isLocked());
	...
	if (currentTokenSimulation.isExecuterAction()) {
		...
		if (GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
			.getSimulationToolBar().isPreSelected()) {
		    boolean userValues = SimulationAdaptator
			    .searchAnyUserExpressionOrConcreteObjectType(myTask);
	    ...
	    KMADESimulationToolBar toolbar = GraphicEditorAdaptator
		    .getMainFrame().getSimulationDialog()
	    ...
	    if (!ExpressSimulation.isExecutableTask(exe, event, pre, value,
		    myTask)) {
		return false;
	    } else {
		ExpressSimulation.setExecuter(myTask, toolbar.isPostSelected(),
			toolbar.isIterSelected(),
		...
		if (temp.isExistCurrentEvaluateConcreteObject()) {
		    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
			    .setCurrentObjectPanel(temp.toString());
		} else {
		    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
			    .hideCurrentObjectPanel();
		}
	...
	int value = GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getSelectionUserListCombo();
	TokenRecordScenarioSimulation myTokenScenarioSimulation;
	if (value == -1) {
	    myTokenScenarioSimulation = new TokenRecordScenarioSimulation(
		    currentTokenSimulation);
	} else {
	    User selectUser = ExpressSimulation.getCurrentUserList(value);
	    myTokenScenarioSimulation = new TokenRecordScenarioSimulation(
		    currentTokenSimulation, selectUser);
	}
	...
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getSimulationToolBar().setDisabledNoSelectedTask();
    }
	...
	GraphicEditorAdaptator
		...
		.setVisible(
			!GraphicEditorAdaptator.getMainFrame()
				.getSimulationDialog().getPreviewWindow()
				.isVisible());
    }

    public static void enabledMainFrameAfterEdition() {
	InDevelopmentGlassPanel.unPlugWindowInDisabled(GraphicEditorAdaptator
		.getMainFrame().getSimulationDialog());
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.enableSimulationDialog();
    }
	...
	InDevelopmentGlassPanel.plugWindowInDisabled(GraphicEditorAdaptator
		.getMainFrame().getSimulationDialog());
    }
	...
	Expression myEffetsDeBordExpression = currentUserEditTask
		.getEffetsDeBordExpression();
	...
	for (int i = 0; i < refInputDialog.getExpressionPreconditionFieldList()
		.size() && !erreurUserExpression; i++) {
	    erreurUserExpression = erreurUserExpression
		    | refInputDialog.getExpressionPreconditionFieldList()
			    .get(i).isUnknownOrErrorUserExpressionState();
	}
	boolean erreurConcreteObject = false;
	for (int i = 0; i < refInputDialog.getExpressionPreconditionComboList()
		.size() && !erreurConcreteObject; i++) {
	    erreurConcreteObject = erreurConcreteObject
		    | refInputDialog.getExpressionPreconditionComboList()
			    .get(i).isConcreteObjectEmpty();
	}
	...
	if (message.equals("")) {
	    ...
	    SimulationAdaptator.storeUserConcreteObjects(
		    ...
		    currentUserEditTask.getPreExpression());
	    SimulationAdaptator.storeUserConcreteObjects(
		    refInputDialog.getExpressionEffetsDeBordComboList(),
		    currentUserEditTask.getEffetsDeBordExpression());
	    SimulationAdaptator.storeUserConcreteObjects(
	    ...
	    JOptionPane.showMessageDialog(
		    refInputDialog,
		    message,
		    KMADEConstant.USER_VALUES_PROBLEM_MESSAGE + "...",
    ...
    private static void storeUserValues(
	    ArrayList<KMADEUserExpressionField> myFieldList,
    ...
    private static void storeUserConcreteObjects(
	    ArrayList<KMADEGroupTypeComboBox> arrayList, Expression myExpression) {
	...
	ArrayList<Object> myListPre = myTask.getPreExpression()
		.getNodeExpression().getLinearExpression();
	ArrayList<JComponent> pCompoPre = new ArrayList<JComponent>();
	boolean valeur = SimulationAdaptator
		.buildAnyUserExpressionOrConcreteObjectType(myListPre,
			refInputDialog.getExpressionPreconditionFieldList(),
			myTask.getPreExpression().getObjectValues(), myTask
				.getPreExpression().isLocked(), refInputDialog
				.getExpressionPreconditionComboList(),
			pCompoPre);

	// Préparation pour les valeurs utilisateurs de la effetsdebord.
	ArrayList<Object> myListPost = myTask.getEffetsDeBordExpression()
		.getNodeExpression().getLinearExpression();
	ArrayList<JComponent> pCompoPost = new ArrayList<JComponent>();
	valeur |= SimulationAdaptator
		.buildAnyUserExpressionOrConcreteObjectType(myListPost,
			refInputDialog.getExpressionEffetsDeBordFieldList(),
			myTask.getEffetsDeBordExpression().getObjectValues(),
			myTask.getPreExpression().isLocked(),
			refInputDialog.getExpressionEffetsDeBordComboList(),
			pCompoPost);
	...
	if (valeur) {
	    refInputDialog
		    .setLockOrUnlockPreconditionUserValuesAndConcreteObjects(myTask
			    .getPreExpression().isLocked());
	    refInputDialog.setInputUserConcretePreconditionPanel(pCompoPre);
	    refInputDialog.setInputUserConcreteEffetsDeBordPanel(pCompoPost);
	    refInputDialog.setInputUserConcreteIterationPanel(pCompoIter);
    ...
    /**
     * @param noOpen
     *            : force ou pas l'ouverture selon les contraintes d'ouverture.
     * @param myList
     *            : la liste des objets de l'expression
     * @param myfield
     *            : la référence à la liste des futurs champs de texte
     * @param value
    ...
    private static boolean buildAnyUserExpressionOrConcreteObjectType(
	    ...
	    ArrayList<String> value, boolean isLocked,
	...
	for (Object tt : myList) {
		...
		if (myIte.hasNext()) {
		    current.setInitValueIntoUserExpression(myIte.next()
			    .toString());
		} else {
	...
	recordScenarioModified = false;
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getRecordingPanel().clearScenarioListModel();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getRecordingPanel().setToNoActionState();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getRecordingPanel().clearAvailableTasksListModel();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getRecordingPanel().setDisableUndoButton();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().setToInitState();
    }

    public static boolean isRecordScenario() {
	// Enregistrer le scénario
	if ((!GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getRecordingPanel().isEmptyScenarioModel())
		&& recordScenarioModified) {
	    int result = JOptionPane.showConfirmDialog(GraphicEditorAdaptator
		    .getMainFrame().getSimulationDialog(),
	...
	if (myScenarioFile != null) {
	    ReplayScenarioModel scenarioModel = ExpressSimulationXML
		    .loadScenario(myScenarioFile);
	    ArrayList<TokenReplayScenarioSimulation> scenarioList = scenarioModel
		    .getTokenReplayScenarioSimulation();
		...
		GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
			.getReplayPanel().updateReplayListTask(scenarioList);
		SimulationAdaptator.initReplayScenario(scenarioModel
			.getRootTask());
	...
	if (!value) {
	    ...
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getReplayPanel().resumeAfterExecuteAction();
	}
    }
    ...
    private static boolean launchActionFromReplay(
	...
	ArrayList<TokenSimulation> myTasks = ExpressSimulation
		.continueSimulation();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().updateListEnabledTask(myTasks);
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().incrementCurrentReplaySimulation();
	SimulationAdaptator.updateSimulationGraph();
	...
	if (GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		...
		GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
			.getReplayPanel().setSelectCurrentTask();
	    }
	}
	if (isFinished) {
	    ...
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getReplayPanel().stopWaitController();
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getReplayPanel().setToStopState();
	}
	return true;
	...
	if (ExpressSimulation.getCurrentTask() == null) {
	    ...
	    SimulationAdaptator.initReplayScenario(ExpressSimulation
		    .getCurrentTask());
	}
    }
	...
	case SimulationAdaptator.STOP_STATE: {
	    ...
	    if (REPLAY_SCENARIO_STATE == REPLAY_SCENARIO_FINISHED
		    || REPLAY_SCENARIO_STATE == REPLAY_SCENARIO_STOPED) {
		if (!SimulationAdaptator.isReplayScenario()) {
		    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
			    .getReplayPanel().setToNoPlaySelectedStateButton();
		    return;
		}
		GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
			.getReplayPanel().clearScenarioListModel();
	    }
	    ...
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getReplayPanel().startWaitController();
	    break;
	}
	case SimulationAdaptator.PLAY_STATE: {
	    // Ne rien faire.
	...
	case SimulationAdaptator.PLAY_STATE: {
	    ...
	    GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		    .getReplayPanel().pauseWaitController();
	    break;
	}
	case SimulationAdaptator.PAUSE_STATE: {
	    // Ne rien faire.
	...
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().setToPauseState();
    }

    public static void fireNextActionReplaySimulation(
	    TokenReplayScenarioSimulation p) {
	switch (SIMULATION_STATE) {
	case SimulationAdaptator.PAUSE_STATE: {
	    SimulationAdaptator.SIMULATION_STATE = SimulationAdaptator.PAUSE_STATE;
	    break;
	...
	replayScenarioModified = false;
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().clearAvailableTasksListModel();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().clearScenarioListModel();
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().clearScenarioPerformedListModel();
	SIMULATION_STATE = SimulationAdaptator.STOP_STATE;
	REPLAY_SCENARIO_STATE = SimulationAdaptator.NO_REPLAY_SCENARIO;
	GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().setToInitState();
	...
	// Enregistrer le scénario
	if ((!GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
		.getReplayPanel().isEmptyScenarioModel())
