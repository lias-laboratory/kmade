...
import fr.upensma.lias.kmade.tool.coreadaptator.simulation.ExpressSimulation;
import fr.upensma.lias.kmade.tool.coreadaptator.simulation.ExpressSimulationXML;
import fr.upensma.lias.kmade.tool.coreadaptator.simulation.ReplayScenarioModel;
...
import fr.upensma.lias.kmade.tool.view.simulation.KMADEInputUserExpressionAndConcreteObjectDialog;
import fr.upensma.lias.kmade.tool.view.simulation.KMADESimulationToolBar;
import fr.upensma.lias.kmade.tool.view.taskmodel.KMADEDefaultGraphCell;
import fr.upensma.lias.kmade.tool.view.taskproperties.constrainteditors.KMADEArrayTypeComboBox;
import fr.upensma.lias.kmade.tool.view.taskproperties.constrainteditors.KMADEGroupTypeComboBox;
import fr.upensma.lias.kmade.tool.view.taskproperties.constrainteditors.KMADESetTypeComboBox;
...
public final class SimulationAdaptator {
	private static KMADEInputUserExpressionAndConcreteObjectDialog refInputDialog = new KMADEInputUserExpressionAndConcreteObjectDialog(
	...
	private static boolean recordOrReplay = true; // False : record / True :
	// replay

	private static boolean recordScenarioModified = false; // False : non
	// modifié / True :
	// modifié
	...
	public static boolean saveScenario() {
		...
		if (scenarioFileName != null) {
			...
			if (recordOrReplay) {
				isSuccess = ExpressSimulationXML
						.saveScenario(
								scenarioFileName, GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
										.getRecordingPanel().getRecordScenarioList(),
								ExpressSimulation.getCurrentTask());
				if (isSuccess)
					recordScenarioModified = false;
			} else {
				isSuccess = ExpressSimulationXML.saveScenario(scenarioFileName, GraphicEditorAdaptator.getMainFrame()
						.getSimulationDialog().getReplayPanel().getReplayScenarioList(),
						ExpressSimulation.getCurrentTask());
				if (isSuccess)
	...
	public static void setGlobalTaskModel() {
		Dimension dimension = GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getGraphSimulation()
				.getPreferredSize();
		Dimension dimension1 = GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getGraphSimulation()
				.getBounds().getSize();
		...
		Dimension dimension2 = GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getJGraphScrollPaneParent()
				.getSize();
	...
	private static void generateExecutant() {
		...
		for (int i = 0; i < userList.size(); i++) {
			...
			if (userList.get(i).getImage() == null || userList.get(i).getImage().equals("")) {
				...
				Image tempImage = KMADEToolUtilities.getImageThumbnail(userList.get(i).getImage(),
	...
	public static void undoSimulation() {
		...
		GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getRecordingPanel()
				.clearLastActionInScenarioModel();
	...
	public static void initRecordLaunchSimulation() {
		...
		KMADEDefaultGraphCell myCell = (KMADEDefaultGraphCell) (GraphicEditorAdaptator.getMainFrame()
				.getSimulationDialog().getGraphSimulation().getSelectionCell());
		if (myCell == null) {
			...
			GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getGraphSimulation().repaint();
			GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getRecordingPanel()
					.updateListEnabledTask(myTasks);
		}
	}

	public static void goToTask(TokenSimulation tokenSimulation) {
		if (tokenSimulation != null) {
			GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getGraphSimulation()
					.scrollCellToVisible((KMADEDefaultGraphCell) (tokenSimulation.getTask().getRefJTask()));
		}
	}
	...
	public static void selectAndGoToTask() {
		...
		if (recordOrReplay) {
			tokenSimulation = (TokenSimulation) GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
					.getRecordingPanel().getActionList().getSelectedValue();
		} else {
			tokenSimulation = (TokenSimulation) GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
					.getReplayPanel().getScenarioList().getSelectedValue();
		}
		...
		if (tokenSimulation != null) {
			...
			GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getGraphSimulation().scrollCellToVisible(
					(KMADEDefaultGraphCell) (currentSelectedTokenSimulation.getTask().getRefJTask()));
			GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getGraphSimulation()
					.setSelectionCell(currentSelectedTokenSimulation.getTask().getRefJTask());
	...
	private static void isExecutableTask(TokenSimulation tokenSimulation) {
		...
		KMADESimulationToolBar toolbar = GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
				.getSimulationToolBar();

		// Utilisateur ...
		String[] executantTab = new String[2];
		boolean isSystem = tokenSimulation.getTask().getExecutor().equals(Executor.SYS);
		if (isSystem) {
		...
		if (toolbar.isPreSelected()) {
			if (tokenSimulation.getTask().getPreExpression().getNodeExpression() == null) {
				preconditionTab[0] = tokenSimulation.getTask().getPreExpression().getFormalText();
			} else {
				preconditionTab[0] = ExpressSimulation.getLinearExpressionWithUserValues(
						tokenSimulation.getTask().getPreExpression().getNodeExpression().getLinearExpression());
			}
	...
	public static void editUserValues() {
		...
		SimulationAdaptator.buildAnyUserExpressionOrConcreteObjectType(myListPre,
				refInputDialog.getExpressionPreconditionFieldList(),
				currentUserEditTask.getPreExpression().getObjectValues(),
				currentUserEditTask.getPreExpression().isLocked(), refInputDialog.getExpressionPreconditionComboList(),
				pCompoPre);

		// Préparation pour les valeurs utilisateurs de la effetsdebord.
		ArrayList<Object> myListPost = currentUserEditTask.getEffetsDeBordExpression().getNodeExpression()
				.getLinearExpression();
		ArrayList<JComponent> pCompoPost = new ArrayList<JComponent>();
		SimulationAdaptator.buildAnyUserExpressionOrConcreteObjectType(myListPost,
				refInputDialog.getExpressionEffetsDeBordFieldList(),
				currentUserEditTask.getEffetsDeBordExpression().getObjectValues(),
				currentUserEditTask.getPreExpression().isLocked(), refInputDialog.getExpressionEffetsDeBordComboList(),
				pCompoPost);

		// Préparation pour les valeurs utilisateurs de l'itération.
		ArrayList<Object> myListIter = currentUserEditTask.getIterExpression().getNodeExpression()
				.getLinearExpression();
		ArrayList<JComponent> pCompoIter = new ArrayList<JComponent>();
		SimulationAdaptator.buildAnyUserExpressionOrConcreteObjectType(myListIter,
				refInputDialog.getExpressionIterationFieldList(),
				currentUserEditTask.getIterExpression().getObjectValues(),
				currentUserEditTask.getPreExpression().isLocked(), refInputDialog.getExpressionIterationComboList(),
				pCompoIter);

		refInputDialog.setLockOrUnlockPreconditionUserValuesAndConcreteObjects(
	...
	private static boolean launchAction(TokenSimulation tokenSimulation) {
		...
		if (currentTokenSimulation.isExecuterAction()) {
			if (!ExpressIteration.isFinished(myTask)) {
				if (GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getSimulationToolBar()
						.isPreSelected()) {
			...
			if (!ExpressSimulation.isExecutableTask(exe, event, pre, value, myTask)) {
				...
				ExpressSimulation.setExecuter(myTask, toolbar.isPostSelected(), toolbar.isIterSelected(),
						toolbar.isTrigEventSelected(), event);
				// Mise à jour de la partie événement.
				GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
						.setEventListModel(ExpressEvent.getAllCurrentEvents());
	...
	private static void addActionInScenarioModel() {
		...
		GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getRecordingPanel()
				.addActionInScenarioModel(myTokenScenarioSimulation);
	}
	...
	public static void selectOneTask(Object defaultGraphCell) {
		if (defaultGraphCell instanceof KMADEDefaultGraphCell) {
			...
			KMADEMainFrame.getProjectPanel().getPanelProprieteTache().displayTaskProperties(tache.getNumber(),
					tache.getMotherTaskName(), tache.getName(), tache.getGoal(),
					/* tache.getMedia(), */
					tache.getLabelName(),
					/* tache.getFeedBack(), */
					tache.getDuration(), tache.getDescription(), tache.getExecutor(), tache.getModality().getValue(),
					tache.getFrequency().getValue(), tache.getFrequencyValue(), tache.getImportance().getValue(),
					tache.getEventsName(), tache.isOptional(), tache.isInterruptible(), tache.getRaisingEventName(),
					tache.getActors(), tache.getPreExpression(), tache.getEffetsDeBordExpression(),
					tache.getOrdering().getValue(), tache.getIterExpression());
			GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getSimulationToolBar()
					.setEnabledOneSelectedTask();
		} else {
			// C'est un lien.
			GraphicEditorAdaptator.getMainFrame().getSimulationDialog().hideTaskProperties();
			GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getSimulationToolBar()
					.setDisabledNoSelectedTask();
		}
	}

	public static void showOverviewWindow() {
		GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getPreviewWindow().setVisible(
				!GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getPreviewWindow().isVisible());
	}
	...
	public static void closeInputConcreteDialog() {
		...
		for (int i = 0; i < refInputDialog.getExpressionPreconditionFieldList().size() && !erreurUserExpression; i++) {
			erreurUserExpression = erreurUserExpression
					| refInputDialog.getExpressionPreconditionFieldList().get(i).isUnknownOrErrorUserExpressionState();
		}
		...
		for (int i = 0; i < refInputDialog.getExpressionPreconditionComboList().size() && !erreurConcreteObject; i++) {
			erreurConcreteObject = erreurConcreteObject
					| refInputDialog.getExpressionPreconditionComboList().get(i).isConcreteObjectEmpty();
		}
		...
		if (message.equals("")) {
			...
			SimulationAdaptator.storeUserValues(refInputDialog.getExpressionPreconditionFieldList(),
					currentUserEditTask.getPreExpression());
			SimulationAdaptator.storeUserValues(refInputDialog.getExpressionEffetsDeBordFieldList(),
					currentUserEditTask.getEffetsDeBordExpression());
			SimulationAdaptator.storeUserValues(refInputDialog.getExpressionIterationFieldList(),
					currentUserEditTask.getIterExpression());

			// Stocke les objets concrets choisis par l'utilisateur.
			SimulationAdaptator.storeUserConcreteObjects(refInputDialog.getExpressionPreconditionComboList(),
					currentUserEditTask.getPreExpression());
			SimulationAdaptator.storeUserConcreteObjects(refInputDialog.getExpressionEffetsDeBordComboList(),
					currentUserEditTask.getEffetsDeBordExpression());
			SimulationAdaptator.storeUserConcreteObjects(refInputDialog.getExpressionIterationComboList(),
					currentUserEditTask.getIterExpression());
			...
			JOptionPane.showMessageDialog(refInputDialog, message, KMADEConstant.USER_VALUES_PROBLEM_MESSAGE + "...",
					JOptionPane.ERROR_MESSAGE,
					new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.INFO_DIALOG_IMAGE)));
		}
	...
	private static boolean searchAnyUserExpressionOrConcreteObjectType(Task myTask) {
		// Préparation pour les valeurs utilisateurs de la précondition.
		ArrayList<Object> myListPre = myTask.getPreExpression().getNodeExpression().getLinearExpression();
		ArrayList<JComponent> pCompoPre = new ArrayList<JComponent>();
		boolean valeur = SimulationAdaptator.buildAnyUserExpressionOrConcreteObjectType(myListPre,
				refInputDialog.getExpressionPreconditionFieldList(), myTask.getPreExpression().getObjectValues(),
				myTask.getPreExpression().isLocked(), refInputDialog.getExpressionPreconditionComboList(), pCompoPre);

		// Préparation pour les valeurs utilisateurs de la effetsdebord.
		ArrayList<Object> myListPost = myTask.getEffetsDeBordExpression().getNodeExpression().getLinearExpression();
		ArrayList<JComponent> pCompoPost = new ArrayList<JComponent>();
		valeur |= SimulationAdaptator.buildAnyUserExpressionOrConcreteObjectType(myListPost,
				refInputDialog.getExpressionEffetsDeBordFieldList(),
				myTask.getEffetsDeBordExpression().getObjectValues(), myTask.getPreExpression().isLocked(),
				refInputDialog.getExpressionEffetsDeBordComboList(), pCompoPost);

		// Préparation pour les valeurs utilisateurs de l'itération.
		ArrayList<Object> myListIter = myTask.getIterExpression().getNodeExpression().getLinearExpression();
		ArrayList<JComponent> pCompoIter = new ArrayList<JComponent>();
		valeur |= SimulationAdaptator.buildAnyUserExpressionOrConcreteObjectType(myListIter,
				refInputDialog.getExpressionIterationFieldList(), myTask.getIterExpression().getObjectValues(),
				myTask.getPreExpression().isLocked(), refInputDialog.getExpressionIterationComboList(), pCompoIter);

		currentUserEditTask = myTask;

		if (valeur) {
			refInputDialog
					.setLockOrUnlockPreconditionUserValuesAndConcreteObjects(myTask.getPreExpression().isLocked());
			refInputDialog.setInputUserConcretePreconditionPanel(pCompoPre);
			refInputDialog.setInputUserConcreteEffetsDeBordPanel(pCompoPost);
			refInputDialog.setInputUserConcreteIterationPanel(pCompoIter);
			SimulationAdaptator.disabledMainFrameBeforeEdition();
	...
	/**
	 * @param noOpen          : force ou pas l'ouverture selon les contraintes
	 *                        d'ouverture.
	 ...
	 */
	private static boolean buildAnyUserExpressionOrConcreteObjectType(ArrayList<Object> myList,
			ArrayList<KMADEUserExpressionField> myfield, ArrayList<String> value, boolean isLocked,
			ArrayList<KMADEGroupTypeComboBox> arrayList, ArrayList<JComponent> myListComponent) {
		// Nettoyage de la liste des champs pour les valeurs utilisateurs et les
		// objets concrets utilisateurs.
		myfield.clear();
		arrayList.clear();
			...
			if (tt instanceof UserExpression) {
				...
				if (((ConcreteObjectType) tt).getConcreteObject() != null) {
					...
					if (((ConcreteObjectType) tt).isGroupSetType()) {
						isUserConcreteExpression = true;
						KMADESetTypeComboBox currentCombo = new KMADESetTypeComboBox((ConcreteObjectType) tt,
						...
						isUserConcreteExpression = true;
						KMADEArrayTypeComboBox currentCombo = new KMADEArrayTypeComboBox((ConcreteObjectType) tt,
								((ConcreteObjectType) tt).getConcreteObjects());
						arrayList.add(currentCombo);
						myListComponent.add(currentCombo);
					} else {
	...
	public static boolean isRecordScenario() {
		...
		if ((!GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getRecordingPanel().isEmptyScenarioModel())
				&& recordScenarioModified) {
			int result = JOptionPane.showConfirmDialog(GraphicEditorAdaptator.getMainFrame().getSimulationDialog(),
					KMADEConstant.RECORD_SCENARIO_MESSAGE);
			if (result == JOptionPane.YES_OPTION) {
				// Montrer la boite de dialogue qui permet d'enregistrer le
				// scéanrio
	...
	public static void loadScenario() {
		// Avant de charger un nouveau scénario, regardons s'il n'existe pas un
		...
		if (myScenarioFile != null) {
			...
			if (scenarioModel.getRootTask() == null) {
				...
				GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getReplayPanel()
						.updateReplayListTask(scenarioList);
				SimulationAdaptator.initReplayScenario(scenarioModel.getRootTask());
				// Initialise la présentation du gestionnaire de Replay :
	...
	private static boolean launchActionFromReplay(TokenReplayScenarioSimulation tokenSimulation) {
		if (tokenSimulation.isNotFounded()) {
			...
			KMADEHistoryMessageManager
					.printlnMessage(KMADEConstant.NO_FOUNDED_TASK_TO_REPLAY_PROBLEM_MESSAGE + " : UNDER DEVELOPMENT");
			return false;
		} else {
			if (!tokenSimulation.isActionable(GraphicEditorAdaptator.getMainFrame().getSimulationDialog()
					.getReplayPanel().getAvailableTaches())) {
				KMADEHistoryMessageManager.printlnMessage(
						KMADEConstant.CAN_NOT_EXECUTE_TASK_TO_REPLAY_PROBLEM_MESSAGE + " : UNDER DEVELOPMENT");
				return false;
			}
			...
			tokenSimulation.getTask().getIterExpression().setObjectValues(tokenSimulation.getUserValueIter());
		}
	...
	private static void addActionInScenarioReplayModel() {
		...
		GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getReplayPanel()
				.addActionInReplayScenarioModel(myTokenScenarioSimulation);
	}
	...
	public static void firePlayActionReplaySimulation() {
		...
		case SimulationAdaptator.STOP_STATE: {
			...
			if (REPLAY_SCENARIO_STATE == REPLAY_SCENARIO_FINISHED || REPLAY_SCENARIO_STATE == REPLAY_SCENARIO_STOPED) {
				if (!SimulationAdaptator.isReplayScenario()) {
					GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getReplayPanel()
							.setToNoPlaySelectedStateButton();
					return;
				}
	...
	private static boolean isReplayScenario() {
		...
		GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getReplayPanel().stopWaitController();
		// Enregistrer le scénario
		if ((!GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getReplayPanel().isEmptyScenarioModel())
				&& replayScenarioModified) {
			int result = JOptionPane.showConfirmDialog(GraphicEditorAdaptator.getMainFrame().getSimulationDialog(),
					KMADEConstant.RECORD_SCENARIO_MESSAGE);
			if (result == JOptionPane.YES_OPTION) {
				// Montrer la boite de dialogue qui permet d'enregistrer le
