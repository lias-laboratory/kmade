...
public class KMADEReadWriteAbstractAttributesTable extends JScrollPane implements LanguageFactory {
	...
	public KMADEReadWriteAbstractAttributesTable() {
		String[] columnNames = { KMADEConstant.ABSTRACT_ATTRIBUT_NAME_TABLE,
				KMADEConstant.ABSTRACT_ATTRIBUT_DESCRIPTION_TABLE, KMADEConstant.ABSTRACT_ATTRIBUT_TYPE_TABLE,
				...
				if (!RegularExpression.isGoodAbstractAttributName(name)) {
					...
					if (AttributAbstrait.isUniqueName(name)) {
						...
						name = (String) JOptionPane.showInputDialog(GraphicEditorAdaptator.getMainFrame(),
								...
								new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.ASK_DIALOG_IMAGE)),
								null, AttributAbstrait.propositionNom(name));
					}
				}
			}
			return name;
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
			...
			if (rowIndex + 1 == this.getRowCount()) {
				...
				String newAbstractObject = AbstractAttributeAdaptator
						.addAbstractAttributes(AbstractObjectPanelAdaptator.getActiveAbstractObject());
				value = AbstractAttributeAdaptator.setAbstractAttributeName(newAbstractObject, name);
				String[] nomType = { "", Oid.OID_NULL };
				Object[] tempo = { name, "", TypeStructure.STRING_STRUCT.getValue(), nomType, newAbstractObject,
						false };
				...
				case 0: {
					...
					value = AbstractAttributeAdaptator.setAbstractAttributeName(oidRow, name);
					break;
				}
					...
					if (aValue instanceof Integer) {
						if ((Integer) aValue != -1) {
							...
							if (typeOfStructure == 1 && listEnumeration != null) {
								...
								AbstractAttributeAdaptator.setAbstractAttributNameType(oidRow,
			...
			if (super.isCellEditable(rowIndex, columnIndex)) {
				if (columnIndex == 3) {
					...
					if (((String) maList.get(rowIndex)[columnIndex - 1])
						...
						typeOfStructure = 1;
						myListTable.getMyTable().getColumnModel().getColumn(3)
								.setCellEditor(new MyCustomListTableCellEditor(listEnumerationName));
					} else {
						typeOfStructure = 0;
						return false;
					}
	...
	class MyKeyListener extends KeyAdapter {
		public void keyPressed(KeyEvent ke) {
			if (ke.getKeyCode() == KeyEvent.VK_DELETE) {
				...
				boolean state = GraphicEditorAdaptator.removeSelectedItem("Attribut", r,
