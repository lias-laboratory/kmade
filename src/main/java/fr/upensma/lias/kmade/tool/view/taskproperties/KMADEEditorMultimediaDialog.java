...
import fr.upensma.lias.kmade.tool.viewadaptator.MediaAdaptator;
 ...
 */
public class KMADEEditorMultimediaDialog extends JPropertiesEditorDialog
    ...
    private static final ImageIcon PLAY_IMAGE = new ImageIcon(
	    KMADEEditorMultimediaDialog.class
		    .getResource(KMADEConstant.PLAY_IMAGE));

    private static final ImageIcon PAUSE_IMAGE = new ImageIcon(
	    KMADEEditorMultimediaDialog.class
		    .getResource(KMADEConstant.PAUSE_IMAGE));

    private static final ImageIcon PLAY_MARK_IMAGE = new ImageIcon(
	    KMADEEditorMultimediaDialog.class
		    .getResource(KMADEConstant.PLAY_MARK_IMAGE));

    private static final ImageIcon PAUSE_MARK_IMAGE = new ImageIcon(
	    KMADEEditorMultimediaDialog.class
		    .getResource(KMADEConstant.PAUSE_MARK_IMAGE));

    private static final ImageIcon START_MARK_IMAGE = new ImageIcon(
	    KMADEEditorMultimediaDialog.class
		    .getResource(KMADEConstant.START_MARK_IMAGE));

    private static final ImageIcon FINISH_MARK_IMAGE = new ImageIcon(
	    KMADEEditorMultimediaDialog.class
		    .getResource(KMADEConstant.FINISH_MARK_IMAGE));

    private static final ImageIcon INIT_START_MARK_IMAGE = new ImageIcon(
	    KMADEEditorMultimediaDialog.class
		    .getResource(KMADEConstant.INIT_START_MARK_IMAGE));

    private static final ImageIcon INIT_FINISH_MARK_IMAGE = new ImageIcon(
	    KMADEEditorMultimediaDialog.class
		    .getResource(KMADEConstant.INIT_FINISH_MARK_IMAGE));
	...
	panelInformation
		.setBorder(BorderFactory
			.createTitledBorder(KMADEConstant.INFORMATION_MEDIA_TITLE_NAME));
	...
	panelTopInformation.setBorder(BorderFactory.createEmptyBorder(5, 5, 5,
		5));
	...
	panelInformationEmpty
		.add(new JLabel(
			KMADEConstant.MEDIA_NO_INFORMATION_PANEL_MESSAGE,
			JLabel.CENTER));
	...
	panelMovie.setBorder(BorderFactory
		.createTitledBorder(KMADEConstant.MEDIA_TITLE_NAME));
	cardMovie = new CardLayout();
	panelMovie.setLayout(cardMovie);

	JPanel panelEmpty = new JPanel(new BorderLayout());
	panelEmpty.add(new JLabel(KMADEConstant.NO_MEDIA_FILE_MESSAGE,
		JLabel.CENTER));

	JPanel panelNoCodec = new JPanel(new BorderLayout());
	panelNoCodec.add(new JLabel(
		KMADEConstant.QUICK_TIME_NO_INSTALLED_ERROR_MESSAGE,
		JLabel.CENTER));
	...
	panelControl.setBorder(BorderFactory
		.createTitledBorder(KMADEConstant.CONTROLE_MEDIA_TITLE_NAME));
	...
	this.setTitle(KMADEConstant.EDITOR_TEXT_TITLE_NAME + " : "
		+ refModel.getValueAt(prow, 0));

	MediaAdaptator.openMediaEditor(refModel.getValue(prow));
	super.showPropertiesEditor(refModel, prow);
    }

    public void openNewMovie(QTJComponent qc, int duration,
	    String realDuration, String fileName, long length) {
