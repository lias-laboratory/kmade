...
public class AbstractObjectCell extends ObjectDefaultGraphCell {
	...
	public String[] getObjectAttributes() {
		if (!item.getInverseAttributsAbs().isEmpty()) {
			String[] attributes = new String[item.getInverseAttributsAbs().size()];
			for (int i = 0; i < attributes.length; i++) {
				attributes[i] = this.item.getInverseAttributsAbs().get(i).getName() + " : "
						+ TypeStructure.getEnumereIntoLocaleTypeStructure(
								this.item.getInverseAttributsAbs().get(i).getTypeStructure().getValue());
			}
