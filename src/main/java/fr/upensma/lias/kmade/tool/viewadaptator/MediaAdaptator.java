...
public final class MediaAdaptator {
	...
	private static final ImageIcon START_MARK_SLIDER_IMAGE = new ImageIcon(
			KMADEEditorMultimediaDialog.class.getResource(KMADEConstant.START_MARK_SLIDER_IMAGE));

	private static final ImageIcon FINISH_MARK_SLIDER_IMAGE = new ImageIcon(
			KMADEEditorMultimediaDialog.class.getResource(KMADEConstant.FINISH_MARK_SLIDER_IMAGE));
	...
	public static void openMovie(QTFile qtf) {
		try {
			...
			GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().openNewMovie(qc, duration,
					getTimeWithFormat(duration / currentMovie.getTimeScale()),
					mediaFilePath + File.separator + mediaFileName, qtf.length() / 1024);
			if (startMark != -1) {
	...
	private static void startMarkMovie() {
		...
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia()
				.setStartMarkTextField(getTimeWithFormat((getRealCurrentTime(startMark))));
	}
	...
	private static void finishMarkMovie() {
		...
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia().setEnablePlayMarkButton(labelTable);
		GraphicEditorAdaptator.getPanelProp().getEditorMultimedia()
				.setFinishMarkTextField(getTimeWithFormat((getRealCurrentTime(finishMark))));
	}
