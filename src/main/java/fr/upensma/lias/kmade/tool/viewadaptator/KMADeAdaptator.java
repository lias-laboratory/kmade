...
public final class KMADeAdaptator {
	...
	public static void debugSimulation() {
		...
		if (myFile != null) {
			...
			GraphicEditorAdaptator.getMainFrame().getProgressBarDialog().goReadSPF(false, "");
			GraphicEditorAdaptator.enabledMainFrameAfterEdition();
			GraphicEditorAdaptator.getMainFrame().getProgressBarDialog().setVisible(false);
	...
	private static boolean coherenceBeforeTool() {
		...
		if (erreur != 0) {
			...
			message += " " + warning + " ";
			message += (erreur == 1) ? KMADEConstant.COHERENCE_ONE_WARNING : KMADEConstant.COHERENCE_SOME_WARNING;
			String titre = KMADEConstant.COHERENCE_VERIFICATION_TITLE;
			JOptionPane.showMessageDialog(GraphicEditorAdaptator.getMainFrame(), message, titre,
	...
	public static void undoAction() {
		...
		GraphicEditorAdaptator.getTaskModelPanel().getUndoRedoManager().undo();

	}

	public static void redoAction() {

		GraphicEditorAdaptator.getTaskModelPanel().getUndoRedoManager().redo();
	...
	public static void closeProject() {
		KMADeAdaptator.doSaveProjectBeforeClose(KMADEConstant.WRITE_BEFORE_CLOSE_PROJECT_MESSAGE);
	}

	public static void showPreferences() {
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().setVisible(true);
	}
	...
	public static void saveProjectXML() {
		if (fileName == null | fileName.equals("")) {
			...
			if (myCurrentFile != null) {
				...
				GraphicEditorAdaptator.getMainFrame().getProgressBarDialog().writeKMADModelFromXMLFile();
			}
		}
	}
	...
	public static void exportSimpleText() {
		ExpressKMADTXT.displayKMADSimpleModel();
		GraphicEditorAdaptator.getMainFrame().getSimpleTreePanel().setVisible(true);
	...
	public static void loadItemsXML() {
		// Avant d'ouvrir demander si le projet courant doit �tre sauvegard�.
		if (InterfaceExpressJava.isBddSet()) {
			int value = KMADeAdaptator.askToSaveBeforeAction(KMADEConstant.WRITE_BEFORE_OPEN_OBJECTS_MESSAGE);
			if (value == JOptionPane.YES_OPTION) {
				// On peut sauvegarder avant
	...
	/**
	 * @author Joachim TROUVERIE To save the objects separately of the model
	 */
	public static void saveItemsXML() {
		if (fileName == null | fileName.equals("")) {
			...
			if (myCurrentFile != null) {
				ExpressKMADXML.saveKMADModel(myCurrentFile.getAbsolutePath());
				GraphicEditorAdaptator.getMainFrame().getProgressBarDialog().writeKMADItemsFromXMLFile();
			}
		}
	}
	...
	public static void openProject() {
		...
		if (myCurrentFile != null) {
			...
			try {
				...
				message = KMADEConstant.OPEN_EXPRESS_OK_FILE + myCurrentFile.getAbsolutePath();
				ioError = false;
				fileName = myCurrentFile.getAbsolutePath();
				// Activer le bouton Sauvegarde et changer le titre.
				GraphicEditorAdaptator.getMainFrame().enableSaveTitleName(fileName);
			} catch (Exception e) {
				message = KMADEConstant.OPEN_EXPRESS_NO_OK_FILE + e.getLocalizedMessage();
				ioError = true;
				fileName = "";
				GraphicEditorAdaptator.getMainFrame().disableSaveTitleName();
			}
			updateThread = true;
			GraphicEditorAdaptator.getMainFrame().getProgressBarDialog().goReadSPF(ioError, message);
		}
	}
	...
	public static void showHistoryDialog() {
		GraphicEditorAdaptator.getMainFrame().getInfoHistoryDialog().showInfoAbout(KMADEConstant.HISTORY_TITLE_NAME,
				KMADEConstant.HISTORY_KMADE_HTML);
	}

	public static void showInfoDebugDialog() {
		GraphicEditorAdaptator.getMainFrame().getInfoDebugDialog().showInfoAbout(KMADEConstant.INFO_DEBUG_TITLE_NAME,
				KMADEConstant.DEBUG_KMADE_HTML);
	}

	public static void showHelpModelDialog() {
		GraphicEditorAdaptator.getMainFrame().getInfoNMDADialog().showInfoAbout(KMADEConstant.HELP_KMAD_TITLE_NAME,
				KMADEConstant.HELP_KMAD_HTML);
	}

	public static void showHelpToolInfoDebugDialog() {
		GraphicEditorAdaptator.getMainFrame().getInfoHistoryDialog().showInfoAbout(KMADEConstant.HELP_KMADE_TITLE_NAME,
	...
	public static void showClipBoardOverView() {
		GraphicEditorAdaptator.getMainFrame().getClipBoardDialog()
				.setVisible(!GraphicEditorAdaptator.getMainFrame().getClipBoardDialog().isVisible());
	}
	...
	private static void setToLocalisation(Locale myLocale) {
		if (!KMADeMain.isTheSameLocale(myLocale)) {
			if (KMADeAdaptator.doSaveProjectBeforeClose(KMADEConstant.WRITE_BEFORE_CHANGE_LOCALE_MESSAGE)) {
				KMADeMain.loadKMADEResourceBundle(myLocale);
				GraphicEditorAdaptator.notifLocalisationModification();
				TaskPropertiesEnhancedEditorAdaptator.notifLocalisationModification();
			}
		} else {
		}
	}
	...
	public static int askToSaveBeforeAction(String message) {
		return JOptionPane.showConfirmDialog(GraphicEditorAdaptator.getMainFrame(), message,
				KMADEConstant.CONFIRMATION_DIALOG_MESSAGE, JOptionPane.YES_NO_CANCEL_OPTION,
