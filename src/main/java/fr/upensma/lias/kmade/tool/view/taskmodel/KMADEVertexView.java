...
public class KMADEVertexView extends VertexView {
	...
	public static final ImageIcon ABSTRACT_TASK_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.ABSTRACT_TASK_IMAGE));

	public static final ImageIcon FEEDBACK_TASK_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.FEEDBACK_TASK_IMAGE));
	...
	public static final ImageIcon INTERACTIF_TASK_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.INTERACTIF_TASK_IMAGE));
	...
	public static final ImageIcon OPTIONAL_ABSTRACT_TASK_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.OPTIONAL_ABSTRACT_TASK_IMAGE));

	public static final ImageIcon OPTIONAL_USER_TASK_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.OPTIONAL_USER_TASK_IMAGE));

	public static final ImageIcon OPTIONAL_INTERACTIF_TASK_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.OPTIONAL_INTERACTIF_TASK_IMAGE));

	public static final ImageIcon OPTIONAL_UNKNOWN_TASK_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.OPTIONAL_UNKNOWN_TASK_IMAGE));

	public static final ImageIcon INTERRUPTIBLE_TASK_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.INTERRUPTIBLE_TASK_IMAGE));
	...
	/** * Icon des �v�nements entrants/sortants** */
	...
	public static final ImageIcon EVENEMENT_OUT_IMAGE_ICON = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.EVENEMENT_OUT_IMAGE));

	public static final ImageIcon EVENEMENT_IN_IMAGE_ICON_DISABLE = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.EVENEMENT_IN_IMAGE_DISABLE));

	public static final ImageIcon EVENEMENT_OUT_IMAGE_ICON_DISABLE = new ImageIcon(
			KMADEVertexView.class.getResource(KMADEConstant.EVENEMENT_OUT_IMAGE_DISABLE));
	...
	protected static final int NUM_LIMIT = 30; // a �t� augment�
	...
	@SuppressWarnings("unused")
	// dimension du rectangle mis � jour mais pas utilis�
	...
	public Rectangle2D getBounds() {
		...
		int hauteur = 10 + fnum.getHeight() + 5 + 32 + fname.getHeight() + 10 + 10 + 15 + 5;

		rectTache = new Rectangle((int) (zoom * temp.getX()), (int) (zoom * temp.getY()), (int) (zoom * width),
				(int) (zoom * hauteur));
	...
	/**
	 * Retourne la longueur d'une t�che ayant pour nom name
	 * 
	 * @param name
	 * @return la longueur
	 */
	...
	/**
	 * Retourne la hauteur d'une t�che
	 * 
	 * @return la hauteur
	 */
	public static int heightView() {
		...
		FontMetrics fname = myPanel.getFontMetrics(KMADEConstant.TASK_NAME_FONT);
		FontMetrics fnum = myPanel.getFontMetrics(KMADEConstant.TASK_NUM_FONT);
		return 10 + fnum.getHeight() + 5 + 32 + fname.getHeight() + 10 + 10 + 15 + 5;
	}
	...
	public class MyVertexRenderer extends VertexRenderer {
		...
		protected void paintTask(Graphics gr) {
			...
			// on d�tecte les incoh�rences pour la d�composition et le nom
			detecteIncoherence();
			...
			f.drawString(numero, (int) ((int) width - (rnum.getWidth() / 2)), (int) rnum.getHeight() + BEGIN_V_GAP);

			rectNum = new Rectangle((int) (zoom * (this.getBounds().getX() + width - (rnum.getWidth() / 2))),
					(int) (zoom * (this.getBounds().getY() + rnum.getHeight())), (int) (zoom * rnum.getWidth()),
			...
			if (!preview) {
				myImage.paintIcon(this, gr, width - IMAGE_32_WIDTH / 2, BEGIN_V_GAP + (int) rnum.getHeight() + V_GAP);
			}
			int currentHeighImage = BEGIN_V_GAP + (int) rnum.getHeight() + V_GAP + IMAGE_32_HEIGH + V_GAP;
			...
			rectNom = new Rectangle((int) (zoom * (this.getBounds().getX() + width - (rn.getWidth() / 2))),
					(int) (zoom * (this.getBounds().getY() + currentHeighImage)), (int) (zoom * rn.getWidth()),
					(int) (zoom * rn.getHeight()));
			...
			f.drawRect((int) (width - (ro.getWidth() / 2)) - H_GAP, currentHeighImage - H_GAP,
					(int) ro.getWidth() + 2 * H_GAP, (int) ro.getHeight() + 3 * V_GAP);

			rectOpe = new Rectangle((int) (zoom * (this.getBounds().getX() + width - (ro.getWidth() / 2) - H_GAP)),
					(int) (zoom * (this.getBounds().getY() + currentHeighImage - H_GAP)),
					(int) (zoom * (ro.getWidth() + 2 * H_GAP)), (int) (zoom * (ro.getHeight() + 3 * V_GAP)));

			if (!coherent) {
				f.setColor(Color.RED);
				f.fillRect((int) (width - (ro.getWidth() / 2)) - H_GAP, currentHeighImage - H_GAP,
						(int) ro.getWidth() + 2 * H_GAP, (int) ro.getHeight() + 3 * V_GAP);
				f.setColor(KMADEConstant.TASK_OPERATOR_COLOR);
			}
			...
			if (graph != null && graph.getScale() >= 1) {
				...
				myImage_evt_in.paintIcon(this, gr, widthg, (int) rnum.getHeight());
				// la tooltips
				rectEvenementIn = new Rectangle((int) (zoom * (this.getBounds().getX() + widthg)),
						(int) (zoom * (this.getBounds().getY() + myImage_evt_in.getIconHeight())),
						(int) (zoom * myImage_evt_in.getIconWidth()), (int) (zoom * myImage_evt_in.getIconHeight()));

				// L'ic�ne PRE
				String s_pre = KMADEConstant.VERTEX_PRECONDITION;

				if (!myGraphCell.getTask().getPreExpression().getProtoTaskConditionExpression().getValue()
						.getDescription().equals("")
						|| !myGraphCell.getTask().getPreExpression().getFormalText().equals("true")
						|| (myGraphCell.getTask().getPreExpression().getDescription() != null
								&& !myGraphCell.getTask().getPreExpression().getDescription().equals(""))) {
					f.setColor(Color.BLACK);
				} else {
				...
				f.drawString(s_pre, widthg, (int) rpre.getHeight() + BEGIN_V_GAP + myImage_evt_in.getIconHeight());

				rectPre = new Rectangle((int) ((this.getBounds().getX() + widthg) * zoom),
						(int) ((this.getBounds().getY() + rpre.getHeight() + myImage_evt_in.getIconHeight()) * zoom),
						(int) (zoom * rpre.getWidth()), (int) (zoom * rpre.getHeight()));
				...
				f.drawString(s_opt, widthg,
						(int) ropt.getHeight() + BEGIN_V_GAP + myImage_evt_in.getIconHeight() + (int) rpre.getHeight());

				rectOpt = new Rectangle((int) (zoom * (this.getBounds().getX() + widthg)),
						(int) ((this.getBounds().getY() + ropt.getHeight() + myImage_evt_in.getIconHeight()
								+ rpre.getHeight()) * zoom),
						(int) (zoom * ropt.getWidth()), (int) (zoom * ropt.getHeight()));
				...
				f.drawString(s_int, widthg, (int) ropt.getHeight() + BEGIN_V_GAP + myImage_evt_in.getIconHeight()
						+ (int) rpre.getHeight() + (int) rint.getHeight());
				rectInter = new Rectangle((int) (zoom * (this.getBounds().getX() + widthg)),
						(int) (zoom * (this.getBounds().getY() + ropt.getHeight() + myImage_evt_in.getIconHeight()
								+ rpre.getHeight() + rint.getHeight())),
						(int) (zoom * rint.getWidth()), (int) (zoom * rint.getHeight()));
				...
				if (myGraphCell.getTask().canHaveActor()) {
					...
					if (myGraphCell.getTask().getActors() != null && myGraphCell.getTask().getActors().size() != 0) {
						f.setColor(Color.BLACK);
					} else {
						f.setColor(Color.LIGHT_GRAY);
					}

					f.drawString(s_act, widthg,
							(int) ropt.getHeight() + BEGIN_V_GAP + myImage_evt_in.getIconHeight()
									+ (int) rpre.getHeight() + (int) rint.getHeight() + (int) ract.getHeight()
									+ IMAGE_32_HEIGH + (int) ract.getHeight());
					f.setColor(Color.BLACK);
					// la tooltip
					rectAct = new Rectangle((int) (zoom * (this.getBounds().getX() + widthg)),
							(int) (zoom * (this.getBounds().getMinY() + (int) rpre.getHeight()
									+ myImage_evt_in.getIconHeight() + (int) ropt.getHeight() + (int) rint.getHeight()
									+ (int) rn.getHeight() + (int) ro.getHeight() + (int) ract.getHeight()
									+ (int) ract.getHeight() + 5)),
							(int) (zoom * ract.getWidth()), (int) (zoom * ract.getHeight()));
				}
				...
				if (!myGraphCell.getTask().getIterExpression().getProtoTaskConditionExpression().getValue()
						.getDescription().equals("") ||

						(!myGraphCell.getTask().getIterExpression().getFormalText().equals("[1]"))
						|| (myGraphCell.getTask().getIterExpression().getDescription() != null
								&& !myGraphCell.getTask().getIterExpression().getDescription().equals(""))) {
					f.setColor(Color.BLACK);
				} else {
					f.setColor(Color.LIGHT_GRAY);
				}

				f.drawString(s_rep, widthd - (int) rrep.getWidth(),
						(int) rpre.getHeight() + BEGIN_V_GAP + myImage_evt_in.getIconHeight() - V_GAP);
				// la tooltip
				rectRep = new Rectangle((int) (zoom * (this.getBounds().getX() + widthd - (int) rrep.getWidth())),
						(int) (zoom
								* (this.getBounds().getY() + (int) rpre.getHeight() + myImage_evt_in.getIconHeight())),
						(int) (zoom * rrep.getWidth()), (int) (zoom * rrep.getHeight()));
				...
				if (myGraphCell.getTask().getEffetsDeBordExpression().getFormalText().equals("void")
						&& (myGraphCell.getTask().getEffetsDeBordExpression().getDescription() == null
								|| myGraphCell.getTask().getEffetsDeBordExpression().getDescription().equals(""))) {
					f.setColor(Color.LIGHT_GRAY);
				} else {
					f.setColor(Color.BLACK);
				}
				...
				f.drawString(s_post, widthd - (int) rpost.getWidth(), (int) rrep.getHeight() + BEGIN_V_GAP
						+ myImage_evt_in.getIconHeight() + (int) rpost.getHeight() - V_GAP);

				// la tooltip
				rectPost = new Rectangle((int) (zoom * (this.getBounds().getX() + widthd - (int) rpost.getWidth())),
						(int) (zoom * (this.getBounds().getY() + myImage_evt_in.getIconHeight() + (int) rrep.getHeight()
								+ (int) rpost.getHeight())),
						(int) (zoom * rpost.getWidth()), (int) (zoom * rpost.getHeight()));

				// événement
				ImageIcon myImage_evt_out;
				if (myGraphCell.getTask().getEvents() != null && myGraphCell.getTask().getEvents().size() != 0) {
					myImage_evt_out = KMADEVertexView.EVENEMENT_OUT_IMAGE_ICON;
				} else {
					myImage_evt_out = KMADEVertexView.EVENEMENT_OUT_IMAGE_ICON_DISABLE;

				}
				myImage_evt_out.paintIcon(this, gr, widthd - myImage_evt_out.getIconWidth(), (int) rpre.getHeight()
						+ BEGIN_V_GAP + myImage_evt_in.getIconHeight() + (int) rpost.getHeight());
				// la tooltip
				rectEvenementOut = new Rectangle(
						(int) (zoom * (this.getBounds().getX() + widthd - myImage_evt_out.getIconWidth())),
						(int) (zoom * (this.getBounds().getY() + myImage_evt_in.getIconHeight() + (int) rrep.getHeight()
								+ (int) rpost.getHeight() + myImage_evt_out.getIconHeight())),
						(int) (zoom * myImage_evt_out.getIconWidth()), (int) (zoom * myImage_evt_out.getIconHeight()));
				...
				f.setColor(Color.LIGHT_GRAY);
				f.drawString(s_obj, widthd - (int) robj.getWidth(),
						(int) ropt.getHeight() + BEGIN_V_GAP + myImage_evt_in.getIconHeight() + (int) rpre.getHeight()
								+ (int) rint.getHeight() + (int) robj.getHeight() + IMAGE_32_HEIGH
								+ (int) robj.getHeight());
				f.setColor(Color.BLACK);
				// la tooltip
				rectDesc = new Rectangle((int) (zoom * (this.getBounds().getX() + widthd - (int) robj.getWidth())),
						(int) (zoom * (this.getBounds().getMinY() + (int) rpre.getHeight()
								+ myImage_evt_in.getIconHeight() + (int) ropt.getHeight() + (int) rint.getHeight()))
								+ (int) (zoom * (rn.getHeight() + (int) ro.getHeight() + (int) robj.getHeight()
										+ (int) robj.getHeight() + 5)),
						(int) (zoom * robj.getWidth()), (int) (zoom * robj.getHeight()));
		...
		protected void paintLabelCaract(Graphics2D f, KMADEDefaultGraphCell myCell) {
			if (GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().getEditorsToolBar()
					.isLabelColorSelected()) {
				if (myCell.getTask().getLabel() != null) {
					if (myCell.getTask().getLabel().isColorVisible()) {
						...
						Color colorWithTransparency = new Color(taskColor.getRed(), taskColor.getGreen(),
		...
		protected void paintNumCaract(Graphics2D f, int widthString, String numString, double num) {
		...
		public void paint(Graphics gr) {
			...
			if (!myGraphCell.isMotherPortNoEdged()
					&& (myGraph == GraphicEditorAdaptator.getTaskModelPanel().getJGraph())) {
				...
				if (!preview) {
					/** icone repositionn�e * */
					myImageIcon.paintIcon(this, gr,
							(int) this.getBounds().getWidth() - H_GAP - myImageIcon.getIconWidth(), V_GAP);
				}
			}
		...
		public void detecteIncoherence() {
			...
			if (myGraphCell.getTask().getMother() != null || (myGraphCell.getTask().getChildren() != null
					&& myGraphCell.getTask().getChildren().size() != 0)) {
				tacheIsolee = false;
			} else {
				tacheIsolee = true;
			}
			...
			if (fils == null || fils.size() == 0) {
				...
				if (fils.size() == 1) {
					// une t�che ne peut pas avoir une seule sous t�che
					coherent = false;
					filsCoherent = false;
	...
	class MyCellHandle implements CellHandle {
		...
		public void mousePressed(MouseEvent event) {
			...
			if (depart != null) {
				...
				distanceX = Math.abs(event.getX() - graph.getScale() * depart.getX());

				distanceY = Math.abs(event.getY() - graph.getScale() * depart.getY());
			...
			int rx = (int) (rec.getX() + rec.getWidth() - H_GAP - KMADEVertexView.EXPAND_TASK_IMAGE_ICON.getIconWidth()
					- tolerance);
		...
		public void mouseDragged(MouseEvent event) {
			...
			// on trouve la cellule la plus pr�s
			cellRef = trouvePlusPres();

			if (ancienY != Double.POSITIVE_INFINITY) {

				graph.getGraphics().clearRect(0, (int) (ancienY), (int) (graph.getWidth()), (int) (1));
			...
			if (cellRef != null) {
				...
				if (haut) {
					// on dessine le trait sur le haut de la cellule de
					// r�f�rence
					((Graphics2D) graph.getGraphics()).drawLine(0, (int) (zoom * (cellRef.getBounds().getY())),
							(int) (graph.getWidth()), (int) (zoom * (cellRef.getBounds().getY())));
					ancienY = (int) (zoom * (cellRef.getBounds().getY()));
				} else {

					// on dessine le trait sur le bas de la cellule de r�f�rence
					((Graphics2D) graph.getGraphics()).drawLine(0,
							(int) ((zoom * (cellRef.getBounds().getY() + cellRef.getBounds().getHeight()))),
							(int) (graph.getWidth()),
							(int) (zoom * ((cellRef.getBounds().getY() + cellRef.getBounds().getHeight()))));
					ancienY = (int) (zoom * (cellRef.getBounds().getY() + cellRef.getBounds().getHeight()));
				}
			}
		...
		public void mouseReleased(MouseEvent event) {
			...
			// on efface l'�ventuel trait
			if (ancienY != Double.POSITIVE_INFINITY) {
				graph.getGraphics().clearRect(0, (int) (zoom * ancienY), (int) (graph.getWidth()), 0);
				graph.repaint();
			}
			...
			if (plusPres != null) {
				...
				// on aimante
				Rectangle2D newBounds = new Rectangle((int) cell.getBounds().getX(), (int) plusPres.getBounds().getY(),
						(int) cell.getBounds().getWidth(), (int) cell.getBounds().getHeight());
			...
			for (int i = 0; i < tabCellules.length; i++) {
				...
				if (tmp != select && !graph.getModel().isEdge(tmp) && !graph.getModel().isPort(tmp)) {
					CellView cellRef = tabCellules[i];

					intersect = intersect || cellRef.getBounds().contains(cell.getBounds().getCenterX(),
							cell.getBounds().getCenterY());

					if (intersect) {
						// on remet aux coordonn�es de d�part
		...
		public void mouseClicked(MouseEvent event) {
			...
			// on regarde si on a double cliqu� dans un des rectangles d�finis
			...
			if (rectOpt != null && rectOpt.contains(event.getPoint())) {
				((KMADEDefaultGraphCell) cell.getCell()).getTask()
						.setOptional(!((KMADEDefaultGraphCell) cell.getCell()).getTask().getFacultatif());

			} else if (rectInter != null && rectInter.contains(event.getPoint())) {
				((KMADEDefaultGraphCell) cell.getCell()).getTask()
						.setInterruptible(!((KMADEDefaultGraphCell) cell.getCell()).getTask().getInterruptible());

			} else if (rectNom != null && rectNom.contains(event.getPoint())) {

				((MyBasicGraphUI) graph.getUI()).startEditing(cell.getCell(), event);
			} else if (rectAct != null && rectAct.contains(event.getPoint())) {
				// on rajoute un test pour savoir si la tache peut avoir des
				// acteurs ou non
				KMADEDefaultGraphCell myGraphCell = (KMADEDefaultGraphCell) KMADEVertexView.this.getCell();
				if (myGraphCell.getTask().canHaveActor()) {
					KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getProprieteTache().setActorList();
				}

			} else if (rectPre != null && rectPre.contains(event.getPoint())) {
				KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getProprieteTache().setPrecondition();
			} else if (rectPost != null && rectPost.contains(event.getPoint())) {
				KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getProprieteTache().setEffetsDeBord();
			} else if (rectRep != null && rectRep.contains(event.getPoint())) {
				KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getProprieteTache().setIteration();
			} else if (rectEvenementOut != null && rectEvenementOut.contains(event.getPoint())) {
				KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getProprieteTache().setFiredEvents();
			} else if (rectEvenementIn != null && rectEvenementIn.contains(event.getPoint())) {
				KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getProprieteTache().setEventDecl();
			} else if (rectDesc != null && rectDesc.contains(event.getPoint())) {
				KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getProprieteTache().setObservation();
		...
		public CellView trouvePlusPres() {
			...
			for (int i = 0; i < tabCellules.length; i++) {
				...
				if (!graph.getModel().isEdge(enCours) && !(graph.getModel().isPort(enCours))) {
					if (select != enCours) {
