...
public class KMADEVertexView extends VertexView {
	...
	FontMetrics fname = myPanel
		.getFontMetrics(KMADEConstant.TASK_NAME_FONT);
	FontMetrics fnum = myPanel.getFontMetrics(KMADEConstant.TASK_NUM_FONT);
	return 10 + fnum.getHeight() + 5 + 32 + fname.getHeight() + 10 + 10
		+ 15 + 5;
    }
	...
	protected void paintTask(Graphics gr) {
	    ...
	    rectNom = new Rectangle((int) (zoom * (this.getBounds().getX()
		    ...
		    .getBounds().getY() + currentHeighImage)),
		    (int) (zoom * rn.getWidth()), (int) (zoom * rn.getHeight()));
		...
		myImage_evt_in.paintIcon(this, gr, widthg,
			(int) rnum.getHeight());
		// la tooltips
		...
		rectOpt = new Rectangle(
			...
			(int) ((this.getBounds().getY() + ropt.getHeight()
				...
				.getHeight()) * zoom),
			(int) (zoom * ropt.getWidth()),
			(int) (zoom * ropt.getHeight()));
		...
		rectInter = new Rectangle((int) (zoom * (this.getBounds()
			...
			(int) (zoom * rint.getWidth()),
			(int) (zoom * rint.getHeight()));
		...
		if (myGraphCell.getTask().canHaveActor()) {
		    ...
		    f.setColor(Color.BLACK);
		    // la tooltip
		    rectAct = new Rectangle((int) (zoom * (this.getBounds()
			    .getX() + widthg)),
			    ...
			    (int) (zoom * ract.getWidth()),
			    (int) (zoom * ract.getHeight()));
		}
		...
		f.drawString(
			...
			(int) rpre.getHeight() + BEGIN_V_GAP
				+ myImage_evt_in.getIconHeight() - V_GAP);
		// la tooltip
		...
		rectEvenementOut = new Rectangle((int) (zoom * (this
			.getBounds().getX() + widthd - myImage_evt_out
			.getIconWidth())),
			(int) (zoom * (this.getBounds().getY()
				+ myImage_evt_in.getIconHeight()
				+ (int) rrep.getHeight()
		...
		f.drawString(
			s_obj,
			widthd - (int) robj.getWidth(),
			(int) ropt.getHeight() + BEGIN_V_GAP
				+ myImage_evt_in.getIconHeight()
				+ (int) rpre.getHeight()
				+ (int) rint.getHeight()
				+ (int) robj.getHeight() + IMAGE_32_HEIGH
				+ (int) robj.getHeight());
		f.setColor(Color.BLACK);
		// la tooltip
		rectDesc = new Rectangle((int) (zoom * (this.getBounds().getX()
			...
			(int) (zoom * robj.getWidth()),
			(int) (zoom * robj.getHeight()));
	...
	protected void paintLabelCaract(Graphics2D f,
	    ...
	    if (GraphicEditorAdaptator.getMainFrame().getApplicationToolBar()
		    .getEditorsToolBar().isLabelColorSelected()) {
	...
	public void paint(Graphics gr) {
	    ...
	    if (GraphicEditorAdaptator.getMainFrame().getApplicationToolBar()
		    .getEditorsToolBar().isLabelSelected()) {
		...
		if (!preview) {
		    ...
		    myImageIcon.paintIcon(this, gr, (int) this.getBounds()
			    .getWidth() - H_GAP - myImageIcon.getIconWidth(),
			    V_GAP);
		}
	    }
