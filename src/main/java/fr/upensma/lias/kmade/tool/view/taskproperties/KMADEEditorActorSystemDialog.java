...
import fr.upensma.lias.kmade.tool.viewadaptator.MaterielAdaptator;

/**
 * @author Mickael BARON
 */
public class KMADEEditorActorSystemDialog extends JPropertiesEditorDialog {
	...
	public KMADEEditorActorSystemDialog() {
		super();
		this.setModal(false);
		this.setTitle(KMADEConstant.ACTORSYSTEM_CREATE_TITLE_NAME);
		JScrollPane myScrollPane = new JScrollPane();
		String[] columnNames = { KMADEConstant.ACTORSYSTEM_NAME_TABLE, KMADEConstant.ACTORSYSTEM_EXPERIENCE_TABLE,
				KMADEConstant.ACTORSYSTEM_COMPETENCE_TABLE };
		myTableListModel = new MyTableListModel(columnNames, KMADEConstant.ACTORSYSTEM_NEW_ROW_TABLE);
		...
		panelCenter.add(MaterielAdaptator.getMaterielPanel());
		// myListTable.getMyTable().getColumnModel().getColumn(0).setCellEditor(new
		// MyCustomListTableCellEditor());
		// myListTable.setCellEditor(0,
		// "List",Experience.getNameLocaleExperience());
	...
	public void addNewActorSystem(String name, String experience, String competence, String oidActorSystem) {
		Object[] myNewRow = { name, Experience.getEnumereIntoLocaleExperience(experience), competence, oidActorSystem,
				false };
	...
	public void showPropertiesEditor(DefaultPropertiesTableModel refModel, int row) {
		MaterielAdaptator.refreshReadMaterielTable();
		listOfUser = MaterielAdaptator.getMaterielsName();
		...
		for (int i = 0; i < myEventList.size(); i++) {
			ActorSystem myRef = (ActorSystem) myEventList.get(i);
			Object[] myNew = { myRef.getName(),
					Experience.getEnumereIntoLocaleExperience(myRef.getExperience().getValue()), myRef.getCompetence(),
					myRef.getOid().get(), false };
	...
	class MyCustomListTableCellEditor extends AbstractCellEditor implements TableCellEditor {
		...
		public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row,
				int column) {
			// enlever les users d�j� enregistr�
	...
	class MyTableListModel extends DefaultListTableModel {
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			if (listOfUser.length == 0 && columnIndex == 0) {
				return KMADEConstant.ACTORSYSTEM_NONEW_ROW_TABLE;
			} else {
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
			...
			if (rowIndex + 1 == this.getRowCount()) {
				...
				String[] myValue = ActorSystemAdaptator.addNewActorSystem(value);
				if (myValue == null) {
					return;
				} else {
					KMADEEditorActorSystemDialog.this.addNewActorSystem(myValue[0], myValue[1], myValue[2], myValue[3]);
				...
				case 0: {
					...
					if (((String) this.getValueAt(rowIndex, columnIndex)).equals(value)) {
						...
						String newOid = ActorSystemAdaptator.setOldActorSystemSelectedTask(oidRow, value);
						// s'il n'y a pas eu de modification de l'oid de
						// l'acteur on ne fait rien
						if (newOid == null) {
							return;
						} else {
							// si il y a eu une mise � jour de l'acteur : l'oid
							// a changer,
							// il faut donc le mettre � jour !
				...
				case 2: {
					ActorSystemAdaptator.setActorSystemCompetence(oidRow, value);
					break;
				}
				}
	...
	class MyKeyListener extends KeyAdapter {
		public void keyPressed(KeyEvent ke) {
			if (ke.getKeyCode() == KeyEvent.VK_DELETE) {
				...
				boolean state = GraphicEditorAdaptator.removeSelectedItem("ActorSystem", r,
						myTableListModel.getListTableData(), KMADEEditorActorSystemDialog.this,
						KMADEConstant.ACTORSYSTEM_REMOVE_NAME_TITLE);
