...
public class KMADEEditorActorSystemDialog extends JPropertiesEditorDialog {
	...
	String[] columnNames = { KMADEConstant.ACTORSYSTEM_NAME_TABLE,
		KMADEConstant.ACTORSYSTEM_EXPERIENCE_TABLE,
		KMADEConstant.ACTORSYSTEM_COMPETENCE_TABLE };
	myTableListModel = new MyTableListModel(columnNames,
		KMADEConstant.ACTORSYSTEM_NEW_ROW_TABLE);
	...
	myListTable.setCellEditor(1, "List",
		Experience.getNameLocaleExperience());
    ...
    public void addNewActorSystem(String name, String experience,
	    String competence, String oidActorSystem) {
	Object[] myNewRow = { name,
		Experience.getEnumereIntoLocaleExperience(experience),
		competence, oidActorSystem, false };
	...
	for (int i = 0; i < myEventList.size(); i++) {
	    ActeurSysteme myRef = (ActeurSysteme) myEventList.get(i);
	    Object[] myNew = {
		    myRef.getName(),
		    Experience.getEnumereIntoLocaleExperience(myRef
			    .getExperience().getValue()),
		    myRef.getCompetence(), myRef.getOid().get(), false };
	...
	public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
		...
		String[] myValue = ActorSystemAdaptator
			.addNewActorSystem(value);
		if (myValue == null) {
		    return;
		} else {
		    KMADEEditorActorSystemDialog.this.addNewActorSystem(
			    myValue[0], myValue[1], myValue[2], myValue[3]);
		...
		case 0: {
		    ...
		    if (((String) this.getValueAt(rowIndex, columnIndex))
			    .equals(value)) {
			return;
		    } else {
			String newOid = ActorSystemAdaptator
				.setOldActorSystemSelectedTask(oidRow, value);
		...
		case 2: {
		    ActorSystemAdaptator
			    .setActorSystemCompetence(oidRow, value);
		    break;
		}
		}
	...
	public void keyPressed(KeyEvent ke) {
		...
		boolean state = GraphicEditorAdaptator.removeSelectedItem(
			"ActorSystem", r, myTableListModel.getListTableData(),
			KMADEEditorActorSystemDialog.this,
			KMADEConstant.ACTORSYSTEM_REMOVE_NAME_TITLE);

		if (state) {
		    for (int i = 0; i < r.length; i++)
			myListTable.getListSelectionModel()
