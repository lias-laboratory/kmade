...
public final class PreconditionAdaptator {
	...
	public static void initPrecondition() {
		...
		if (myCurrentTask.getPreExpression().getNodeExpression() != null) {
			...
			KMADEEditorPrePostIterDialog.getPreconditionPanel()
					.disableEvaluateControl(KMADEConstant.PRECONDITION_ERROR_MESSAGE);
		}
	}

	public static void modifiedExpression() {
		KMADEEditorPrePostIterDialog.getPreconditionPanel()
				.disableEvaluateControl(KMADEConstant.NOT_YET_CHECKED_PRECONDITION_MESSAGE);
	}
	...
	public static void evaluatePrecondition(String text) {
		...
		try {
			...
			KMADEEditorPrePostIterDialog.getPreconditionPanel()
					.giveResult(myCurrentTask.getPreExpression().getNodeExpression().getNodeValue().toString() + " =");
			...
			KMADEHistoryMessageManager.printlnMessage(KMADEConstant.VALUE_MESSAGE + " = "
					+ myCurrentTask.getPreExpression().getNodeExpression().getNodeValue());
	...
	private static void checkPreconditionBuilder(String s) {
		Task myCurrentTask = GraphicEditorAdaptator.getSelectedGraphicTask().getTask();
		myCurrentTask.getPreExpression().setFormalText(s);
	...
	public static void setPrecondition() {
		ExpressPrecondition.setPreconditionDescription(GraphicEditorAdaptator.getSelectedGraphicTask().getTask(),
				KMADEEditorPrePostIterDialog.getPreconditionPanel().getDescriptionArea());
