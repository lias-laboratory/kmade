...
public final class PreconditionAdaptator {
	...
	try {
	    ...
	    KMADEEditorPrePostIterDialog.getPreconditionPanel().giveResult(
		    myCurrentTask.getPreExpression().getNodeExpression()
			    ...
			    + " =");
	    KMADEHistoryMessageManager
		    .printlnMessage(KMADEConstant.PRECONDITION_EVAL_OK_MESSAGE);
	    KMADEHistoryMessageManager
		    .printlnMessage(KMADEConstant.VALUE_MESSAGE
			    + " = "
			    + myCurrentTask.getPreExpression()
				    .getNodeExpression().getNodeValue());
	} catch (SemanticErrorException e) {
	    KMADEHistoryMessageManager
		    .printlnMessage(KMADEConstant.PRECONDITION_EVAL_NO_OK_MESSAGE);
	    KMADEHistoryMessageManager
	...
	try {
		...
		KMADEHistoryMessageManager
			.printlnMessage(KMADEConstant.PRECONDITION_OK_MESSAGE);
	    }
	} catch (SemanticException e) {
	    KMADEHistoryMessageManager
		    .printlnMessage(KMADEConstant.PRECONDITION_NO_OK_MESSAGE);
	    KMADEHistoryMessageManager
		    .printlnMessage(KMADEConstant.CAUSE_MESSAGE + " : "
			    + KMADEConstant.SEMANTICAL_ERROR_MESSAGE + " , "
			    + e.getMessage());
