	...
	public KMADEMainFrame() {
		...
		panelEmpty = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				int gapw = 20;
				int gaph = 20;
				g.drawImage(LISI_LOGO.getImage(), w - (100 + gapw), h - 73 - gaph, 100, 41, this);// 50,100
				g.drawImage(INRIA_LOGO.getImage(), gapw, h - 22 - 73 - gaph - gaph, 100, 22, this);// 50,100
				g.drawImage(MERLIN_LOGO.getImage(), gapw, h - 73 - gaph, 100, 73, this);// 90,100
			}
		...
		panelInfo.add(BorderLayout.NORTH, new JLabel(
				new ImageIcon(KMADEMainFrame.class.getResource(KMADEConstant.ICON_LOGO_IMAGE)), JLabel.CENTER));
		...
		JLabel newProjectLabel = new JLabel(
				APPLICATION_TOOL_BAR.getProjectAction().getValue(AbstractAction.NAME).toString(), JLabel.CENTER);
		...
		JLabel openLabel = new JLabel(APPLICATION_TOOL_BAR.getLoadAction().getValue(AbstractAction.NAME).toString(),
				JLabel.CENTER);
		...
		// //////////rajout�/////////////////////
		PROJECT_PANEL.getTaskDescriptionPanel().getJSplitPaneV().setDividerLocation(this.getWidth());
		jSplitPaneBH.setDividerLocation(this.getHeight());

		locationDividerEstAvant = PROJECT_PANEL.getTaskDescriptionPanel().getJSplitPaneV().getDividerLocation();
