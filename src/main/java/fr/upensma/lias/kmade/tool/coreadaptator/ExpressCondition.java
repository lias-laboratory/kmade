...
public final class ExpressCondition {
	...
	ProtoTaskCondition m = (ProtoTaskCondition) InterfaceExpressJava
		.prendre(new Oid(oid));
	...
	for (ProtoTaskCondition protoTaskCondition : conditions) {
		...
		if (t.getPreExpression().getProtoTaskConditionExpression()
			.getValue().equals(protoTaskCondition)
			|| (t.getIterExpression()
				.getProtoTaskConditionExpression().getValue()
				.equals(protoTaskCondition))) {
	...
	if (!in) {
	    ProtoTaskCondition a = (ProtoTaskCondition) InterfaceExpressJava
		    .prendre(new Oid(oidAct));
	    a.delete();
	} else {
	}
    ...
    public static String setConditionDefaultValue(String oid,
	    String defaultValue) {
	ProtoTaskCondition m = (ProtoTaskCondition) InterfaceExpressJava
		.prendre(new Oid(oid));
	...
	ProtoTaskCondition c = (ProtoTaskCondition) InterfaceExpressJava
		.prendre(new Oid(oidCondition));
