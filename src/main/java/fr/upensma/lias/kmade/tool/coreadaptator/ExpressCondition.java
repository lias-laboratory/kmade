...
public final class ExpressCondition {
	...
	public static ArrayList<ProtoTaskCondition> getConditionsIntoTasks() {
		...
		for (ProtoTaskCondition protoTaskCondition : conditions) {
			for (Task t : tasks) {
				if (t.getPreExpression().getProtoTaskConditionExpression().getValue().equals(protoTaskCondition) || (t
						.getIterExpression().getProtoTaskConditionExpression().getValue().equals(protoTaskCondition))) {
