...
public class PROTOExecution {
	...
	List<Tache> root = ExpressTask.getRootTasks();
	if (root.size() == 1) {
    ...
    public static void setCurrentTask(Tache t, boolean historic,
	    HashMap<String, ChoiceEnum> map) {
	int reponse = 42;
	/*
	 * if(t.getPreExpression()!= null &&
	 * t.getPreExpression().getDescription()!=null &&
	...
	if (currentTask.getMotherTask() != null)
	    displayPrecondition = displayCondition(currentTask
		    .getPreExpression().getDescription());

	// update displayed task
	// update information
	GraphicEditorAdaptator.getMainFrame().getPrototypeDialog()
		.setCurrentTask(currentTask, displayPrecondition);
	GraphicEditorAdaptator.getMainFrame().getPrototypeDialog()
	...
	case PASSIVE:
	    ...
	    KMADEHistoryMessageManager
		    .printlnError(KMADEConstant.PROTOTYPING_TOOL_PASSIVE_INACTIVE_ERROR);
	    return;
	case ACTIVE:
	...
	case WAITEND:
	    ...
	    GraphicEditorAdaptator.getMainFrame().getPrototypeDialog()
		    .setEnabledEnd(currentTask, true);
	    iteration(map);
	...
	if ((currentTask.getIteExpression().getDescription() != null)
		&& !currentTask.getIteExpression().getDescription().equals("")
		&& (map != null && map.get(currentTask.getIteExpression()
			.getDescription()) == ChoiceEnum.vrai)) {
	    GraphicEditorAdaptator.getMainFrame().getPrototypeDialog()
		    .setIterationEnabled(currentTask, true, ChoiceEnum.vrai);
	} else {
	    if ((currentTask.getIteExpression().getDescription() == null)
		    || currentTask.getIteExpression().getDescription()
			    .equals("")) {
		GraphicEditorAdaptator
			.getMainFrame()
			.getPrototypeDialog()
			.setIterationEnabled(currentTask, false,
				ChoiceEnum.indeterminée);
	    } else {
		ChoiceEnum choiceE;
		if (map == null
			|| map.get(currentTask.getIteExpression()
				.getDescription()) == null) {
		    choiceE = ChoiceEnum.indeterminée;
		} else {
		    choiceE = map.get(currentTask.getIteExpression()
			    .getDescription());
		}
		GraphicEditorAdaptator.getMainFrame().getPrototypeDialog()
			.setIterationEnabled(currentTask, true, choiceE);
	    }
	}
	...
	if (currentTask.equals(t)) {
	    ...
	    KMADEHistoryMessageManager
		    .printlnError(KMADEConstant.PROTOTYPING_TOOL_END_ERROR
			    + " : " + t.getName());
	}
    }
	...
	for (Tache t : currentTask.getFils()) {
	    String displayCondition = displayCondition(t.getPreExpression()
		    .getDescription());
	    if (t.getStateExecution() == StateExecution.FINISHED) {
		GraphicEditorAdaptator
			.getMainFrame()
			.getPrototypeDialog()
			.setExecutableTask(t, true, false, 0, null, null,
				displayCondition);
	    } else {
		...
		List<String> list = preonditionEvaluation(t.getPreExpression()
			.getDescription());
		if (list != null) {
			...
			GraphicEditorAdaptator
				.getMainFrame()
				.getPrototypeDialog()
				.setExecutableTask(t, false, (true && !choice),
					0, primary, secondary, displayCondition);
		    else
			GraphicEditorAdaptator
				.getMainFrame()
				.getPrototypeDialog()
				.setExecutableTask(t, false, false, 0, primary,
					secondary, displayCondition);
		} else {
		    GraphicEditorAdaptator
			    .getMainFrame()
			    .getPrototypeDialog()
			    .setExecutableTask(t, false, (true && !choice), 0,
				    null, null, displayCondition);
		}
	    }
	}
	if (choice) {
	...
	for (Tache t : currentTask.getFils()) {
		...
		GraphicEditorAdaptator
			.getMainFrame()
			.getPrototypeDialog()
			.setExecutableTask(t, true, false, number++, null,
				null, displayCondition);
	    } else {

		// si la tâche n'est pas terminée il faut regarder si la
		// dernière active lançeable a été trouvée
		if (futurActiveFound
			|| currentTask.getFils().indexOf(lastfinished) > currentTask
				.getFils().indexOf(t)) {
		    // afficher la tâche comme non éxécutable
		    GraphicEditorAdaptator
			    .getMainFrame()
			    .getPrototypeDialog()
			    .setExecutableTask(t, false, false, number++, null,
				    null, displayCondition);
		    ...
		    HashMap<String, ChoiceEnum> secondary = new HashMap<String, ChoiceEnum>();
		    HashMap<String, ChoiceEnum> primary = new HashMap<String, ChoiceEnum>();
		    List<String> list = preonditionEvaluation(t
			    .getPreExpression().getDescription());
		    if (list != null) {
			primary.put(list.get(0), map.get(list.get(0)));
			System.out.println("map get" + map.get(list.get(0)));
			...
			if (map.get(list.get(0)) == ChoiceEnum.vrai) {
			    GraphicEditorAdaptator
				    .getMainFrame()
				    .getPrototypeDialog()
				    .setExecutableTask(t, false, true,
			    ...
			    GraphicEditorAdaptator
				    .getMainFrame()
				    .getPrototypeDialog()
				    .setExecutableTask(t, false, false,
					    number++, primary, secondary,
					    displayCondition);
			}
		    } else {
			GraphicEditorAdaptator
				.getMainFrame()
				.getPrototypeDialog()
				.setExecutableTask(t, false, true, number++,
					null, null, displayCondition);
		    }
	...
	for (Tache t : currentTask.getFils()) {
		...
		if (list != null) {
			...
			GraphicEditorAdaptator
				.getMainFrame()
				.getPrototypeDialog()
				.setExecutableTask(t, false, true, 0, primary,
					secondary, displayCondition);
		    } else {
			GraphicEditorAdaptator
				.getMainFrame()
		    ...
		    GraphicEditorAdaptator
			    .getMainFrame()
			    .getPrototypeDialog()
			    .setExecutableTask(t, false, true, 0, null, null,
				    displayCondition);
		}
	...
	String delim = "\\^";
	String[] tokens = tache.getPreExpression().getDescription()
		.split(delim);
	expressions.clear();
	map.clear();
	...
	boolean rema = true;
	StringTokenizer myST = new StringTokenizer(formalDecomp[0], "[//?]",
		true);
	...
	// differentes partie de l'expression à afficher
	...
	while (myST.hasMoreTokens()) {
		...
		while (!operator.get(operator.size() - 1).equals("(")) {
			...
			if (value.size() > 0) {
			    myExpression = new ExpressionTree("!",
				    new ExpressionTree(
					    value.get(value.size() - 1), null,
					    null), null);
			...
			if (myExpression == null) {
			    myExpression = new ExpressionTree(
				    expressions.get(Integer.parseInt(element)),
				    null, null);
			}
			myExpression = new ExpressionTree("+", myExpression,
				new ExpressionTree(value.get(value.size() - 1),
					null, null));
			operator.remove(operator.size() - 1);
			value.remove(value.size() - 1);
			...
			if (myExpression == null) {
			    myExpression = new ExpressionTree(value.get(value
				    .size() - 1), null, null);
			    value.remove(value.size() - 1);
			}
			myExpression = new ExpressionTree("*", myExpression,
		...
		if (first) {
		    myExpression = new ExpressionTree(expressions.get(Integer
			    .parseInt(element)), null, null);
		    first = false;
		} else {
		    value.add(expressions.get(Integer.parseInt(element)));
	...
	if ((expression == null) || expression.equals(""))
	    ...
	    // il faut donner l'ensemble des expressions à afficher avec leurs
	    // valeurs
