...
public class PROTOExecution {
    ...
    public static void setCurrentTask(Task t, boolean historic,
	...
	case FINISHED:
	    ...
	    if (currentTask.getMother() != null
		    && !currentTask.equals(startTask)) {
	...
	if ((currentTask.getIterExpression().getDescription() != null)
		...
		&& (map != null && map.get(currentTask.getIterExpression()
			.getDescription()) == ChoiceEnum.vrai)) {
	    GraphicEditorAdaptator.getMainFrame().getPrototypeDialog()
		    .setIterationEnabled(currentTask, true, ChoiceEnum.vrai);
	} else {
	    if ((currentTask.getIterExpression().getDescription() == null)
		    || currentTask.getIterExpression().getDescription()
			    .equals("")) {
		...
		ChoiceEnum choiceE;
		if (map == null
			|| map.get(currentTask.getIterExpression()
				.getDescription()) == null) {
		    choiceE = ChoiceEnum.indeterminee;
		} else {
		    choiceE = map.get(currentTask.getIterExpression()
			    .getDescription());
		}
		GraphicEditorAdaptator.getMainFrame().getPrototypeDialog()
	...
	for (Task t : currentTask.getChildren()) {
		...
		GraphicEditorAdaptator
			.getMainFrame()
			.getPrototypeDialog()
			.setExecutableTask(t, false, true, number++, false,
				ChoiceEnum.vrai);
		onefinished = true;
	    } else {
		if ((t.getPreExpression().getDescription() == null)
			|| t.getPreExpression().getDescription().equals("") || // (!(t.getPreExpression().getDescription()!=
									       ...
									       // &&
			(map != null && map.get(t.getPreExpression()
				.getDescription()) == ChoiceEnum.vrai)) {
		    GraphicEditorAdaptator
			    .getMainFrame()
			    .getPrototypeDialog()
			    .setExecutableTask(
				    t,
				    t.getStateExecution() != StateExecution.FINISHED,
				    !choice, number++, true && !onefinished,
				    ChoiceEnum.vrai);
		} else {
		    ChoiceEnum choiceE;
		    if (map == null
			    || map.get(t.getPreExpression().getDescription()) == null) {
			choiceE = ChoiceEnum.indeterminee;
		    } else {
			choiceE = map
				.get(t.getPreExpression().getDescription());
		    }
		    GraphicEditorAdaptator
			    .getMainFrame()
			    .getPrototypeDialog()
			    .setExecutableTask(t, !choice, false, number++,
				    true && !onefinished, choiceE);
		}
	    }
	}
	if (choice) {
	...
	for (Task t : currentTask.getChildren()) {
		...
		if (futurActiveFound
			|| currentTask.getChildren().indexOf(lastfinished) > currentTask
				.getChildren().indexOf(t)) {
		    // afficher la tâche comme non éxécutable
		    GraphicEditorAdaptator
			    .getMainFrame()
			    .getPrototypeDialog()
			    .setExecutableTask(t, true, false, number++, false,
				    ChoiceEnum.vrai);
			...
			GraphicEditorAdaptator
				.getMainFrame()
				.getPrototypeDialog()
				.setExecutableTask(t, true, true, number++,
					true, ChoiceEnum.vrai);
		    } else {
			ChoiceEnum choice;
			if (map == null
				|| map.get(t.getPreExpression()
					.getDescription()) == null) {
			    choice = ChoiceEnum.indeterminee;
			} else {
			    choice = map.get(t.getPreExpression()
				    .getDescription());
			}
			GraphicEditorAdaptator
				.getMainFrame()
				.getPrototypeDialog()
				.setExecutableTask(t, true, false, number++,
					true, choice);
	...
	for (Task t : currentTask.getChildren()) {
		...
		if ((t.getPreExpression().getDescription() == null)
		    ...
		    GraphicEditorAdaptator
			    .getMainFrame()
			    .getPrototypeDialog()
			    .setExecutableTask(t, true, true, number++, true,
				    ChoiceEnum.vrai);
		} else {
		    ChoiceEnum choice;
		    if (map == null
		    ...
		    GraphicEditorAdaptator
			    .getMainFrame()
			    .getPrototypeDialog()
			    .setExecutableTask(t, true, false, number++, true,
				    choice);
		}
		if (t.getFacultatif()) {
		    nbFinished++;
