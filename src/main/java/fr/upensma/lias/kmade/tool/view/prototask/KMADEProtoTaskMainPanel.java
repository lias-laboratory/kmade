...
public class KMADEProtoTaskMainPanel extends JFrame {
	...
	if (t.getIterExpression().getProtoTaskConditionExpression() != null
		&& t.getIterExpression().getProtoTaskConditionExpression()
			.getValue().getDescription() != null
		&& !t.getIterExpression().getProtoTaskConditionExpression()
			.getValue().getDescription().equals("")) {
	    conditionLine line = new conditionLine(t.getIterExpression()
		    .getProtoTaskConditionExpression().getValue());
	    condition.put(t.getPreExpression()
		    .getProtoTaskConditionExpression().getValue()
		    .getDescription(), line);
	}
	...
	for (final Task tache : fils) {
	    ...
	    // if (precondition)
	    if (tache.getPreExpression().getProtoTaskConditionExpression() != null
		...
		if (tache.getStateExecution() != StateExecution.FINISHED
			&& tache.getStateExecution() != StateExecution.INACTIVE
			&& tache.getStateExecution() != StateExecution.PASSEE) {
		    if (!condition.containsKey(tache.getPreExpression()
			    .getProtoTaskConditionExpression().getValue()
			    .getDescription())) {
			{
			    conditionLine line = new conditionLine(tache
				    .getPreExpression()
				    .getProtoTaskConditionExpression()
				    .getValue());
			    condition.put(tache.getPreExpression()
				    .getProtoTaskConditionExpression()
				    .getValue().getDescription(), line);
			}
		    }
		}
	    }
	...
	JLabel condi = new JLabel("<HTML><FONT SIZE=" + "5" + ">"
		+ KMADEConstant.PROTOTASK_CONDITION_LABEL + "</FONT></HTML>");
	TitledBorder condiTitle = new TitledBorder(null,
		KMADEConstant.PROTOTASK_CONDITION_LABEL, TitledBorder.LEFT,
