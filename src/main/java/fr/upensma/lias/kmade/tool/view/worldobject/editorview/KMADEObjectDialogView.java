...
public class KMADEObjectDialogView extends JDialog {
	...
	tabs.addTab(KMADEConstant.ABSTRACT_OBJECT_TITLE_NAME,
		this.abstractObjectPanel);

	concreteObjectPanel = new ConcreteObjectPanel(bdd);
	tabs.addTab(KMADEConstant.CONCRETE_OBJECT_TITLE_NAME,
		this.concreteObjectPanel);
	...
	CellView[] allViews = abstractObjectPanel.getGraph()
	...
	JGraph concreteObjectPanelGraph = concreteObjectPanel
		.getConcreteObjectGraphCreation();
	x = 0;
	y = 0;
	...
	if (abstractObjectPanel.getSelected() != null) {
	    ...
	    String message = "Etes vous s�r de vouloir supprimer "
		    + abstractObjectPanel.getSelected().getName() + "\n";
	    if (!(((ObjetAbstrait) abstractObjectPanel.getSelected()
		    .getObject()).getInverseAttributsAbs().isEmpty())) {
		message += KMADEConstant.ABSTRACT_ATTRIBUT_REMOVE_MESSAGE_TITLE
			+ " : ";
		for (AttributAbstrait a : ((ObjetAbstrait) abstractObjectPanel
			.getSelected().getObject()).getInverseAttributsAbs()) {
		    message += a.getName() + " ";
		}
		message += "\n";
	    }
	    if (!(((ObjetAbstrait) abstractObjectPanel.getSelected()
		    .getObject()).getInverseGroupe().isEmpty())) {
		message += KMADEConstant.ABSTRACT_GROUP_REMOVE_MESSAGE_TITLE
			+ " : ";
		for (Groupe g : ((ObjetAbstrait) abstractObjectPanel
			.getSelected().getObject()).getInverseGroupe()) {
		    message += g.getName() + " ";
		}
		message += "\n";
	    }
	    if (!(((ObjetAbstrait) abstractObjectPanel.getSelected()
		    .getObject()).getInverseObjConc().isEmpty())) {
		message += KMADEConstant.CONCRETE_OBJECT_REMOVE_MESSAGE_TITLE
			+ " : ";
		for (ObjetConcret o : ((ObjetAbstrait) abstractObjectPanel
			.getSelected().getObject()).getInverseObjConc()) {
		    message += o.getName() + " ";
		}
		message += "\n";
	    }
	    ...
	    int answer = JOptionPane.showConfirmDialog(
		    KMADEObjectDialogView.this, message,
		    KMADEConstant.ABSTRACT_OBJECT_REMOVE_MESSAGE_TITLE,
		    JOptionPane.YES_NO_OPTION);
	    if (answer == JOptionPane.YES_OPTION) {
		// Remove for the concrete object creation panel
		JGraph concreteObjectCreationGraph = concreteObjectPanel
			.getConcreteObjectGraphCreation();
		CellView[] allViews = concreteObjectCreationGraph
			.getGraphLayoutCache().getAllViews();
		for (int i = allViews.length - 1; i >= 0; i--) {
		    if (((AbstractObjectCell) allViews[i].getCell())
			    .getObject().equals(
				    abstractObjectPanel.getSelected()
					    .getObject())) {
			Object[] cells = { (AbstractObjectCell) allViews[i]
				.getCell() };
			concreteObjectCreationGraph.getGraphLayoutCache()
				.remove(cells);
		    }
		}
		...
		abstractObjectPanel.getGraph().getGraphLayoutCache()
			.remove(cells);
		...
		for (ObjetConcret o : ((ObjetAbstrait) abstractObjectPanel
		    ...
		    concreteObjectCreationGraph = concreteObjectPanel
			    .getConcreteObjectGraph();
		    allViews = concreteObjectCreationGraph
			    .getGraphLayoutCache().getAllViews();
		    for (int i = allViews.length - 1; i >= 0; i--) {
			if (allViews[i].getCell() instanceof ObjectDefaultGraphCell
				&& ((ObjectDefaultGraphCell) allViews[i]
					.getCell()).getObject().equals(o)) {
			    Object[] cell = { (ObjectDefaultGraphCell) allViews[i]
				    .getCell() };
			    concreteObjectCreationGraph.getGraphLayoutCache()
				    .remove(cell);
			}
		...
		for (Groupe g : ((ObjetAbstrait) abstractObjectPanel
			...
			if (allViews[i].getCell() instanceof GroupDefaultGraphCell
				&& ((GroupDefaultGraphCell) allViews[i]
					.getCell()).getObject().equals(g)) {
			    Object[] object = { (GroupDefaultGraphCell) allViews[i]
				    .getCell() };
			    concreteObjectCreationGraph.getGraphLayoutCache()
				    .remove(object);
			}
		...
		InterfaceExpressJava
			.remove(((ObjetAbstrait) abstractObjectPanel
				.getSelected().getObject()).getOid());
	...
	ObjetAbstrait o = (ObjetAbstrait) abstractObjectPanel.getSelected()
		.getObject();
	String oidAttAbs = ExpressAbstractAttribut.createAbstractAttribut(o
		.getOid());
	AttributAbstrait newAtt = (AttributAbstrait) InterfaceExpressJava.bdd
		.prendre(new Oid(oidAttAbs));

	((DescriptionTableModel) abstractObjectPanel.getTable().getModel())
		.addObjectAttribute("", TypeStructure
			.getEnumereIntoLocaleTypeStructure(newAtt
	...
	attRemoved = ((ObjetAbstrait) abstractObjectPanel.getSelected()
		.getObject()).getInverseAttributsAbs().get(index);
	((ObjetAbstrait) abstractObjectPanel.getSelected().getObject())
		.removeInverseAttributsAbs(attRemoved);
	((DescriptionTableModel) abstractObjectPanel.getTable().getModel())
		.clearTable();
	((DescriptionTableModel) abstractObjectPanel.getTable().getModel())
		.writeDetails((ObjetAbstrait) abstractObjectPanel.getSelected()
			.getObject());
	abstractObjectPanel
		.getGraph()
		.getGraphLayoutCache()
		.editCell(abstractObjectPanel.getSelected(),
	...
	if (lastCreated != null) {
	    ...
	    ObjetAbstrait o = (ObjetAbstrait) InterfaceExpressJava
		    .prendre(attRemoved.getUtiliseParClasse().getOid());
	    o.addInverseAttributsAbs(attRemoved);
	    ((DescriptionTableModel) abstractObjectPanel.getTable().getModel())
		    .clearTable();
	    ((DescriptionTableModel) abstractObjectPanel.getTable().getModel())
		    .writeDetails((ObjetAbstrait) abstractObjectPanel
	    ...
	    InterfaceExpressJava.mettre(
		    ((ObjetAbstrait) lastRemoved.getObject()).getOid(),
		    (ObjetAbstrait) lastRemoved.getObject());
	    InterfaceExpressJava.mettre(((ObjetAbstrait) lastRemoved
		    .getObject()).getPoint().getOid(),
		    ((ObjetAbstrait) lastRemoved.getObject()).getPoint());
	    // Abstract attributes
	    for (AttributAbstrait a : ((ObjetAbstrait) lastRemoved.getObject())
	    ...
	    // Concrete object
	    for (ObjetConcret o : ((ObjetAbstrait) lastRemoved.getObject())
		    .getInverseObjConc()) {
		InterfaceExpressJava.mettre(o.getOid(), o);
		InterfaceExpressJava
			.mettre(o.getPoint().getOid(), o.getPoint());
		if (o.getAppartientGroupe() == null)
		    concreteObjectPanel.getConcreteObjectGraph()
			    .getGraphLayoutCache()
	    ...
	    // Groups
	    for (Groupe g : ((ObjetAbstrait) lastRemoved.getObject())
		    .getInverseGroupe()) {
		InterfaceExpressJava.mettre(g.getOid(), g);
		InterfaceExpressJava
			.mettre(g.getPoint().getOid(), g.getPoint());
		...
		concreteObjectPanel.getConcreteObjectGraph()
			.getGraphLayoutCache().insert(group);
	    }
	    abstractObjectPanel.getGraph().getGraphLayoutCache()
		    .insert(lastRemoved);
	    // Set the position in concrete object panel
	    ...
	    int x = 0;
	    int y = 0;
	    CellView[] allViews = concreteObjectPanelGraph
		    .getGraphLayoutCache().getAllViews();
	    for (int i = allViews.length - 1; i >= 0; i--) {
		if (allViews[i].getBounds().contains(x, y)) {
		    y += allViews[i].getBounds().getHeight() + 10;
	    ...
	    AbstractObjectCell cell2 = new AbstractObjectCell(
		    (ObjetAbstrait) lastRemoved.getObject(), x, y);
	    concreteObjectPanelGraph.getGraphLayoutCache().insert(cell2);
	    lastRemoved = null;
	} else if (objectRemoved != null) {
	    InterfaceExpressJava.mettre(objectRemoved.getObject().getOid(),
		    objectRemoved.getObject());
	    for (AttributConcret a : objectRemoved.getObject()
		    .getInverseListAttribut()) {
		InterfaceExpressJava.mettre(a.getOid(), a);
	    }
	    concreteObjectPanel.getConcreteObjectGraph().getGraphLayoutCache()
		    .insert(objectRemoved);
	    objectRemoved = null;
	} else if (groupRemoved != null) {
	    InterfaceExpressJava.mettre(groupRemoved.getObject().getOid(),
		    groupRemoved.getObject());
	    for (ObjetConcret o : groupRemoved.getObject().getEnsemble()
		    .getLstObjConcrets()) {
		InterfaceExpressJava.mettre(o.getOid(), o);
	...
	CellView[] allViews = concreteObjectPanel.getConcreteObjectGraph()
		.getGraphLayoutCache().getAllViews();
	for (int i = allViews.length - 1; i >= 0; i--) {
	    if (allViews[i].getBounds().contains(x, y)) {
		x += 10;
	...
	if (AgregatName.equals(ExpressConstant.STACK_NAME)) {
	    ...
	    ExpressGroup.removeOldAgregatAndCreateNewAgregat(g.getOid().get(),
		    AgregatStructure.SET_AGREGAT.getValue());
	...
	if (concreteObjectPanel.getConcreteObjectGraph().getSelectionCell() instanceof GroupDefaultGraphCell)
	    ((GroupDefaultGraphCell) concreteObjectPanel
		    .getConcreteObjectGraph().getSelectionCell()).edit();
	else if ((concreteObjectPanel.getConcreteObjectGraph()
		.getSelectionCell() instanceof ConcreteObjectCell))
	    ((ConcreteObjectCell) concreteObjectPanel.getConcreteObjectGraph()
		    .getSelectionCell()).edit(concreteObjectPanel
		    .getConcreteObjectGraph());
	else
	    return;
    }
	...
	if (concreteObjectPanel.getConcreteObjectGraph().getSelectionCell() instanceof GroupDefaultGraphCell) {
	    groupRemoved = (GroupDefaultGraphCell) concreteObjectPanel
		    .getConcreteObjectGraph().getSelectionCell();
	    ...
	    groupRemoved.getObject().getContientObj()
		    .removeInverseGroupe(groupRemoved.getObject());
	    // For the undo
	    objectRemoved = null;
	}
		...
		.getSelectionCell() instanceof ConcreteObjectCell) {
	    objectRemoved = (ConcreteObjectCell) concreteObjectPanel
		    .getConcreteObjectGraph().getSelectionCell();
    ...
    /**
     * Method to update the views in case news objects would be created out of
     * the interface
     */
    public void update() {
	List<ObjetAbstrait> abstracts = new ArrayList<ObjetAbstrait>();
