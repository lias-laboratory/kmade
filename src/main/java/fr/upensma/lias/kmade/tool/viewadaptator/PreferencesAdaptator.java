...
public final class PreferencesAdaptator {
	...
	prop.setProperty(KMADEConstant.LEVEL_DISTANCE_NAME,
		KMADEConstant.LEVEL_DISTANCE + "");
	prop.setProperty(KMADEConstant.TASK_MODEL_ORIENTATION_NAME, "3");
	prop.setProperty(KMADEConstant.TASK_DISTANCE_NAME,
		KMADEConstant.TASK_DISTANCE + "");
	...
	// Tasks Tree Parameters
	KMADEPreferencesTaskTreePanel treePanel = GraphicEditorAdaptator
		.getMainFrame().getPreferencesDialog().getTree();
	prop.setProperty(KMADEConstant.LEVEL_DISTANCE_NAME,
		(String) (treePanel.getHauteurArbre().getSelectedItem()));
	prop.setProperty(KMADEConstant.TASK_MODEL_ORIENTATION_NAME,
		(String) (treePanel.getOrientationArbre().getSelectedItem()));
	prop.setProperty(KMADEConstant.TASK_DISTANCE_NAME,
		(String) (treePanel.getDistanceNodale().getSelectedItem()));
	prop.setProperty(KMADEConstant.TASK_SELECTION_COLOR_NAME,
		translateColorUIToConfig(treePanel.getCouleurSelectionTache()
			.getSelectedItem()));
	prop.setProperty(KMADEConstant.ORTHOGONAL_EDGE_NAME, Boolean
		.toString(treePanel.getOrthogonaliteArbre().isSelected()));
	...
	prop.setProperty(KMADEConstant.TASK_FONT_STYLE_NAME,
		translateFontStyleUIToConfig(GraphicEditorAdaptator
			.getMainFrame().getPreferencesDialog().getTask()
			.getFontStyleTask().getSelectedItem()));
	...
	prop.setProperty(
		KMADEConstant.AUTOMATIC_SAVE_NAME,
		Boolean.toString(GraphicEditorAdaptator.getMainFrame()
	...
	prop.setProperty(KMADEConstant.TIME_SAVE_PERIOD_NAME,
		(String) (GraphicEditorAdaptator.getMainFrame()
			...
			.getSelectedItem()));
	prop.setProperty(
		KMADEConstant.SPLASH_SCREEN_NAME,
		Boolean.toString(GraphicEditorAdaptator.getMainFrame()
			.getPreferencesDialog().getGeneral().getSplashScreen()
			.isSelected()));
	prop.setProperty(
		KMADEConstant.BUG_MESSAGE_NAME,
		Boolean.toString(GraphicEditorAdaptator.getMainFrame()
	...
	KMADEPreferencesTaskTreePanel treePanel = GraphicEditorAdaptator
		.getMainFrame().getPreferencesDialog().getTree();
	treePanel.setHauteurArbre(prop
		.getProperty(KMADEConstant.LEVEL_DISTANCE_NAME));
	treePanel.setOrientationArbre(prop
		.getProperty(KMADEConstant.TASK_MODEL_ORIENTATION_NAME));
	treePanel.setDistanceNodale(prop
		.getProperty(KMADEConstant.TASK_DISTANCE_NAME));
	treePanel.setCouleurSelectionTache(translateColorConfigToUI(prop
		.getProperty(KMADEConstant.TASK_SELECTION_COLOR_NAME)));
	treePanel.setOrthogonaliteArbre(prop
		.getProperty(KMADEConstant.ORTHOGONAL_EDGE_NAME));
	...
	GraphicEditorAdaptator
		...
		.setFontSizeOperatorTask(
			prop.getProperty(KMADEConstant.DECOMPOSITION_FONT_SIZE_NAME));
	...
	Color selectedColor = getColorFromString(prop
		.getProperty(KMADEConstant.TASK_SELECTION_COLOR_NAME));
	...
	try {
	    KMADEConstant.TASK_DISTANCE = Integer.parseInt(prop
		    .getProperty(KMADEConstant.TASK_DISTANCE_NAME));
	...
	try {
	    KMADEConstant.LEVEL_DISTANCE = Integer.parseInt(prop
		    .getProperty(KMADEConstant.LEVEL_DISTANCE_NAME));
	} catch (NumberFormatException e) {
	}
    }
	...
	KMADEConstant.ORTHOGONAL_EDGES = Boolean.parseBoolean(prop
		.getProperty(KMADEConstant.ORTHOGONAL_EDGE_NAME));
    }

    private static void setWheelZoom() {
	try {
	    KMADEConstant.WHEEL_ZOOM = Double.parseDouble(prop
		    .getProperty(KMADEConstant.WHEEL_ZOOM_NAME));
	} catch (Exception e) {
	    e.printStackTrace();
	}
    }
