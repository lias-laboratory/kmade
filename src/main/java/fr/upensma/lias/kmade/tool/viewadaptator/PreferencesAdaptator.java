...
public final class PreferencesAdaptator {
	...
	private static void getParametersFromPreferenceUI() {
		// Tasks Tree Parameters
		KMADEPreferencesTaskTreePanel treePanel = GraphicEditorAdaptator.getMainFrame().getPreferencesDialog()
				.getTree();
		prop.setProperty(KMADEConstant.LEVEL_DISTANCE_NAME, (String) (treePanel.getHauteurArbre().getSelectedItem()));
		prop.setProperty(KMADEConstant.TASK_MODEL_ORIENTATION_NAME,
				(String) (treePanel.getOrientationArbre().getSelectedItem()));
		prop.setProperty(KMADEConstant.TASK_DISTANCE_NAME, (String) (treePanel.getDistanceNodale().getSelectedItem()));
		prop.setProperty(KMADEConstant.TASK_SELECTION_COLOR_NAME,
				translateColorUIToConfig(treePanel.getCouleurSelectionTache().getSelectedItem()));
		prop.setProperty(KMADEConstant.ORTHOGONAL_EDGE_NAME,
				Boolean.toString(treePanel.getOrthogonaliteArbre().isSelected()));

		/* Param�tres du panel Tache */
		prop.setProperty(KMADEConstant.TASK_FONT_NAME, (String) (GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getTask().getFontTask().getSelectedItem()));
		prop.setProperty(KMADEConstant.TASK_FONT_SIZE_NAME, (String) (GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getTask().getFontSizeTask().getSelectedItem()));
		prop.setProperty(KMADEConstant.TASK_FONT_COLOR_NAME, translateColorUIToConfig(GraphicEditorAdaptator
				.getMainFrame().getPreferencesDialog().getTask().getColorTask().getSelectedItem()));
		prop.setProperty(KMADEConstant.TASK_FONT_STYLE_NAME, translateFontStyleUIToConfig(GraphicEditorAdaptator
				.getMainFrame().getPreferencesDialog().getTask().getFontStyleTask().getSelectedItem()));

		prop.setProperty(KMADEConstant.TASK_BACKGROUND_COLOR_NAME, translateColorUIToConfig(GraphicEditorAdaptator
				.getMainFrame().getPreferencesDialog().getTask().getBackgroundColorTask().getSelectedItem()));

		prop.setProperty(KMADEConstant.DECOMPOSITION_FONT_NAME, (String) (GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getTask().getFontOperatorTask().getSelectedItem()));
		prop.setProperty(KMADEConstant.DECOMPOSITION_FONT_SIZE_NAME, (String) (GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getTask().getFontSizeOperatorTask().getSelectedItem()));
		prop.setProperty(KMADEConstant.DECOMPOSITION_FONT_STYLE_NAME,
				translateFontStyleUIToConfig(GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
						.getFontStyleOperatorTask().getSelectedItem()));
		prop.setProperty(KMADEConstant.DECOMPOSITION_FONT_COLOR_NAME, translateColorUIToConfig(GraphicEditorAdaptator
				.getMainFrame().getPreferencesDialog().getTask().getColorOperatorTask().getSelectedItem()));
		prop.setProperty(KMADEConstant.TASK_SKIN_STYLE_NAME, (String) (GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getTask().getModelTask().getSelectedItem()));

		prop.setProperty(KMADEConstant.TRIGGER_EVENT_NAME, Boolean.toString(
				GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask().getEventTask().isSelected()));
		prop.setProperty(KMADEConstant.ACTOR_TASK_NAME, Boolean.toString(
				GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask().getActorTask().isSelected()));

		/* Param�tres du panel g�n�ral */
		prop.setProperty(KMADEConstant.AUTOMATIC_SAVE_NAME, Boolean.toString(GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getGeneral().getAutomaticSave().isSelected()));
		prop.setProperty(KMADEConstant.TIME_SAVE_PERIOD_NAME, (String) (GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getGeneral().getTimeSave().getSelectedItem()));
		prop.setProperty(KMADEConstant.SPLASH_SCREEN_NAME, Boolean.toString(GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getGeneral().getSplashScreen().isSelected()));
		prop.setProperty(KMADEConstant.BUG_MESSAGE_NAME, Boolean.toString(
				GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getGeneral().getEnvoiBug().isSelected()));
		prop.setProperty(KMADEConstant.LANGUAGE_NAME, (String) (GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getGeneral().getLanguage().getSelectedItem()));
		prop.setProperty(KMADEConstant.WHEEL_ZOOM_NAME, (String) (GraphicEditorAdaptator.getMainFrame()
				.getPreferencesDialog().getGeneral().getWheelZoom().getSelectedItem()));

	}

	/**
	 * M�thode qui initialise les composants selon les param�tres du fichier
	 * config.ini
	 */
	private static void setParametersToPreferenceUI() {
		...
		treePanel.setCouleurSelectionTache(
				translateColorConfigToUI(prop.getProperty(KMADEConstant.TASK_SELECTION_COLOR_NAME)));
		...
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
				.setFontSizeTask(prop.getProperty(KMADEConstant.TASK_FONT_SIZE_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
				.setColorTask(translateColorConfigToUI(prop.getProperty(KMADEConstant.TASK_FONT_COLOR_NAME)));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
				.setFontStyleTask(translateFontStyleConfigToUI(prop.getProperty(KMADEConstant.TASK_FONT_STYLE_NAME)));

		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask().setBackgroundColorTask(
				translateColorConfigToUI(prop.getProperty(KMADEConstant.TASK_BACKGROUND_COLOR_NAME)));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask().setColorOperatorTask(
				translateColorConfigToUI(prop.getProperty(KMADEConstant.DECOMPOSITION_FONT_COLOR_NAME)));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
				.setModelTask(prop.getProperty(KMADEConstant.TASK_SKIN_STYLE_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
				.setFontOperatorTask(prop.getProperty(KMADEConstant.DECOMPOSITION_FONT_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask().setFontStyleOperatorTask(
				translateFontStyleConfigToUI(prop.getProperty(KMADEConstant.DECOMPOSITION_FONT_STYLE_NAME)));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
				.setFontSizeOperatorTask(prop.getProperty(KMADEConstant.DECOMPOSITION_FONT_SIZE_NAME));

		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
				.setEventTask(prop.getProperty(KMADEConstant.TRIGGER_EVENT_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getTask()
				.setActorTask(prop.getProperty(KMADEConstant.ACTOR_TASK_NAME));

		/* Param�tres de la partie G�n�ral */
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getGeneral()
				.setAutomaticSave(prop.getProperty(KMADEConstant.AUTOMATIC_SAVE_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getGeneral()
				.setTimeSave(prop.getProperty(KMADEConstant.TIME_SAVE_PERIOD_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getGeneral()
				.setSplashScreen(prop.getProperty(KMADEConstant.SPLASH_SCREEN_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getGeneral()
				.setEnvoiBug(prop.getProperty(KMADEConstant.BUG_MESSAGE_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getGeneral()
				.setLanguage(prop.getProperty(KMADEConstant.LANGUAGE_NAME));
		GraphicEditorAdaptator.getMainFrame().getPreferencesDialog().getGeneral()
				.setWheelZoom(prop.getProperty(KMADEConstant.WHEEL_ZOOM_NAME));
	}
	...
	/**
	 * M�thode renvoyant le choix de l'utilisateur pour l'affichage du splashscreen
	 * 
	 * @return boolean
	 */
	...
	private static void setTaskNameFont() {
		...
		int size = java.lang.Integer.parseInt((prop.getProperty(KMADEConstant.TASK_FONT_SIZE_NAME)));
		int style = getStyleFromString(prop.getProperty(KMADEConstant.TASK_FONT_STYLE_NAME));
		Color fontColor = getColorFromString(prop.getProperty(KMADEConstant.TASK_FONT_COLOR_NAME));
	...
	private static void setTaskOperatorFont() {
		...
		int size = java.lang.Integer.parseInt((prop.getProperty(KMADEConstant.DECOMPOSITION_FONT_SIZE_NAME)));
		int style = getStyleFromString(prop.getProperty(KMADEConstant.DECOMPOSITION_FONT_STYLE_NAME));
		Color fontColor = getColorFromString(prop.getProperty(KMADEConstant.DECOMPOSITION_FONT_COLOR_NAME));
	...
	/**
	 * Methode qui permet la traduction de la valeur du fichier pour le style de la
	 * police
	 * 
	 * @return String
	 */
