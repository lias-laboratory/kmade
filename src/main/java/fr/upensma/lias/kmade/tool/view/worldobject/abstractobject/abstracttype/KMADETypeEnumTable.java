...
public class KMADETypeEnumTable extends JScrollPane implements LanguageFactory {
	...
	String[] columnNames = { KMADEConstant.ENUMERATION_NAME_TABLE,
		KMADEConstant.ENUMERATION_DESCRIPTION_TABLE };
	myTableListModel = new MyTableListModel(columnNames,
		KMADEConstant.ENUMERATION_NEW_OBJECT_TABLE);
	...
	myListTable.getListSelectionModel().addListSelectionListener(
		new ListSelectionListener() {
			...
			if (myListTable.getListSelectionModel()
			    ...
			    TypePanelAdaptator.setActiveEnumeration("",
				    Oid.OID_NULL);
			} else {
				...
				if (selectedRow != myTableListModel
					.getRowCount() - 1) {
				    Object[] tempoValue = (Object[]) myTableListModel
					    .getListTableData()
					    .get(selectedRow);
				    TypePanelAdaptator.setActiveEnumeration(
					    (String) tempoValue[0],
	...
	public void keyPressed(KeyEvent ke) {
		...
		boolean state = GraphicEditorAdaptator.removeSelectedItem(
			"Enumeration", r, myTableListModel.getListTableData(),
	...
	myTableListModel
		.setNameDefault(KMADEConstant.ENUMERATION_NEW_OBJECT_TABLE);
    }
}
