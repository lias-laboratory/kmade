...
public class AbstractObjectPanel extends JPanel implements MouseListener {
	...
	private static JLabel inactive = new JLabel(KMADEConstant.NO_SELECTED_OBJECT, JLabel.CENTER);
	...
	public AbstractObjectPanel(ExpressDB bdd) {
		...
		this.descriptionTable.setDefaultRenderer(Object.class, new DescriptionTableRenderer());
		this.descriptionTable.setDefaultEditor(String.class, (TableCellEditor) new DescriptionTableEditor());

		// Graphs
		this.abstractObjectGraph.getGraphLayoutCache().setFactory(new DefaultCellViewFactory() {
			...
			public CellView createView(GraphModel gM, Object cell) {
				if (cell instanceof AbstractObjectCell) {
					return new AbstractObjectVertexView(cell, abstractObjectGraph);
				} else {
		...
		TitledBorder border = BorderFactory.createTitledBorder(KMADEConstant.PROPERTIES_MESSAGE);
	 ...
	 * 
	 * @param list of abstracts objects
	 */
		...
		if (!abstracts.isEmpty()) {
			for (int i = 0; i < abstracts.size(); i++) {
				if (abstracts.get(i).getPoint() == null) {
					cell = new AbstractObjectCell(abstracts.get(i), abstractX, abstractY);
					abstractX += cell.getWidth() + KMADEConstant.TASK_DISTANCE;
				} else
					cell = new AbstractObjectCell(abstracts.get(i), abstracts.get(i).getPoint());
	...
	/**
	 * Method to get the table where the details of the selected cell are displayed
	 * 
	 * @return the table
	 */
	...
	public void clearGraph() {
		CellView[] cells = abstractObjectGraph.getGraphLayoutCache().getAllViews();
	...
	public void mouseClicked(MouseEvent e) {
		if (e.getSource().equals(abstractObjectGraph)) {
			// Select the new cell
			selected = (AbstractObjectCell) abstractObjectGraph.getSelectionCell();
			if (selected != null) {
				((ObjetAbstrait) selected.getObject()).getPoint().setX(e.getX());
				((ObjetAbstrait) selected.getObject()).getPoint().setY(e.getY());
			}
		...
		if (selected != null) {
			((DescriptionTableModel) descriptionTable.getModel()).writeDetails((ObjetAbstrait) selected.getObject());
	 ...
	 * @author Joachim TROUVERIE
	 */
	class DescriptionTableEditor extends AbstractCellEditor implements TableCellEditor {
		...
		private String[] values = TypeStructure.getNameLocaleTypeStructureWithoutInterval();

		/**
		 * @return a combo box for the boolean, and a text filed for numbers and text
		 */
		public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row,
			...
			if (table.isCellEditable(row, column)) {
				...
				if (table.getModel().getValueAt(row, 0).equals(KMADEConstant.ABSTRACT_ATTRIBUT_TYPE_TABLE)) {
	...
	class DescriptionTableRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			if (table.getValueAt(row, 0).equals(KMADEConstant.GENERALITY)
					|| table.getValueAt(row, 0).equals(KMADEConstant.ATTRIBUTES_NAME)) {
				cell.setBackground(Color.LIGHT_GRAY);
			}

			else if (table.getValueAt(row, 0).equals("") && table.getValueAt(row, 1).equals(""))
	...
	class DescriptionTableModel extends AbstractTableModel {
		...
		public boolean isCellEditable(int rowIndex, int columnIndex) {
			if (data.get(rowIndex)[0].equals(KMADEConstant.GENERALITY)
					|| data.get(rowIndex)[0].equals(KMADEConstant.ATTRIBUTES_NAME)) {
				return false;
			} else
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
			...
			if (rowIndex == nameRow)
				AbstractObjectAdaptator.setAbstractObjectName(objet.getOid().get(), (String) aValue);
			else if (rowIndex == descriptionRow)
				AbstractObjectAdaptator.setAbstractObjectDescription(objet.getOid().get(), (String) aValue);
			else if (rowIndex >= 3) {
				if (rowIndex % 4 == 0)
					this.objet.getInverseAttributsAbs().get(rowIndex / 4 - 1).setName((String) aValue);
				else if (rowIndex % 4 == 1) {
					String type = TypeStructure.getLocaleTypeStructureIntoEnumere((String) aValue);
					this.objet.getInverseAttributsAbs().get(rowIndex / 4 - 1)
							.setTypeStructure(TypeStructure.getValue(type));
				}

				else if (rowIndex % 4 == 2)
					this.objet.getInverseAttributsAbs().get(rowIndex / 4 - 1).setDescription((String) aValue);
			}

			ObjectDialogViewAdaptator.updateTaskPropPanel();
			this.fireTableDataChanged();

			abstractObjectGraph.getGraphLayoutCache().editCell(selected, selected.getAttributes());
		}
		...
		public void addObjectAttribute(String name, String type, String description) {
			...
			this.addNewRow(KMADEConstant.ABSTRACT_ATTRIBUT_DESCRIPTION_TABLE, description);
		}
		...
		public void writeDetails(ObjetAbstrait objet) {
			...
			if (!objet.getInverseAttributsAbs().isEmpty()) {
				for (int i = 0; i < objet.getInverseAttributsAbs().size(); i++) {
					AttributAbstrait att = objet.getInverseAttributsAbs().get(i);
					this.addObjectAttribute(att.getName(),
							TypeStructure.getEnumereIntoLocaleTypeStructure(att.getTypeStructure().getValue()),
							att.getDescription());
					this.addNewRow("", "");
				}
			}
