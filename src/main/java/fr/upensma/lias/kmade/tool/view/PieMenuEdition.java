...
public class PieMenuEdition extends Menu {
	...
	public PieMenuEdition(Canvas c) {
		...
		this.canvas = c;
		String[] items = { KMADEConstant.COMPLETE_EDIT_TASK_ACTION_MESSAGE_PIE_MENU, KMADEConstant.COPY_ACTION_MESSAGE,
				KMADEConstant.PASTE_ACTION_MESSAGE, KMADEConstant.CUT_ACTION_MESSAGE,
		...
		smText = new JStateMachine() {
			...
			public State initialState = new State() {
				/*
				 * on "press" le bouton de droite sur la tache pour afficher le menu
				 */
								...
								if (selection.length == 1) {
									...
									for (int i = 0; i < allViews.length; i++) {
										if (!selection[0].getClass().equals(KMADEDefaultEdge.class)
											...
											/*
											 * on v�rifie que l'on est sur la t�che
											 */
											...
											Point2D pointMaxRectTask = new Point2D.Double(pointMinRectTask.getX()
													+ ((KMADEVertexView) allViews[i]).getBounds().getWidth() * scale,
													pointMinRectTask.getY()
															+ ((KMADEVertexView) allViews[i]).getBounds().getHeight()
																	* scale);

											Point2D currentPosition = getPoint();

											if (currentPosition.getX() > pointMinRectTask.getX()
													&& currentPosition.getX() < pointMaxRectTask.getX()
													&& currentPosition.getY() > pointMinRectTask.getY()
													&& currentPosition.getY() < pointMaxRectTask.getY()) {
												return true;
											}
		...
		interaction = new CStateMachine() {
			...
			public State menuOff = new State() {
				...
				public void leave() {
					...
					/*
					 * on affiche l'item d'"�dition compl�te" seulement si on n'a pas de s�lection
					 * multiple et l'item "coller" seulement si on a des t�ches copi�es ou coup�es
					 */
					for (tagWhole.reset(); tagWhole.hasNext();) {
						...
						if (tagWholeIterator.getClass().equals(CText.class)) {
							...
							if (textTag.getText().equals(KMADEConstant.COMPLETE_EDIT_TASK_ACTION_MESSAGE_PIE_MENU)) {
							...
							if (textTag.getText().equals(KMADEConstant.PASTE_ACTION_MESSAGE)) {
								CPolyLine formTag = (CPolyLine) precedentTag;
				...
				Transition select = new Release(BUTTON1, ">> menuOff") {
					public void action() {
						...
						if (lastItemVisited != null) {
							...
							/*
							 * on res�lectionne la tache s�lectionn�e avant que le clic gauche ne l'enl�ve
							 */
							if (selection != null) {
								GraphicEditorAdaptator.getMainFrame();
								...
								for (int i = 0; i < selection.length; i++) {
									...
									for (int j = 0; j < allViews.length; j++) {
										if (!selection[0].getClass().equals(KMADEDefaultEdge.class)
												&& allViews[j].getCell().equals(selection[i])) {
											KMADEGraph g = KMADEMainFrame.getProjectPanel().getTaskModelPanel()
													.getJGraph();
											g.addSelectionCell(allViews[j].getCell());
							...
							if (lastItemVisited.equals(KMADEConstant.COMPLETE_EDIT_TASK_ACTION_MESSAGE_PIE_MENU)) {
								TaskPropertiesEnhancedEditorAdaptator.showNMDAEnhancedTaskEditor();
							} else if (lastItemVisited.equals(KMADEConstant.COPY_ACTION_MESSAGE)) {
								KMADeAdaptator.copyAction();
							} else if (lastItemVisited.equals(KMADEConstant.PASTE_ACTION_MESSAGE)) {
								KMADeAdaptator.pasteAction();
							} else if (lastItemVisited.equals(KMADEConstant.CUT_ACTION_MESSAGE)) {
								KMADeAdaptator.cutAction();
							} else if (lastItemVisited.equals(KMADEConstant.DELETE_CELL_ACTION_MESSAGE_PIE_MENU)) {
	...
	void menuLayout(String[] items) {
		...
		for (int i = 0; i < items.length; i++) {
			...
			/* le texte de l'item */
			labelItem = (CText) canvas.newText(0, 0, items[i], Menu.FONT).setFillPaint(Color.BLUE).setPickable(false)
					.addTag(tagWhole).addTag(tagLabels).setReferencePoint(0.5, 0.5)
