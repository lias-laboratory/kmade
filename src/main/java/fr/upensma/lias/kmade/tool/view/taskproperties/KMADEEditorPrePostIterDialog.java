...
public class KMADEEditorPrePostIterDialog extends JPropertiesEditorDialog {
	...
	public KMADEEditorPrePostIterDialog() {
		...
		JSplitPane mySplitPane = KMADEEnhancedSplitPane.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT, cardPanel,
				refReadObject);
	...
	public void showPropertiesEditor(DefaultPropertiesTableModel refModel, int row) {
		...
		if (refModel.getValue(row) instanceof PreExpression) {
			this.setTitle(KMADEConstant.EDITOR_PRE_TITLE_NAME + " : "
					+ GraphicEditorAdaptator.getSelectedGraphicTask().getTask().getName());
			refPreconditionPanel.setOutputMessage();
			refPreconditionPanel.getTextArea().setText(((PreExpression) refModel.getValue(row)).getFormalText());
			...
			refEffetsDeBordPanel.textArea.setText(((SideEffectExpression) refModel.getValue(row)).getFormalText());
			refEffetsDeBordPanel.setDescriptionArea(((SideEffectExpression) refModel.getValue(row)).getDescription());
			...
			refIterconditionPanel.getBaseIterationPanel().textArea
					.setText(((IterExpression) refModel.getValue(row)).getFormalText());
