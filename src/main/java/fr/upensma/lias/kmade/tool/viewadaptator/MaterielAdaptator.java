...
public final class MaterielAdaptator {
	...
	int temporg = KMADEMainFrame.getProjectPanel().getParcMachinesPanel()
		.getParcMachinesObjectTable().getModel().getRowCount() - 1;
	int tempind = KMADEMainFrame.getProjectPanel().getMachinePanel()
		.getMachineObjectTable().getModel().getRowCount() - 1;
	...
	for (int i = temporg; i < temporg + tempind; i++) {
	    temp[i][0] = KMADEMainFrame.getProjectPanel().getMachinePanel()
		    .getMachineObjectTable().getModel()
