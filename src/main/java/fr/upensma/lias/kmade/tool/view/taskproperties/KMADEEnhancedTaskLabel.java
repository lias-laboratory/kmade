...
public class KMADEEnhancedTaskLabel extends JComponent {
    ...
    private static final Image up = new ImageIcon(
	    GraphicEditorAdaptator.class
		    .getResource(KMADEConstant.UP_TASK_IMAGE)).getImage();

    private static final Image down = new ImageIcon(
	    GraphicEditorAdaptator.class
		    .getResource(KMADEConstant.DOWN_TASK_IMAGE)).getImage();

    private static final Image left = new ImageIcon(
	    GraphicEditorAdaptator.class
		    .getResource(KMADEConstant.LEFT_TASK_IMAGE)).getImage();

    private static final Image right = new ImageIcon(
	    GraphicEditorAdaptator.class
		    .getResource(KMADEConstant.RIGHT_TASK_IMAGE)).getImage();

    private static final Image no_up = new ImageIcon(
	    GraphicEditorAdaptator.class
		    .getResource(KMADEConstant.NO_UP_TASK_IMAGE)).getImage();

    private static final Image no_down = new ImageIcon(
	    GraphicEditorAdaptator.class
		    .getResource(KMADEConstant.NO_DOWN_TASK_IMAGE)).getImage();

    private static final Image no_left = new ImageIcon(
	    GraphicEditorAdaptator.class
		    .getResource(KMADEConstant.NO_LEFT_TASK_IMAGE)).getImage();

    private static final Image no_right = new ImageIcon(
	    GraphicEditorAdaptator.class
		    .getResource(KMADEConstant.NO_RIGHT_TASK_IMAGE)).getImage();
	...
	this.addMouseListener(new MouseListener() {
		...
		if (clicking) {
		    TaskPropertiesEnhancedEditorAdaptator
			    .switchToOtherTask(position);
		    TaskPropertiesEnhancedEditorAdaptator
			    .setSelectedTaskAttributes();
		    clicking = false;
		    KMADEEnhancedTaskLabel.this.repaint();
		}
	    }
	...
	int widthText = SwingUtilities.computeStringWidth(fm, titleName
		+ displayText);
	int widthNumero = SwingUtilities.computeStringWidth(fm, titleNum
		+ displayNumero);
	g2.setFont(font);
	switch (position) {
	case 0: {
	...
	case 3: {
	    ...
	    widthText = SwingUtilities.computeStringWidth(fm, titleName
		    + displayText);
	    g2.setColor(Color.black);
	    g2.drawString(titleName + displayText, -20 - widthText, 16);
	    g2.drawString(titleNum + displayNumero, -20 - widthText,
		    16 + heighText);
	    break;
	}
	}
