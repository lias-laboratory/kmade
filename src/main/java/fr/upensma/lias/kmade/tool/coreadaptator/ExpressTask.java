...
public final class ExpressTask {
	...
	public static Task[] pasteElementsFromClipBoard(java.awt.Point pt) {
		counterClipboard++;
		Object[] myTaskList = InterfaceExpressJava.getAllReferencesOfEntityFromClipBoard(ExpressConstant.CORE_PACKAGE,
				ExpressConstant.TASK_CLASS);
		...
		for (int i = 0; i < myTaskList.length; i++) {
			...
			Point refPoint = (Point) InterfaceExpressJava.createEntityReferenceBack(ExpressConstant.CORE_PACKAGE,
					ExpressConstant.POINT_CLASS);
			Task refTache = (Task) InterfaceExpressJava.createEntityReferenceBack(ExpressConstant.CORE_PACKAGE,
					ExpressConstant.TASK_CLASS);
			...
			if (pt != null) {
				...
				refPoint.setX(new Integer(x.intValue() + ExpressTask.counterClipboard * 50));
			...
			if (((Task) myTaskList[i]).getInterruptible() == true) {
				refTache.setInterruptible(((Task) myTaskList[i]).getInterruptible());
			}
			...
			refTache.setSideEffetExpression(((Task) myTaskList[i]).getEffetsDeBordExpression()); // effetsdebord
			refTache.setIterExpression(((Task) myTaskList[i]).getIterExpression()); // iteration
	 ...
	 * @param edgeOIDList
	 */
	public static Task[] copyElementsIntoClipBoard(ArrayList<Task> myTacheList, ArrayList<String[]> edgeOIDList) {
		...
		for (int i = 0; i < myTacheList.size(); i++) {
			...
			Point refPoint = (Point) InterfaceExpressJava
					.createEntityReferenceBackIntoClipBoard(ExpressConstant.CORE_PACKAGE, ExpressConstant.POINT_CLASS);
			Task refTache = (Task) InterfaceExpressJava
					.createEntityReferenceBackIntoClipBoard(ExpressConstant.CORE_PACKAGE, ExpressConstant.TASK_CLASS);
			...
			refTache.setSideEffetExpression(myTacheList.get(i).getEffetsDeBordExpression());// effetsdebord
						...
						if (oldMotherTask.equals(edgeOIDList.get(l)[0])) {
							...
							for (int k = 0; k < myNewTaskList.length && !trouveeFils; k++) {
								if (k != j) { // Une tache mere ne peut etre son
									// fils.
									if (myTacheList.get(k).getOid().equals(new Oid(oldSonTask))) {
										// Ici on a vérifié qu'il y a une
										// nouvelle tache qui doit etre
										// connectee.
										myNewTaskList[j].getChildren().add(myNewTaskList[k]);
										// On n'oublie pas la reference de la
										// sous-taches a la tache mere
										myNewTaskList[k].setMother(myNewTaskList[j]);
										trouveeFils = true;
	...
	public static Task addNewTask(int x, int y, Executor e) {
		...
		Oid oidTache = InterfaceExpressJava.createEntity(ExpressConstant.CORE_PACKAGE, ExpressConstant.TASK_CLASS);
		Oid oidMedia = InterfaceExpressJava.createEntity(ExpressConstant.CORE_PACKAGE, ExpressConstant.MEDIA_CLASS);
	...
	public static boolean setEventTask(Task currentTask, String oldvaleur, String valeur) {
	...
	public static boolean addActorSystem(Task currentTask, String oidactSys) {
		ActorSystem act = (ActorSystem) InterfaceExpressJava.prendre(new Oid(oidactSys));
	...
	public static void removeActorSystem(String oidAct) {
		ActorSystem a = (ActorSystem) InterfaceExpressJava.prendre(new Oid(oidAct));
		a.delete();
		notifyObservers();
	}
	...
	public static void affRemoveEvent(Task currentTask, String maValue) {
		...
		InterfaceExpressJava.getGestionWarning().addMessage(e.getOid(), 4,
				ExpressConstant.REMOVE_OF_THE_TASK_MESSAGE + " \"" + currentTask.getName() + "\"");
	}
	...
	public static void removeProtoTaskPreCondition(String oidCondition) {
		...
		// pttcondition
		GraphicEditorAdaptator.getSelectedGraphicTask().getTask().getPreExpression().getProtoTaskConditionExpression()
				.removeCondition();
	}
	...
	public static void removeProtoTaskIterCondition(String oidCondition) {
		// l'oid est passé pour quand la condition pourra avoir plusieurs
		// pttcondition
		GraphicEditorAdaptator.getSelectedGraphicTask().getTask().getIterExpression().getProtoTaskConditionExpression()
				.removeCondition();
	}
}
