...
public class KMADEUndoRedoManager {
	...
	/* 2 piles contenant les objets traités, ex : un lien supprimé, une tâche supprimée, renommée .... */
	private ArrayDeque<Object> objToUndo = new ArrayDeque<Object>();
	private ArrayDeque<Object> objToRedo = new ArrayDeque<Object>();

	/* 2 piles indiquant l'opération (des strings) à faire : "Edge" pour lien, "Name" pour renommer une tâche,....*/  
		...
		try {
			...
			if(undo.equals("Name")){
				...
				ExpressTask.setDurationTask(
						GraphicEditorAdaptator.getSelectedExpressTask(), duration);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				ExpressTask.setOperatorTask2(task,
						dec);
				...
				ExpressTask.addNewEdge(mere.getTask(),
						fils.getTask());		
	...
	public void redo() {
		...
		try {
			...
			if(Redo.equals("Name")){
				...
				ExpressTask.removeEdge(mere.getTask(),
						fils.getTask());		
			}
	...
	protected  void updateHistoryButtons() {
		...
		/*AG*/
		GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().setUndoActionViewState(undoManager.canUndo(graph.getGraphLayoutCache()));
		GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().setRedoActionViewState(
				canRedo());
