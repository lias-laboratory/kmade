...
public class KMADEUndoRedoManager {
	...
	public KMADEUndoRedoManager(KMADEGraph g) {
		...
		undoManager = new GraphUndoManager() {
			...
			/*
			 * une méthode qui écoute et indique chaque modification au niveau du JGraph
			 */
	...
	public void undo() {
		try {
			...
			if (undo.equals("Name")) {
				...
				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));

				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getName()); // le nom actuel
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getGoal());
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getLabelName());
				...
				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setLabelTask(GraphicEditorAdaptator.getSelectedExpressTask(), label);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getDuration());
				...
				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setDurationTask(GraphicEditorAdaptator.getSelectedExpressTask(), duration);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getDescription());
				...
				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setObservationTask(GraphicEditorAdaptator.getSelectedExpressTask(), obs);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getExecutor());
				...
				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setExecutingUserTask(GraphicEditorAdaptator.getSelectedExpressTask(), ex);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getFrequency());
				...
				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setFrequencyTask(GraphicEditorAdaptator.getSelectedExpressTask(), fr);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getFrequencyValue());
				...
				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setFrequencyValueTask(GraphicEditorAdaptator.getSelectedExpressTask(), fr);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getModality());
				...
				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setModalityTask(GraphicEditorAdaptator.getSelectedExpressTask(), mod);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getFacultatif());

				objToRedo.push(vec);
				whatRedo.push(undo);

				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setOptionalTask(GraphicEditorAdaptator.getSelectedExpressTask(), opt);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getImportance());

				objToRedo.push(vec);
				whatRedo.push(undo);

				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setSignificantTask(GraphicEditorAdaptator.getSelectedExpressTask(), imp);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getInterruptible());

				objToRedo.push(vec);
				whatRedo.push(undo);

				GraphicEditorAdaptator.setSelectedTask(GraphicEditorAdaptator.getTaskModelPanel().getTask(oid));
				ExpressTask.setInterruptibleTask(GraphicEditorAdaptator.getSelectedExpressTask(), intr);
				GraphicEditorAdaptator.updateSelectedTaskGraphModel();
				...
				Vector vec = (Vector) objToUndo.pop();
				objToRedo.push(vec);
				InterfaceExpressJava.toRedo.push((ExpressDB) InterfaceExpressJava.toUndo.peek().clone());

				Task[] taskList = (Task[]) vec.get(1);
				ArrayList<String[]> edgeOIDList = new ArrayList<String[]>();
				for (int i = 0; i < taskList.length; i++) {
					undoManager.undo();
					InterfaceExpressJava.toRedo.push(InterfaceExpressJava.toUndo.pop());
					...
					if (mere != null) {
						...
						oidFils = oid;
						// R�f�rence de la t�che m�re.
						oidMere = ((Task) taskList[i]).getMother().getOid().get();
				...
				InterfaceExpressJava.bdd = (ExpressDB) InterfaceExpressJava.toUndo.peek().clone();
				...
				for (int i = 0; i < edgeOIDList.size(); i++) {
					...
					for (int j = 0; j < taskList.length; j++) {
						if (edgeOIDList.get(i)[0].equals(taskList[j].getOid().get()))
							mere = (Task) taskList[j];
						if (edgeOIDList.get(i)[1].equals(taskList[j].getOid().get()))
				...
				ArrayList<String[]> edgeOIDList = (ArrayList<String[]>) vec.get(1);
				...
				for (int i = 0; i < edgeOIDList.size(); i++) {
					...
					KMADEDefaultGraphCell mere = GraphicEditorAdaptator.getTaskModelPanel()
							.getTask(edgeOIDList.get(i)[0]);
					KMADEDefaultGraphCell fils = GraphicEditorAdaptator.getTaskModelPanel()
							.getTask(edgeOIDList.get(i)[1]);
				...
				KMADEDefaultGraphCell mere = GraphicEditorAdaptator.getTaskModelPanel().getTask(oidMother);
				KMADEDefaultGraphCell fils = GraphicEditorAdaptator.getTaskModelPanel().getTask(oidSon);
				...
				KMADEDefaultGraphCell motherCell = ((KMADEDefaultEdge) edge).getMotherCell();
				// Suppression dans Express du lien.
				ExpressTask.removeEdge(motherCell.getTask(), ((KMADEDefaultEdge) edge).getSonCell().getTask());
				whatUndo.pop();
				undoManager.undo();
	...
	public void redo() {
		...
		try {
			...
			if (Redo.equals("Name")) {
				...
				vec.add(GraphicEditorAdaptator.getSelectedExpressTask().getName());
				...
				Vector vec = (Vector) objToRedo.pop();
				objToUndo.push(vec);
				ExpressDB bddToUndo = (ExpressDB) InterfaceExpressJava.bdd.clone();
				...
				for (int i = 0; i < taskList.length; i++) {
					...
					InterfaceExpressJava.toUndo.push(InterfaceExpressJava.toRedo.pop());
				...
				InterfaceExpressJava.bdd = (ExpressDB) InterfaceExpressJava.toRedo.pop().clone();
				...
				ExpressTask.addNewEdge(motherCell.getTask(), ((KMADEDefaultEdge) edge).getSonCell().getTask());
	...
	public boolean canRedo() {
		...
		return undoManager.canRedo(graph.getGraphLayoutCache())
				|| (!whatRedo.isEmpty() && whatRedo.peek().equals("Decomposition"))
				|| (!whatRedo.isEmpty() && whatRedo.peek().equals("Name"))
				|| (!whatRedo.isEmpty() && whatRedo.peek().equals("Executant"))
				|| (!whatRedo.isEmpty() && whatRedo.peek().equals("Interruptible"))
				...
				|| (!whatRedo.isEmpty() && whatRedo.peek().equals("FrequenceValue"))
	...
	protected void updateHistoryButtons() {
		...
		GraphicEditorAdaptator.getMainFrame().getApplicationToolBar()
				.setUndoActionViewState(undoManager.canUndo(graph.getGraphLayoutCache()));
		GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().setRedoActionViewState(canRedo());
	...
	public void clearAllUndoRedoDeque() {
		...
		GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().setUndoActionViewState(false);
		GraphicEditorAdaptator.getMainFrame().getApplicationToolBar().setRedoActionViewState(false);
