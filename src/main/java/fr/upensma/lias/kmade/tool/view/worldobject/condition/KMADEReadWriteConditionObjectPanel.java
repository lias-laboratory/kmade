 ...
 */
public class KMADEReadWriteConditionObjectPanel extends JScrollPane implements LanguageFactory {
	...
	public KMADEReadWriteConditionObjectPanel() {
		...
		myTableListModel = new MyTableListModel(columnNames, KMADEConstant.CONDITION_ADD_MESSAGE);
		...
		myTitledBorder = new TitledBorder(null, KMADEConstant.PRECONDITION_TAB_PROTOTASK, TitledBorder.CENTER,
				TitledBorder.TOP);
		this.setBorder(myTitledBorder);
		myListTable.setCellEditor(1, "List", StateCondition.getNameStateCondition());
	...
	class MyTableListModel extends DefaultListTableModel {
		...
		private String nameTest(String value) {
			String name = value;
			/*
			 * boolean needVerif = true; // un nom null annule l'�dition, boucle tant que le
			 * nom n'est pas // correct while (name != null && needVerif) { // v�rification
			 * de l'expression r�guli�re avec affichage d'un // popUp if
			 * (!RegularExpression.isGoodLabelName(name)) { name = (String) JOptionPane
			 * .showInputDialog( GraphicEditorAdaptator.getMainFrame(),
			 * KMADEConstant.BAD_CHARACTER_TEXT, KMADEConstant.BAD_CARACTER_TITLE,
			 * JOptionPane.YES_NO_OPTION, new ImageIcon( GraphicEditorAdaptator.class
			 * .getResource(KMADEConstant.ASK_DIALOG_IMAGE)), null, name); } else { //
			 * l'expression est ok if (Label.isUniqueName(name)) { // si le nom est unique,
			 * le nom est correct et possible needVerif = false; } else { name = (String)
			 * JOptionPane .showInputDialog( GraphicEditorAdaptator.getMainFrame(),
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
			if (rowIndex + 1 == this.getRowCount()) {
				...
				value = ConditionAdaptator.setConditionDescription(oidObject, value);
				ConditionAdaptator.setConditionDefaultValue(oidObject, ExpressConstant.INDETERMINATE);
				Object[] tempo = { value, ExpressConstant.INDETERMINATE, oidObject, false };
				myTableListModel.getListTableData().add(tempo);
				...
				case 0: {
					...
					value = ConditionAdaptator.setConditionDescription(oidRow, (String) value);
					break;
				}
				case 1: {
					ConditionAdaptator.setConditionDefaultValue(oidRow, (String) value);
					break;
				}
	...
	class MyKeyListener extends KeyAdapter {
		public void keyPressed(KeyEvent ke) {
			if (ke.getKeyCode() == KeyEvent.VK_DELETE) {
				...
				boolean state = GraphicEditorAdaptator.removeSelectedItem("Condition", r,
