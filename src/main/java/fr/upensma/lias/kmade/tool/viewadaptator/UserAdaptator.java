...
public final class UserAdaptator {
    ...
    /**
     * Retourne la liste des noms des acteurs potentiels de la t�che qui ne sont
     * pas d�j� acteur de la t�che
     * 
     * @param oidtask
     * @param oidTask
     * @return
	...
	int temporg = KMADEMainFrame.getProjectPanel().getOrganisationPanel()
		.getOrganizationObjectTable().getModel().getRowCount() - 1;
	int tempind = KMADEMainFrame.getProjectPanel().tableIndividu()
		.getIndividuObjectTable().getModel().getRowCount() - 1;

	Object[][] temp = new Object[temporg + tempind][6];
	for (int i = 0; i < temporg; i++) {
	    temp[i][0] = KMADEMainFrame.getProjectPanel()
		    .getOrganisationPanel().getOrganizationObjectTable()
	...
	for (int i = temporg; i < temporg + tempind; i++) {
	    temp[i][0] = KMADEMainFrame.getProjectPanel().tableIndividu()
		    .getIndividuObjectTable().getModel()
