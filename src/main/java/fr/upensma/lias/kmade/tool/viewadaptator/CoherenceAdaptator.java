...
public final class CoherenceAdaptator {
		...
		if (valueAt != null) {
			if (valueAt instanceof Task) {
				GraphicEditorAdaptator.getTaskModelPanel().getJGraph()
						.scrollCellToVisible((KMADEDefaultGraphCell) ((Task) valueAt).getRefJTask());
	...
	public static void goToSelectMessage(Object[] pmessage) {
		GraphicEditorAdaptator.getTaskModelPanel().getJGraph()
				.scrollCellToVisible((KMADEDefaultGraphCell) ((Task) pmessage[TASK]).getRefJTask());
	...
	private static void addErrorMessage(String message, int type, int location, Task tache) {
		Object[] temp = { CoherenceAdaptator.ERROR_PROBLEM, message, tache, type, location };
		myMessageList.add(temp);
	}

	private static void addWarningMessage(String message, int type, int location, Task tache) {
	...
	private static synchronized void checkTaskModels() {
		...
		for (Task current : myTaskList) {
			...
			if (current.isRoot() && current.isLeaf()) {
				CoherenceAdaptator.addErrorMessage(KMADEConstant.NO_ALONE_MESSAGE_PROBLEM, HIERARCHICAL_TYPE,
						HIERARCHICAL_TASK_MODEL, current);
			} else {
	...
	private static void checkObjectsTaskModel(Task myTache) {
		if (myTache.getPreExpression().getNodeExpression() == null) {
			CoherenceAdaptator.addErrorMessage(KMADEConstant.PRECONDITION_EXPRESSION_MESSAGE_PROBLEM, EXPRESSION_TYPE,
					PRE_CONDITION_DIALOG, myTache);
		}

		if (myTache.getEffetsDeBordExpression().getNodeExpression() == null) {
			CoherenceAdaptator.addErrorMessage(KMADEConstant.EFFETSDEBORD_EXPRESSION_MESSAGE_PROBLEM, EXPRESSION_TYPE,
					POST_CONDITION_DIALOG, myTache);
		}

		if (myTache.getIterExpression().getNodeExpression() == null) {
			CoherenceAdaptator.addErrorMessage(KMADEConstant.ITERATION_EXPRESSION_MESSAGE_PROBLEM, EXPRESSION_TYPE,
					ITERATION_DIALOG, myTache);
		}
	...
	private static void checkHiearchicalTaskModel(Task myTache) {
		...
		if (!myTache.isLeaf() && myTache.getChildren().size() == 1) {
			CoherenceAdaptator.addErrorMessage(KMADEConstant.NO_ONLY_ONE_SUBTASK_MESSAGE_PROBLEM, HIERARCHICAL_TYPE,
					HIERARCHICAL_TASK_MODEL, myTache);
		}

		if (myTache.isLeaf() && (myTache.getOrdering() != Decomposition.ELE)) {
			CoherenceAdaptator.addErrorMessage(KMADEConstant.ELEMENTARY_DECOMPOSITION_FOR_LEAF_TASK, HIERARCHICAL_TYPE,
					HIERARCHICAL_TASK_MODEL, myTache);
		}

		// Op�rateur de d�composition non pr�cis�.
		if (!myTache.isLeaf()
				&& (myTache.getOrdering() == Decomposition.ELE || myTache.getOrdering() == Decomposition.INCONNU)) {
			CoherenceAdaptator.addErrorMessage(KMADEConstant.NO_DECOMPOSITION_SPECIFIED_MESSAGE_PROBLEM,
		...
		if (!myTache.isLeaf()) {
			if (myTache.getExecutor() == Executor.INCONNU) {
				CoherenceAdaptator.addWarningMessage(KMADEConstant.NO_EXECUTANT_SPECIFIED_MESSAGE_WARNING,
						HIERARCHICAL_TYPE, HIERARCHICAL_TASK_MODEL, myTache);
			} else {
					...
					for (Task subTasks : myTache.getChildren()) {
						if (subTasks.getExecutor() != myTache.getExecutor()) {
							CoherenceAdaptator.addWarningMessage(
									KMADEConstant.SUBTASKS_EXECUTANT_DIFFERENT_MESSAGE_WARNING, HIERARCHICAL_TYPE,
									HIERARCHICAL_TASK_MODEL, myTache);
							break;
						}
					}
