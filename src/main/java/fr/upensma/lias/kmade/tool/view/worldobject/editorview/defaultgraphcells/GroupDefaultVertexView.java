...
public abstract class GroupDefaultVertexView extends VertexView {
	...
	if (((Groupe) cell.getObject()).getEnsemble().getInverseGroupe()
		.getContientObj() == null)
	    this.title = " ";
	...
	ConcreteObjectCell empty = new ConcreteObjectCell(new ObjetConcret(),
		0, 0);
	...
	public void mousePressed(MouseEvent e) {
	    ...
	    int x = (int) (e.getX() - GraphConstants.getBounds(
		    cell.getAttributes()).getX());
	    int y = (int) (e.getY() - GraphConstants.getBounds(
	...
	public void mouseDragged(MouseEvent event) {
		...
		GraphConstants.setBounds(
			cell.getSelectedCell().getAttributes(),
			new Rectangle2D.Double(event.getX(), event.getY(),
				width, height));
	...
	public void mouseReleased(MouseEvent event) {
	    ...
	    if (GraphConstants.getBounds(cell.getAttributes()).contains(
		    event.getPoint())
		    && Graph.getSelectionCell() instanceof ConcreteObjectCell) {
		if (cell.getObject()
			.getContientObj()
			.equals((((ObjetConcret) ((ObjectDefaultGraphCell) Graph
				.getSelectionCell()).getObject())
				.getUtiliseParClass()))) {

		    if (!(cell instanceof SingletonCell)
			    || cell.getCellsInGroup().isEmpty()) {
			cell.addCellToGroup((ConcreteObjectCell) Graph
				.getSelectionCell());
		    ...
		    JOptionPane.showMessageDialog(this.Graph,
			    KMADEConstant.OBJECT_INSERTION_ERROR, "Error",
			    JOptionPane.ERROR_MESSAGE);
		}
	    }

	    else if (!GraphConstants.getBounds(cell.getAttributes()).contains(
		    event.getPoint())) {
