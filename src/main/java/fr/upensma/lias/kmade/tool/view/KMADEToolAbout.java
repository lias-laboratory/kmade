...
public class KMADEToolAbout extends JDialog implements LanguageFactory {
	...
	super(owner, KMADEConstant.ABOUT_TITLE_NAME + " "
		+ KMADEToolConstant.VERSION_VALUE, true);
	...
	panel_info.add(KMADEToolUtilities
		.getLabelCenter(KMADEToolConstant.TOOL_NAME + " "
			+ KMADEConstant.VERSION_MESSAGE + " "
			+ KMADEToolConstant.VERSION_VALUE));
	panel_info.add(KMADEToolUtilities.getLabelCenter("2005 - 2015"));
	panel_info
		.add(KMADEToolUtilities
			.getLabelCenter("INRIA Rocquencourt - Projet MErLIn - LIAS / ISAE-ENSMA"));
	...
	this.setTitle(KMADEConstant.ABOUT_TITLE_NAME + " "
		+ KMADEToolConstant.VERSION_VALUE);
	participants.setText("");
	participants.append(KMADEConstant.PROJECT_LEADER_MESSAGE + "\n");
	participants
		.append(" - Dominique Scapin (dominique.scapin@inria.fr) \n\n");
	participants.append(KMADEConstant.DEVELOPMENT_QUERING_TOOL_MESSAGE
		+ "\n");
	participants
		.append(" - Mickael Baron (baron@ensma.fr)\n\n");
	...
	librairies
		.append(" - QuickTime for Java (http://developer.apple.com/quicktime/qtjava)\n");
	librairies
		.append(" - Timingframework (http://timingframework.dev.java.net)\n");
	librairies.append(" - Velocity (http://jakarta.apache.org/velocity)\n");
	librairies
		.append(" - Swingstates (http://swingstates.sourceforge.net/)\n");
