...
public class Person extends User {
	 ...
	 * A verification is made for avoiding duplication Warning: the reverse
	 * registering is not done at this step
	 * 
	...
	/**
	 * Gives the list of organizations the individual is member of Warning: it is a
	 * reference on the list itself. Be careful not to modify it
	 * 
		...
		if (kmadIndividuOrganisation.item(0) != null) {
			for (int i = 0; i < kmadIndividuOrganisation.getLength(); i++) {
				this.addOrganization((Organization) InterfaceExpressJava.bdd
						.prendre(new Oid(kmadIndividuOrganisation.item(i).getTextContent())));
			}
		}
	}
	...
	public String toSPF() {
		...
		s += ")";
		return oid.get() + "=Individu('" + super.getName() + "','" + super.getStatus() + "','" + super.getRole() + "','"
				+ super.getImage() + "','" + s + "');";
	}
	 ...
	 *         a list of group name the individual is member of
	 */
		...
		if (p.hasAttribute("id-organisation")) {
			...
			for (int i = 0; i < kmadIndividuOrganisation.length; i++) {
				this.addOrganization(
						(Organization) InterfaceExpressJava.bdd.prendre(new Oid(kmadIndividuOrganisation[i])));
			}
		}
