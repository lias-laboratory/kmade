	...
	public void evaluateNode(ObjetConcret ref) throws SemanticException {
		...
		if (getLeftNode().isNumber() && getRightNode().isString()) {
			this.setNodeValue(new Boolean(((NumberValue) (getLeftNode().getNodeValue()))
					.equalOperator((new NumberValue((String) getRightNode().getNodeValue())))));
			return;
		}
		...
		if (this.getLeftNode().isBoolean() && this.getRightNode().isBoolean()) {
			this.setNodeValue(new Boolean(((Boolean) getLeftNode().getNodeValue())
					.booleanValue() == ((Boolean) getRightNode().getNodeValue()).booleanValue()));
			return;
		}

		if (this.getLeftNode().isBoolean() && this.getRightNode().isNumber()) {
			this.setNodeValue(new Boolean(((Boolean) getLeftNode().getNodeValue())
					.booleanValue() == (((NumberValue) getRightNode().getNodeValue())
							.equalOperator(new NumberValue("0")) ? true : false)));
			return;
		}

		if (this.getLeftNode().isNumber() && this.getRightNode().isBoolean()) {
			this.setNodeValue(new Boolean(((Boolean) getRightNode().getNodeValue())
					.booleanValue() == (((NumberValue) getLeftNode().getNodeValue()).equalOperator(new NumberValue("0"))
							? true
							: false)));
			return;
		}
