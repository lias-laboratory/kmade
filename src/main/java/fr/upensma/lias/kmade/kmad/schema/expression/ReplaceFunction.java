 ...
 */
public class ReplaceFunction extends BinaryFunction implements
	ConcreteObjectType {
    ...
    public ReplaceFunction(GroupExpressExpression left,
	    GroupExpressExpression right) {
	...
	ObjetAbstrait groupeAttributed = ((GroupExpressExpression) rightNode)
	...
	if (groupeStored.getName().equals(groupeAttributed.getName())) {
	    ...
	    Groupe g = (Groupe) InterfaceExpressJava
		    .prendre(((GroupExpressExpression) rightNode).getGroup()
			    .getOid());
		...
		etat = etat
			| concreteAttribut.setValeur(refUserObjetConcret
