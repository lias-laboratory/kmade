...
public class IterExpression extends Expression {
	...
	if (this.refNode instanceof NumberVariant) {
	    ...
	    // virgule
	    this.setIterationVariant(((Number) ((NumberValue) this.refNode
		    .getNodeValue()).getValeur()).intValue());
	} else {
	...
	if (refNode instanceof NumberVariant) {
	    return ((NumberValue) (refNode.getNodeValue()))
		    .supOperator(new NumberValue(0));
	} else {
