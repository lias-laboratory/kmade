...
public class Machine extends Material {
	...
	/**
	 * Enregistre que la machine fait partie du parcMachine parc Le parc ne sait pas
	 * que la machine en fait partie
	 * 
	 * @param park
	 */
		...
		if (kmadMachineParc.item(0) != null) {
			for (int i = 0; i < kmadMachineParc.getLength(); i++) {
				this.addToPark((ParkMachines) InterfaceExpressJava.bdd
						.prendre(new Oid(kmadMachineParc.item(i).getTextContent())));
			}
		}
	}
	...
	public String toSPF() {
		...
		s += ")";
		return oid.get() + "=Machine('" + super.getName() + "','" + super.getDescription() + "','" + isComputer()
				+ "','" + super.getImage() + "','" + s + "');";
	}
	...
	public void createObjectFromXMLElement2(org.w3c.dom.Element p) {
		...
		if (p.hasAttribute("id-parcMachine")) {
			...
			for (int i = 0; i < parc.length; i++) {
				this.addToPark((ParkMachines) InterfaceExpressJava.bdd.prendre(new Oid(parc[i])));
			}
		}
