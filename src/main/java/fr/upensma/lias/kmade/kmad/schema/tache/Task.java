...
public class Task implements Entity {
	...
	/**
	 * duration : String -> Task duration - can be of either form. If only a number,
	 * can be interpreted
	 */
	private String duration = "";

	/**
	 * description : String -> informal description of the task. Can be null, but
	 * not recommended
	 */
	...
	/**
	 * number : String -> computed value for the numbering of tasks: root, 1, 1.1,
	 * 1.1.1, etc. Warning: no setter, so setting numbers is made at several places
	 * !
	 */
	...
	/**
	 * mother : Task -> Super-task of the current task. Null if the task is root, or
	 * if it is not attached to the tree
	 */
	...
	/**
	 * actorSystem : ArrayList<actorSystem> -> System actors involved in the task
	 */
	...
	/**
	 * effetsDeBordExpression : EffetsDeBordExpression -> Side effects for the task
	 * (action)
	 */
	...
	/**
	 * noPoint : Boolean -> States if the point owns a valid graphical position or
	 * no. Default false
	 */
	...
	/**
	 * refJTask : GraphicCell reference of the graphical view of the task used to
	...
	public Task() {
		...
		/*
		 * this.stateSimulation = new StateSimulation(); this.stateSimulation = new
		 * StateSimulation();
		 */
	...
	/*
	 * public Tache( String name, String but, String res, String f, String duree,
	 * String obs, String exe, String freq, String comp, String imp, String mod,
	 * Evenement dec, ArrayList<Evenement> events, Boolean facultatif, Boolean
	 * interruptible, String decomposition, ArrayList<Acteur> act, ArrayList<Tache>
	 * fils, Point point, String prec, String post, String it, Oid oid) { this.name
	 * = name; this.but = but; this.ressources = res; this.feed = f; this.duree =
	 * duree; this.observation = obs; this.executant = Executant.getValue(exe);
	 ...
	 * this.facultatif = false; else this.facultatif = facultatif; if (interruptible
	 * == null) this.interruptible = false; else this.interruptible = interruptible;
	 * this.setactors(act); this.decomposition =
	 * Decomposition.getValue(decomposition); this.preExpression = new
	 * PreExpression(prec); this.effetsDeBordExpression = new
	 * EffetsDeBordExpression(post); this.iteExpression = new IterExpression(it);
	 * this.oid = oid; this.fils = new ArrayList<Tache>(); this.numero =
	 ...
	 * this.point = point;
	 * 
	 * // Le media pour rester compatible avec les anciennes versions qui exploitent
	 * SPF. idMedia = null;
	 * 
	...
	public boolean addActorSystem(ActorSystem a) {
		for (int i = 0; i < actorSystem.size(); i++) {
			...
			if (act.getMaterialRef().getOid().equals(a.getMaterialRef().getOid())) {
				return false;
			}
		}
		actorSystem.add(a);
	...
	/*
	 * public ArrayList<String> getActorSystemOid() { ArrayList<String> lst = new
	 ...
	 */
	/*
	 * public ArrayList<String> getActorSystemComp() { ArrayList<String> lst = new
	 * ArrayList<String>(); for (int i = 0; i < actorSystem.size(); i++) {
	...
	/**
	 * The index is the numbered position of the child, starting from 0 -1 if root
	 * task
	 * 
	...
	/**
	 * Cette m̩thode permet de supprimer une t̢che et ses liens si n̩cessaire.
	 * 
	 * @return
	 */
	 ...
	 *//*
		 ...
		 */
	/**
	 * returns the list of tasks which must be modified because of new coordinates
	 * 
	 * @param x
	 * @param y
			...
			for (int i = 0; i < nodeListEvents.getLength(); i++) {
				if (nodeListEvents.item(i).getNodeType() == Element.ELEMENT_NODE) {
					if (InterfaceExpressJava.bdd
							.prendre(new Oid(((Element) nodeListEvents.item(i)).getTextContent())) == null) {
						return true;
					}
				}
			}
		...
		if (nodeList.getLength() != 0) {
			...
			for (int i = 0; i < nodeListActors.getLength(); i++) {
				if (nodeListActors.item(i).getNodeType() == Element.ELEMENT_NODE) {
					if (InterfaceExpressJava.bdd
							.prendre(new Oid(((Element) nodeListActors.item(i)).getTextContent())) == null) {
						return true;
					}
				}
			}
		...
		if (nodeList.getLength() != 0) {
			...
			for (int i = 0; i < nodeListSubTasks.getLength(); i++) {
				if (nodeListSubTasks.item(i).getNodeType() == Element.ELEMENT_NODE) {
					if (InterfaceExpressJava.bdd
							.prendre(new Oid(((Element) nodeListSubTasks.item(i)).getTextContent())) == null) {
						return true;
					}
				}
			}
	...
	public void createObjectFromXMLElement(org.w3c.dom.Element p) {
		...
		nodeList = p.getElementsByTagName("id-task-media");
		if (nodeList.item(0) != null) {
			this.media = (Media) InterfaceExpressJava.bdd.prendre(new Oid(nodeList.item(0).getTextContent()));
		}
		...
		if (nodeList.item(0) != null) {
			Event event = (Event) InterfaceExpressJava.bdd.prendre(new Oid(nodeList.item(0).getTextContent()));
			...
			for (int i = 0; i < nodeListEvent.getLength(); i++) {
				if (nodeListEvent.item(i).getNodeType() == Element.ELEMENT_NODE) {
					Event event = (Event) InterfaceExpressJava.bdd
							.prendre(new Oid(nodeListEvent.item(i).getTextContent()));
			...
			for (int i = 0; i < nodeListActors.getLength(); i++) {
				if (nodeListActors.item(i).getNodeType() == Element.ELEMENT_NODE) {
					Actor acteur = (Actor) InterfaceExpressJava.bdd
							.prendre(new Oid(nodeListActors.item(i).getTextContent()));
			...
			for (int i = 0; i < nodeListSubTasks.getLength(); i++) {
				if (nodeListSubTasks.item(i).getNodeType() == Element.ELEMENT_NODE) {
					Task tache = (Task) InterfaceExpressJava.bdd
							.prendre(new Oid(nodeListSubTasks.item(i).getTextContent()));
		...
		// Point
		nodeList = p.getElementsByTagName("id-task-point");
		this.point = (Point) InterfaceExpressJava.bdd.prendre(new Oid(nodeList.item(0).getTextContent()));

		// Label
		nodeList = p.getElementsByTagName("id-task-label");
		if (nodeList.item(0) != null)
			this.label = (Label) InterfaceExpressJava.bdd.prendre(new Oid(nodeList.item(0).getTextContent()));
		...
		if (nodeList.item(0) != null) {
			this.sideEffectExpression = new SideEffectExpression(nodeList.item(0).getTextContent());
		}
		...
		if (nodeList.item(0) != null) {
			this.sideEffectExpression.setDescription(nodeList.item(0).getTextContent());
		}
		...
		this.iterExpression = new IterExpression(nodeList.item(0).getTextContent());

		// Iteration Description
		nodeList = p.getElementsByTagName("task-descriptioniteration");
		if (nodeList.item(0) != null) {
			this.iterExpression.setDescription(nodeList.item(0).getTextContent());
		}
	}
	 ...
	 */
	public String toSPF() {
		String SPF = oid.get() + "=" + "Tache" + "(" + "'" + name.replaceAll("'", "\\\\'") + "'" + "," + "'" + goal
				+ "'" + "," + "'" /* + resources + "'" */ + "," + "'" + feedback + "'" + "," + "'" + duration + "'"
				+ "," + "'" + description + "'" + "," + executor.toSPF() + "," + frequency.toSPF() + ",'"
				+ frequencyValue + "'," + importance.toSPF() + "," + modality.toSPF() + ",";
		if (raisingEvent != null)
	...
	/**
	 * @param facultatif The facultatif to set.
	 */
	...
	/**
	 * @param interruptible The interruptible to set.
	 */
	...
	/**
	 * @param refJTask The refJTask to set.
	 */
	...
	/**
	 * @param preExpression The preExpression to set.
	 */
	...
	public void setSideEffetExpression(SideEffectExpression effetsDeBordExpression) {
	...
	public Element toXML2(Document doc) throws Exception {
		...
		if (this.getPreExpression().getProtoTaskConditionExpression().getValue().getOid() != null) {
			...
			kmadElement.setTextContent(
					this.getPreExpression().getProtoTaskConditionExpression().getValue().getOid().get());
			racine.appendChild(kmadElement);
		}
		...
		if (!this.getEffetsDeBordExpression().getDescription().equals("")) {
			...
			kmadElement.setTextContent(this.sideEffectExpression.getDescription());
			racine.appendChild(kmadElement);
		}
		// Iteration
		...
		if (this.getIterExpression().getProtoTaskConditionExpression().getValue().getOid() != null) {
			...
			kmadElement.setTextContent(
					this.getIterExpression().getProtoTaskConditionExpression().getValue().getOid().get());
			racine.appendChild(kmadElement);
		}
		// Attributes
		...
		// Event trigger
		if (this.raisingEvent != null) {
			racine.setAttribute("id-task-eventtrigger", this.raisingEvent.getOid().get());
		}
		// Point
	...
	public void createObjectFromXMLElement2(Element p) throws Exception {
		...
		if (p.hasAttribute("id-task-media)"))
			this.media = (Media) InterfaceExpressJava.bdd.prendre(new Oid(p.getAttribute("id-task-media")));
		// Triggering Event
		...
		if (p.hasAttribute("id-task-actorSystem-list")) {
			...
			for (int i = 0; i < actorSys.length; i++) {
				this.addActorSystem((ActorSystem) InterfaceExpressJava.bdd.prendre(new Oid(actorSys[i])));
		...
		if (p.hasAttribute("id-task-subtasks-list")) {
			...
			for (int i = 0; i < set.size(); i++) {
				if (!noPoint)
					this.addSubTask((Task) InterfaceExpressJava.bdd.prendre(new Oid(set.get(i))));
				else {
					this.children.add((Task) InterfaceExpressJava.bdd.prendre(new Oid(set.get(i))));
					this.setMother((Task) InterfaceExpressJava.bdd.prendre(new Oid(set.get(i))));
				}
			}
		}
		...
		if (nodeList.item(0) != null) {
			...
			this.getPreExpression().getProtoTaskConditionExpression().setValue(
					(ProtoTaskCondition) InterfaceExpressJava.bdd.prendre(new Oid(nodeList.item(0).getTextContent())));
		}
		// EffetsDeBord
		...
		if (nodeList.item(0) != null) {
			...
			this.getIterExpression().getProtoTaskConditionExpression().setValue(
					(ProtoTaskCondition) InterfaceExpressJava.bdd.prendre(new Oid(nodeList.item(0).getTextContent())));
		}
