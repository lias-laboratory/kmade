...
public class Machine extends Materiel{
	...
	public void delete(){
		//supprimer la machine des parcs
	...
	/**
	 * Enregistre que la machine fait partie du parcMachine parc
	 * Le parc ne sait pas que la machine en fait partie
	 * @param parc
	 */
	...
	public org.w3c.dom.Element toXML(Document doc) {
		...
		if (!this.getDescription().equals("")) {
			Element kmadMachineDescription = doc.createElement("machine-description");
			kmadMachineDescription.setTextContent(this.getDescription());
			racine.appendChild(kmadMachineDescription);
		}

		Element kmadMachineDescription = doc.createElement("machine-isComputer");
		kmadMachineDescription.setTextContent(isComputer.toString());
		racine.appendChild(kmadMachineDescription);


		if (!super.getImage().equals("")) {
			Element kmadMachineImagePath = doc.createElement("machine-imagepath");
	...
	public void createObjectFromXMLElement(org.w3c.dom.Element p) {
		...
		if (kmadMachineDescription.item(0) != null)
			super.setDescription(kmadMachineDescription.item(0).getTextContent());

		NodeList kmadMachineIsComputer = p.getElementsByTagName("machine-isComputer");
		...
		if (kmadMachineParc.item(0) != null){
			for(int i =0;i<kmadMachineParc.getLength();i++){
				this.addToParc((ParcMachines)InterfaceExpressJava.bdd.prendre(new Oid(kmadMachineParc.item(i).getTextContent())));
			}
		}
	}
	...
	public String toSPF() {
		...
		s += ")";
		return oid.get() + "=Machine('" + super.getName() + "','" + super.getDescription() +"','"+ isComputer() + "','"+ super.getImage()+ "','" + s + "');";
	}
	...
	public Object[] toArray(){
		...
		Object[] res = {super.oid.get(), super.getName(), super.getDescription(),isComputer(),super.getImage(),s};
		return res;
	}
