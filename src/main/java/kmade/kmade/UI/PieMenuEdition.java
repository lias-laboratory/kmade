...
/** Pie Menu pour l'édition d'une tâche */
								...
								if (selection.length == 1) {
									...
									for (int i = 0; i < allViews.length; i++) {
										if (!selection[0].getClass().equals(
											...
											/*
											 * on vérifie que l'on est sur la
											 * tâche
											 */
		...
		interaction = new CStateMachine() {
			...
			public State menuOff = new State() {
				...
				public void leave() {
					...
					/*
					 * on affiche l'item d'"édition complète" seulement si on
					 * n'a pas de sélection multiple et l'item "coller"
					 * seulement si on a des tâches copiées ou coupées
					 */
				...
				Transition select = new Release(BUTTON1, ">> menuOff") {
					public void action() {
						...
						if (lastItemVisited != null) {
							...
							/*
							 * on resélectionne la tache sélectionnée avant que
							 * le clic gauche ne l'enlève
							 */
							if (selection != null) {
								GraphicEditorAdaptator
										.getMainFrame();
