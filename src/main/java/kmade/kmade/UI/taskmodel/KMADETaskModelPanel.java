...
import kmade.kmade.UI.PieMenuCreateTask;
import kmade.kmade.UI.PieMenuEditDecomposition;
import kmade.kmade.UI.PieMenuEditExecutant;
import kmade.kmade.UI.PieMenuEdition;
...
public class KMADETaskModelPanel extends JPanel {
	...
	//unused
	//private static final ImageIcon COLLAPSE_TASK_IMAGE_ICON = new ImageIcon(
	...
	// unused private JScrollPane scroll;

	// rajouter///
	private double zoom;

	/**
	 * Retourne la rï¿½fï¿½rence de la vue du graph.
	 * 
	 * @return
	 */
	...
	/**
	 * Recherche une tï¿½che dans l'arbre de tï¿½che portant l'oid OID.
	 * 
	 * @param oid
	 * @return
	 */
	...
	/**
	 * Supprime tous les ï¿½lï¿½ments graphique du modï¿½le de tï¿½ches.
	 */
	...
	/**
	 * Retourne la cellule sï¿½lectionnï¿½e.
	 * 
	 * @return
	 */
	...
	/**
	 * Effectue un Zoom dï¿½fault sur le graph.
	 * 
	 * @param p
	 */
	...
	static class NMDAGraphSelectionModel extends DefaultGraphSelectionModel
		...
		public void valueChanged(GraphSelectionEvent arg0) {
			...
			if (!enabled) {
				...
				// Compte vraiment le nombre d'ï¿½lï¿½ments.
				if (this.getSelectionCount() == 0) {
					...
					// Avant d'envoyer ï¿½ l'AdaptateurEditeurGraphic faire un
					// tri sur les ï¿½lï¿½ments cachï¿½s.
					Object[] toto = NMDAGraphSelectionModel.this
							.getSelectionCells();
	...
	public class NMDAMarqueeHandler extends BasicMarqueeHandler {
		...
		/*
		 * Cette mï¿½thode permet d'activer ou de dï¿½sactiver le dï¿½placement.
		 * Typiquement la sï¿½lection d'un port retourne vraie.
		 */
				...
				if (GraphicEditorAdaptator.addNewTask(new Point(e.getX(), e
					...
					if (thread == null) {
						...
						if (!interupt) {
							...
							super
									.setMarqueeBounds(new Rectangle2D.Double(
											startPoint.getX(), startPoint
													.getY(), 0, 0));
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (e != null && port != null && firstPort != null
					&& firstPort != port) {
				// Normalement en lachant on devrait crï¿½er la connexion.
				// A vï¿½rifier si on connecte au bon endroit mï¿½re fils ou
				// fils mï¿½re d'une tï¿½che diffï¿½rente
				// et que la connexion n'est pas dï¿½jï¿½ faite
	...
	public class MyBasicGraphUI extends BasicGraphUI {
			...
			if (graph.isCellEditable(cell)) {
				...
				if (cellEditor.isCellEditable(event)) {
					...
					Point2D p = getEditorLocation(cell, editorSize, graph
							.toScreen(new Point2D.Double(cellBounds.getCenterX(),
					...
					editingComponent.setBounds((int) p.getX(), (int) p
							.getY()
		...
		// Le plus haut niveau. Il dispatch et il gï¿½re la sï¿½lection
		class MyMouseHandler extends MouseHandler {
			 ...
			 * <code>c</code> in the specified array of views. The views must
			 * be in order, as returned, for example, by
		...
		class MyRootHandle extends RootHandle {
			...
			public void mouseClicked(MouseEvent e) {
				...
				for (int i = 0; i < views.length; i++) {
					if (views[i] instanceof KMADEVertexView) {
						...
						{
							((KMADEVertexView.MyCellHandle)((KMADEVertexView)views[i]).getHandle(context)).mouseClicked(e);
						}
					}
				}
			...
			public void mousePressed(MouseEvent event) {
				if (SwingUtilities.isRightMouseButton(event)) {
					JPopupMenu menu = GraphicEditorAdaptator.showPopupMenu(event
							.getPoint());
					if (menu != null) {
