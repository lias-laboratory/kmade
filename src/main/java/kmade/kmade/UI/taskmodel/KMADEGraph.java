...
import kmade.kmade.UI.taskmodel.KMADEVertexView.MyVertexRenderer;
	...
	public KMADEGraph() {
		...
		MouseInputAdapter mia = new MouseInputAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				...
				// si on bouge on arrête le thread
				try {
					thread.interrupt();
					interupt = true;
					thread = null;
		...
		addMouseWheelListener(new MouseWheelListener() {
			public void mouseWheelMoved(MouseWheelEvent e) {
				 ...
				 * et dézoomer lorsqu'on la bouge vers le bas
				 */
					...
					if (getSelectionCell() instanceof KMADEDefaultGraphCell)
					{
						for (int i = 0; i<views.length; i++)
						{
							if (views[i] instanceof KMADEVertexView)
							{
								...
								{
									Point p = new Point((int)((KMADEVertexView)views[i]).getBounds().getCenterX(), (int)((KMADEVertexView)views[i]).getBounds().getCenterY());
									setScale(zoom, p);
									return;
								}
							}
	...
	public String getToolTipText(MouseEvent event) {
		...
		if (select != null) {
			...
			if (select.getRectPre() != null
				...
				/* S'il y a une description textuelle, on l'affiche
				 * Sinon, on affiche la condition telle qu'elle est écrite */
				if (((KMADEDefaultGraphCell) cell).getTask()
						.getPreExpression().getDescription().equals("")) {
					return (KMADEConstant.VERTEX_PRECONDITION+ ": " + ((KMADEDefaultGraphCell) cell).getTask()
			...
			if (select.getRectRep() != null
				...
				if (((KMADEDefaultGraphCell) cell).getTask()
						.getIteExpression().getDescription().equals("")) {
					return (KMADEConstant.VERTEX_ITERATION+": " + ((KMADEDefaultGraphCell) cell).getTask()
						.getIteExpression().toSPF().toString());
				}
				return (KMADEConstant.VERTEX_ITERATION+": " + ((KMADEDefaultGraphCell) cell).getTask()
						.getIteExpression().getDescription());
			}
			/* Evt In ok! */
			if (select.getRectEvenementIn() != null
					&& select.getRectEvenementIn().contains(event.getPoint())) {
				return (KMADEConstant.VERTEX_EVENT_IN_LETTER + ((KMADEDefaultGraphCell) cell).getTask()
						.getDeclencheurName());
			}
			...
			if (select.getRectPost() != null
				...
				if (((KMADEDefaultGraphCell) cell).getTask()
						.getEffetsDeBordExpression().getDescription().equals("")) {
						return (KMADEConstant.VERTEX_EFFETSDEBORD+": "+ ((KMADEDefaultGraphCell) cell).getTask()
						.getEffetsDeBordExpression().toSPF().toString());
				}
				return (KMADEConstant.VERTEX_EFFETSDEBORD+": " + ((KMADEDefaultGraphCell) cell).getTask()
