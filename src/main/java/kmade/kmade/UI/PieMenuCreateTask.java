...
/** Pie Menu pour la création des tâches */
public class PieMenuCreateTask extends Menu {
	...
	public PieMenuCreateTask(Canvas c) {
			...
			public State initialState = new State() {
				...
				Transition enterJGraph = new EnterOnComponent(">> initialState") {
					public boolean guard() {
						return ((getComponent() == KMADEMainFrame.getProjectPanel()
								.getTaskModelPanel().getJGraph()) && KMADEMainFrame
				...
				Transition pressOnJGraph = new PressOnComponent(BUTTON3,
					...
					public boolean guard() {
						...
						/* on vérifie que l'on n'est pas au-dessus d'une tâche */
						...
						for (int i = 0; i < allViews.length; i++) {
							if (allViews[i] instanceof KMADEVertexView) {
								...
								rectTask.setRect(rectTask.getX() * scale,
										rectTask.getY() * scale, rectTask
												.getWidth()
												* scale, rectTask.getHeight()
												* scale);
				...
				Transition pressOnTabpane = new PressOnComponent(BUTTON1,
					...
					public void action() {
						...
						int firstSeparation = (int) KMADEMainFrame.getProjectPanel()
								.getTabpaneProjet().getBoundsAt(1).getY();
						int secondSeparation = (int) KMADEMainFrame
								.getProjectPanel().getTabpaneProjet()
								.getBoundsAt(2).getY();
						int thirdSeparation = (int) KMADEMainFrame.getProjectPanel()
								.getTabpaneProjet().getBoundsAt(3).getY();
						int fourthSeparation = (int) KMADEMainFrame
								.getProjectPanel().getTabpaneProjet()
								.getBoundsAt(4).getY();
						int fifthSeparation = (int) KMADEMainFrame.getProjectPanel()
								.getTabpaneProjet().getBoundsAt(5).getY();
		...
		interaction = new CStateMachine() {
			...
			public State menuOff = new State() {
				 ...
				 * est généré par la JStateMachine
				 */
				Transition pressToDisplay = new Press(BUTTON3, ">> menuOn") {
					...
					public void action() {
						...
						// on récupère l'information concernant la position de la souris à l'écran pour calculer le décalage via l'information réç via get point
						...
						Point location = new Point ((int)(locationOnScreen.getX()-locationWindows.getX()),(int)(locationOnScreen.getY()-locationWindows.getY()));
			...
			public State menuOn = new State() {
				...
				Transition changeItem = new EnterOnTag(MenuItem.class) {
					public void action() {
						...
						// timer pour l'affichage de la création du menu
						armTimer(10, false);
					}
				};
					...
					public void action() {
						if (tooltip) {
							if (lastItemVisited != null) {
								 ...
								 * l'extérieur du menu
								 */
								Point2D p = new Point2D.Double(pointToolTip
										.getX() + 10, pointToolTip.getY() - 10);
								...
								for (int i = 0; i < items.length; i++) {
									...
									if (items[i].equals(lastItemVisited)) {
										if (Math.cos((i * angleStep)
												...
												&& Math.sin((i * angleStep)
														+ angleStep / 2) > -0.6) {
											p = new Point2D.Double(pointDisplay
													.getX()
	...
	void menuLayout(String[] items) {
		//an ellipse, specified by its bounding box(upper left corner, width and height)
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (items[i].equals(KMADEConstant.PASTE_ACTION_MESSAGE)) {
				...
				labelItem = (CImage) coller.setTransparencyOutline(0)
						.setPickable(false).addTag(tagWhole).addTag(tagLabels)
