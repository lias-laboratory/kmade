...
public class KMADEPrintingTaskModelPanel extends JPanel {
    ...
    private Image up = new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.UP_ARROW_IMAGE)).getImage();

    private Image down = new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.DOWN_ARROW_IMAGE)).getImage();
    
    private Image left = new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.LEFT_ARROW_IMAGE)).getImage();
    
    private Image right = new ImageIcon(GraphicEditorAdaptator.class.getResource(KMADEConstant.RIGHT_ARROW_IMAGE)).getImage();    
    ...
    public KMADEPrintingTaskModelPanel(KMADEPrintingDialog pdialog) {
		...
		zoomInAction = new AbstractAction("", KMADETaskModelToolBar.ZOOMIN_IMAGE_ICON) {
			...
			public void actionPerformed(ActionEvent e) {				
				KMADEPrintingTaskModelPanel.this.setTaskModelZoom(scaleGraph * 1.5d);
			}
		};
		...
		panelPreview = new JPanel() {
            ...
            public Dimension getPreferredSize() {
                if (buffer_image == null) {
                    ...
                    return new Dimension(buffer_image.getTileWidth(), buffer_image.getTileHeight());
                }
            }  
	...
	private static Rectangle getRealJGraphScaleSize(Rectangle current,JGraph myGraph, double scale) {
    ...
    class JGraphExtendedView extends JGraph {
        ...
        public void paint(Graphics g) {
            ...
            double wp = refDialog.getPageFormat().getImageableWidth() * scaleView;
            double hp = refDialog.getPageFormat().getImageableHeight() * scaleView;

            // Les dimensions du modèle de tâches.
            Rectangle myGraphBounds = 
            	KMADEPrintingTaskModelPanel.getRealJGraphScaleSize(jgraphSize, myGraph, scaleGraph);
            ...
            for (int i = 0; i < nh; i++) {
                for (int j = 0; j < nw; j++) {
                    ...
                    g2d.drawString(namePage + text,(int)(currentx + wp - (width + textHGap)),(int)(currenty + hp - textVGap));
                    ...
                    if (j + 1 < nw) {                     
                        ...
                        g2d.drawString(namePage + text,(int)(currentx + wp ) - ( width + rightHGap ), (int)( currenty + ( hp/2 )) + (heigh/2) - rightVGap);
                        g2d.drawImage(right,(int)(currentx + wp) - widthIcon ,(int)(currenty + (hp/2)) - (heightIcon / 2) , widthIcon, heightIcon, null);
                    }
                    ...
                    if (i + 1 < nh) {
                        ...
                        bounds = g2d.getFont().getStringBounds(text, frc);
                        width = (int)bounds.getWidth();
                        g2d.drawString(namePage + text, (int)(currentx + (wp/2) - (width + bottomHGap)),(int)(currenty + hp) - (heigh + bottomVGap));
                        g2d.drawImage(down,(int)(currentx+wp/2) - (widthIcon / 2),(int)(currenty+hp) - (heightIcon), widthIcon, heightIcon, null);
                    }
                    ...
                    if (i != 0) {
                        ...
                        bounds = g2d.getFont().getStringBounds(text, frc);
                        width = (int)bounds.getWidth();
                        g2d.drawString(namePage + text, (int)(currentx + (wp/2) - (width + topHGap)),(int)(currenty) + (heigh + topVGap));
                        g2d.drawImage(up,(int)(currentx + wp/2) - (widthIcon / 2),(int)(currenty),widthIcon, heightIcon, null);
                    }
                    ...
                    if (j != 0) {
                        ...
                        bounds = g2d.getFont().getStringBounds(text, frc);
                        width = (int)bounds.getWidth();
                        g2d.drawString(namePage + text, (int)currentx + leftHGap,(int)(currenty + (hp/2)) + (heigh/2) - leftVGap);
                        g2d.drawImage(left,(int)currentx,(int)(currenty + (hp/2)) - (heightIcon / 2),widthIcon, heightIcon, null);
                    }                   
	...
	public void makeTraitement() {
		...
		// Les dimensions du modèle de tâches.
		Rectangle myGraphBounds = KMADEPrintingTaskModelPanel.getRealJGraphScaleSize(jgraphSize,myGraph, myGraph.getScale());
		...
		for (int i = 0; i < nh; i++) {
			for (int j = 0; j < nw; j++) {
                BookComponentPrintable toto = new BookComponentPrintable(myGraph, currentx, currenty);
    ...
    class BookComponentPrintable implements Printable {
        ...
        public int print(Graphics graphics, PageFormat pageFormat, int pageIndex)
                ...
                int width = (int)(pageFormat.getImageableX()+pageFormat.getWidth() + 5);
                int heigh = (int)(pageFormat.getImageableY()+pageFormat.getHeight() + 5);
                BufferedImage buffer_image = new BufferedImage(width,heigh, BufferedImage.TYPE_INT_RGB);                
                Graphics2D im = buffer_image.createGraphics();

                AffineTransform at = AffineTransform.getTranslateInstance(pageFormat.getImageableX()- x - 2,pageFormat.getImageableY() - y - 2);
                im.transform(at);
