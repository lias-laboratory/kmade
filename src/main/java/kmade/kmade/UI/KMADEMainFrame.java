...
public class KMADEMainFrame extends JFrame implements LanguageFactory {
    ...
    private KMADEClipBoardDialog clipboardDialog = new KMADEClipBoardDialog(this);
    
    private KMADEProgressBarPanel progressBarDialog = new KMADEProgressBarPanel();
    
    private KMADEPreferencesDialog preferencesDialog = new KMADEPreferencesDialog(this);
    
    private KMADESimulationDialog simulationDialog = new KMADESimulationDialog();
    
    private KMADEStatisticDialog statisticDialog = new KMADEStatisticDialog(this);
    
    private KMADECoherenceDialog coherenceDialog = new KMADECoherenceDialog(this);
    
    private KMADEEntityDialog entityDialog = new KMADEEntityDialog(this);
    
    private KMADEFindReplaceDialog findReplaceDialog = new KMADEFindReplaceDialog(this);
	...
	public static final ImageIcon INRIA_LOGO = new ImageIcon(KMADEMainFrame.class.getResource(KMADEConstant.INRIA_LOGO_IMAGE));  

	public static final ImageIcon MERLIN_LOGO = new ImageIcon(KMADEMainFrame.class.getResource(KMADEConstant.MERLIN_LOGO_IMAGE)); 
	...
	public KMADEMainFrame() {
		...
		this.setIconImage(new ImageIcon(KMADEMainFrame.class.getResource(KMADEConstant.ICON_LOGO_IMAGE)).getImage());
        previewDialog.setOverviewGraph(PROJECT_PANEL.getTaskModelPanel().getJGraph());
		...
		panelEmpty = new JPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				g.drawImage(INRIA_LOGO.getImage(), w - (100 + gapw),h - 22 - 73 - gaph - gaph, 100 ,22, this);// 50,100
				g.drawImage(MERLIN_LOGO.getImage(), w - (100 + gapw),h - 73 - gaph, 100 ,73, this);// 90,100
			}
		...
		panelInfo.add(BorderLayout.NORTH,new JLabel(new ImageIcon(KMADEMainFrame.class.getResource(KMADEConstant.ICON_LOGO_IMAGE)), JLabel.CENTER));
		...
		myNewProject.setIcon(new ImageIcon(KMADEMainFrame.class.getResource(KMADEConstant.NEW_PROJECT_BIG_IMAGE)));
		myNewProject.setText("");
		panelNew.add(BorderLayout.CENTER, myNewProject);
		JLabel newProjectLabel = new JLabel(APPLICATION_TOOL_BAR.getProjectAction().getValue(AbstractAction.NAME).toString(), JLabel.CENTER);
		...
		myOpenProject.setIcon(new ImageIcon(KMADEMainFrame.class.getResource(KMADEConstant.OPEN_PROJECT_BIG_IMAGE)));
		myOpenProject.setText("");
		
		panelOpen.add(BorderLayout.CENTER, myOpenProject);
		JLabel openLabel = new JLabel(APPLICATION_TOOL_BAR.getLoadAction().getValue(AbstractAction.NAME).toString(), JLabel.CENTER);
        ...
        myOutputTabbedPane.add(KMADEConstant.ERREURS_MESSAGE,outputErrorMessage);
        ...
        JSplitPane jSplitPaneBH = new JSplitPane();
        ...
        previewDialog.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent e) {
                KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getTaskModelToolBar().setSelectedOverviewWindowToggleButton(false);
            }            
        });
