...
import kmade.kmade.UI.taskmodel.KMADETaskModelToolBar;
...
public class KMADEReplaySimulationPanel extends JPanel implements LanguageFactory {
    ...
    private static final ImageIcon PLAYER_PLAY_IMAGE = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.PLAYER_PLAY_IMAGE));

    private static final ImageIcon PLAYER_STOP_IMAGE = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.PLAYER_STOP_IMAGE));

    private static final ImageIcon PLAYER_PAUSE_IMAGE = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.PLAYER_PAUSE_IMAGE));

    public static final ImageIcon PLAYER_REW_IMAGE = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.PLAYER_REW_IMAGE));

    public static final ImageIcon PLAYER_FWD_IMAGE = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.PLAYER_FWD_IMAGE));
    
    public static final ImageIcon SAVEAS_SCENARIO_IMAGE = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.SAVEAS_SCENARIO_IMAGE));
        
    private static final ImageIcon ONE_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.ONE_STEP_IMAGE));

    private static final ImageIcon TWO_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.TWO_STEP_IMAGE));

    private static final ImageIcon THREE_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.THREE_STEP_IMAGE));

    private static final ImageIcon FOUR_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.FOUR_STEP_IMAGE));

    private static final ImageIcon FIVE_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.FIVE_STEP_IMAGE));

    private static final ImageIcon SIX_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.SIX_STEP_IMAGE));

    private static final ImageIcon SEVEN_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.SEVEN_STEP_IMAGE));

    private static final ImageIcon HEIGH_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.HEIGH_STEP_IMAGE));

    private static final ImageIcon NINE_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.NINE_STEP_IMAGE));

    private static final ImageIcon TEN_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.TEN_STEP_IMAGE));

    private static final ImageIcon ELEVEN_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.ELEVEN_STEP_IMAGE));

    private static final ImageIcon TWELVE_TIME = new ImageIcon(KMADETaskModelToolBar.class.getResource(KMADEConstant.TWELVE_STEP_IMAGE));
    
    private static final int[] speed = {3500,3000,2500,2000,1500,1000,500};
    ...
    public KMADEReplaySimulationPanel() {
        ...
        myTaskList.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
        ...
        myTaskList.setCellRenderer(new ListCellRenderer() {
            public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            		...
            		} else if (((TokenReplayScenarioSimulation)value).isErrorState()) {
            			myColor = Color.RED;
            		}
            		...
            		JLabel myLabel = new JLabel((index + 1) + " - "+ (value == null ? "" : value.toString()));
        ...
        this.myTaskListScrollBorder = new TitledBorder(null, KMADEConstant.SCENARIO_REPLAY_MESSAGE, TitledBorder.CENTER, TitledBorder.TOP);
        ...
        this.myScenarioListScrollBorder = new TitledBorder(null, KMADEConstant.RECORD_BUILDING_SCENARION_MESSAGE, TitledBorder.CENTER, TitledBorder.TOP);
        ...
        JScrollPane myAvailableActionListScroll = new JScrollPane(availableActionList);
        this.myAvailableActionoListScrollBorder = new TitledBorder(null, KMADEConstant.RECORD_AVAILABLE_ACTIONABLE_MESSAGE, TitledBorder.CENTER, TitledBorder.TOP);
        myAvailableActionListScroll.setBorder(myAvailableActionoListScrollBorder);
        ...
        myGestion.add(openScenario = new JButton(KMADEToolToolBar.OPEN_PROJECT));
        ...
        nextAction.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                // Ce bouton devrait être désactivé quand le scénario est terminé, mais nous ne savons jamais ...
                if (currentReplaySimulation < myReplayModel.getSize()) {
                    SimulationAdaptator.fireNextActionReplaySimulation(((TokenReplayScenarioSimulation)myReplayModel.get(currentReplaySimulation)));
                }
            }
        });
        ...
        this.myReplayOptionsBorder = new TitledBorder(null, KMADEConstant.REPLAY_OPTIONS_MESSAGE, TitledBorder.LEFT, TitledBorder.TOP);
        playOptionPanel.setBorder(this.myReplayOptionsBorder);
        userValues = new JCheckBox(KMADEConstant.REPLAY_KEEP_USER_VALUES_MESSAGE);
        ...
        this.myTempoBorder = new TitledBorder(null, KMADEConstant.REPLAY_TEMPO_MESSAGE, TitledBorder.LEFT, TitledBorder.TOP);
        ...
        panelRegulation.add(BorderLayout.SOUTH, Box.createRigidArea(new Dimension(0,5)));
        ...
        mySlider.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent e) {
                if (waitController != null) {
                	waitController.setCycle(new Cycle(speed[mySlider.getValue()],30));
                }
            }            
        });
        
        Hashtable<Integer,JLabel> labelTable = new Hashtable<Integer,JLabel>();
        labelTable.put(new Integer(0), new JLabel(KMADEConstant.REPLAY_SLOW_TEMPO));
        labelTable.put(new Integer(3), new JLabel(KMADEConstant.REPLAY_MODERATE_TEMPO));
        labelTable.put(new Integer(6), new JLabel(KMADEConstant.REPLAY_SPEED_TEMPO));
    ...
    public ArrayList<Tache> getAvailableTaches() {
    		...
    		for (int i = 0 ; i < this.myAvailableActionModel.size() ; i++) {
    			temp.add(((TokenSimulation)(this.myAvailableActionModel.get(i))).getTask());
    		}
    		return temp;
    }
    ...
    public ArrayList<TokenRecordScenarioSimulation> getReplayScenarioList() {
        ...
        for (int i = 0 ; i < this.myScenarioReplayModel.size() ; i++) {
            myList.add((TokenRecordScenarioSimulation)this.myScenarioReplayModel.get(i));             
        }
        return myList;        
    }
    ...
    public void updateListEnabledTask(ArrayList<TokenSimulation> my) {
        ...
        if (this.myAvailableActionModel.size() != 0) {
            this.availableActionList.getSelectionModel().setSelectionInterval(0,0);
        }
    }
    
    public void updateReplayListTask(ArrayList<TokenReplayScenarioSimulation> list) {
    ...
    public void startWaitController() {
        if (waitController == null || !waitController.isRunning()) {
            ...
            PropertyRange range = PropertyRange.createPropertyRangeInt("current", 0, 11);
            waitController = new TimingController(cycle, envelope, new ObjectModifier(myLabel, range));
            waitController.addTimingListener(new TimingListener() {
				...
				public void timerRepeated(TimingEvent arg0) {
					waitController.stop();
					waitController = null;
					SimulationAdaptator.playTask(((TokenReplayScenarioSimulation)myReplayModel.get(currentReplaySimulation)));
				}        		
        	});
