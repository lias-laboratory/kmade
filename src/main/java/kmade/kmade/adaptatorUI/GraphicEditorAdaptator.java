...
//    }
    ...
    /**
     * Cette m�thode permet d'effectuer un lien graphique entre deux t�ches dans
     * le presse-papier.
     * 
     * @param oidMere
     * @param oidFils
    ...
    public static boolean addNewEdge(KMADEDefaultPort source, KMADEDefaultPort target) {
        ...
        if (source != null && target != null) {
            if ((source.getParent() instanceof KMADEDefaultGraphCell)  && (target.getParent() instanceof KMADEDefaultGraphCell)) {
                // Règle 1 : ne pas connecter le port "mother" avec le port "son" de la même tâche.
                if (source.getParent() == target.getParent()) {
                    // Règle 2 : ne pas connecter un port "mother" avec un autre port "mother"
                } else if (source.isMotherPort() && target.isMotherPort()) {
                    // Règle 3 : ne pas connecter un port "son" avec un autre port "son"
                } else if (source.isSonPort() && target.isSonPort()) {
                    // Règle 4 : pas de connexion d'un port "mother" avec un port "son" déjà connecté
                } else if (source.isMotherPort() && (target.getEdges().size() != 0)) {
                    // Règle 5 : pas de connexion d'un port "son" déjà connecté à un port "mother"
    ...
    public static void removeTaskAndEdge() {
        ...
        if (cellSelected.length == 0) {
            ...
            // L'optimisation propos�e ici vient du fait que dans les �l�ments
            // s�lectionn�e
            // il peut y avoir des liens qui ont pu �tre supprim�s avant. Donc
            // pour un lien
    ...
    public static void selectNoTask() {
        myDefaultGraphCellRef = null;
        // D�sactivation des actions associ�es � la non s�lection d'une t�che.
    ...
    /**
     * Cette méthode s'occupe de la mise à jour de la partie graphique après chargement d'un fichier SPF.
     */
    ...
    public static void copyAction() {
        ...
        //System.out.println("Ajout d'une t�che dans le clipBord  "+ cellSelected.length);
