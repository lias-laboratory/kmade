...
public class KMADEReadMachineNotInParc extends JScrollPane implements LanguageFactory {

	private static final long serialVersionUID = 8609087968641570586L;

	private KMADEJTable table;

	private myModel modele;

	public KMADEReadMachineNotInParc(){
		modele = new myModel();
			...
			public void keyTyped(KeyEvent ke) { 
				...
				if (ke.getKeyChar() == KeyEvent.VK_DELETE || ke.getKeyChar() == KeyEvent.VK_ENTER) {
					...
					for(int i =0;i<r.length;i++){
						...
						KMADEMainFrame.getProjectPanel().getParcMachinesPanel().addMachineInParcMachines(oidMachine);
					}
					KMADEMainFrame.getProjectPanel().getParcMachinesPanel().refreshActiveParcMachines();
	...
	public void setMachineMemberNameBorder(String name) {
		this.setBorder(new TitledBorder(null, KMADEConstant.PARCMACHINES_OTHERS +" : "+KMADEConstant.PARCMACHINES_MACHINE_TABLE + " " + name,
	...
	public void updateDataModel(Object[][] o) {
		...
		//les parcs 	{oid, name, description,iscomput,imagepath, machines du parc}
