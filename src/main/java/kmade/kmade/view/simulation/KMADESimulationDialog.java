...
import kmade.kmade.view.taskmodel.KMADEPreviewWindow;
...
import kmade.kmade.view.toolutilities.VerticalTextIcon;
...
public class KMADESimulationDialog extends JFrame {
    ...
    private kmade.kmade.view.taskmodel.KMADEGraph mySimulationGraph;
    ...
    public KMADESimulationDialog() {        
        ...
        tabpaneProjet.addTab(null, new VerticalTextIcon(KMADEConstant.RECORD_TITLE_MESSAGE, false,myRecordingPanel), myRecordingPanel);
        tabpaneProjet.addTab(null, new VerticalTextIcon(KMADEConstant.REPLAY_TITLE_MESSAGE, false,myReplayPanel), myReplayPanel);
        
        JSplitPane myCentralSplitPane = KMADEEnhancedSplitPane.createStrippedSplitPane(JSplitPane.HORIZONTAL_SPLIT, myScrollGraphPane, tabpaneProjet);
        ...
        panelReplayEmpty.add(new JLabel(KMADEConstant.NO_SELECTED_SCENARIO_ACTION_FOR_CONSTRAINTS_MESSAGE,JLabel.CENTER));
        ...
        this.myConstraintsBorder = new TitledBorder(null, KMADEConstant.CONSTRAINTS_TITLE_MESSAGE, TitledBorder.CENTER, TitledBorder.TOP);
        panelConstraints.setPreferredSize(new Dimension(100,80));
        panelConstraints.setBorder(this.myConstraintsBorder);

        JSplitPane myConstraintsHistory = KMADEEnhancedSplitPane.createStrippedSplitPane(
        		JSplitPane.HORIZONTAL_SPLIT, panelConstraints, myHMP);
        ...
        protertiesTable = new JPropertiesTable(KMADEMainFrame.getProjectPanel().getTaskDescriptionPanel().getProprieteTache().getPropertiesModel());
        ...
        this.myPropertiesBorder = new TitledBorder(null, KMADEConstant.PROPERTIES_MESSAGE, TitledBorder.CENTER, TitledBorder.TOP);
        ...
        this.myEventBorder = new TitledBorder(null, KMADEConstant.EVENT_TITLE_MESSAGE, TitledBorder.CENTER, TitledBorder.TOP);
        ...
        JSplitPane executantEventConcreteObject = KMADEEnhancedSplitPane.createStrippedSplitPane(
        ...
        JSplitPane executantEventConcreteObjectContraints = KMADEEnhancedSplitPane.createStrippedSplitPane(
        		JSplitPane.HORIZONTAL_SPLIT,
        		executantEventConcreteObject,
                myEventPanel);
...
//				if (myRecordingPanel.getActionList().getSelectedIndex() != -1) {
...
//				}
        ...
        this.myUserBorder = new TitledBorder(null, KMADEConstant.EXECUTING_USER_TITLE_MESSAGE, TitledBorder.CENTER, TitledBorder.TOP);
        ...
        JSplitPane upDownSplitPane = KMADEEnhancedSplitPane.createStrippedSplitPane(
              JSplitPane.VERTICAL_SPLIT, 
              executantEventConcreteObjectContraintsProperties, 
              myMainSplitPane);
            ...
            if (!enabled) {
                ...
                if (this.getSelectionCount() == 0) {
                    ...
                    for (int i = 0 ; i < myRecordingPanel.getActionListModel().getSize() ; i++) {
                        if (!find && ((TokenSimulation)myRecordingPanel.getActionListModel().getElementAt(i)).getTask().getJTask() == toto) {                        
                            firstIndex = i;
                            find = true;
                        }
                    }    
                    ...
                    if (find) {
                        ...
                        if (!myRecordingPanel.getActionList().isSelectionEmpty()) {
                            if (((TokenSimulation)myRecordingPanel.getActionList().getSelectedValue()).getTask().getJTask() == toto) {
                                return;
                            } else {
                                myRecordingPanel.getActionList().getSelectionModel().setSelectionInterval(firstIndex,firstIndex);
                                return;                                
                            }
                        } else {
                            myRecordingPanel.getActionList().getSelectionModel().setSelectionInterval(firstIndex,firstIndex);
    ...
    static class ConstraintExecuteTaskTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            if (columnIndex == 0) {
                switch(rowIndex) {
                case 0 : return KMADEConstant.UTILISATEUR_LABEL_NAME;
                case 1 : return KMADEConstant.PRECONDITION_LABEL_VALUE;
                case 2 : return KMADEConstant.DECLENCHEMENT_LABEL_NAME;
                }
            } else {
            		switch(rowIndex) {
            		case 0 : return executant[columnIndex - 1];
            		case 1 : return precondition[columnIndex - 1];
            		case 2 : return evenement[columnIndex - 1];
            		}
            }
            return null;
        }
        ...
        public String getColumnName(int column) {
            switch(column) {
                case 0 : return KMADEConstant.CONSTRAINTS_NAME_COLUMN_NAME;
                case 1 : return KMADEConstant.CONSTRAINTS_VALEUR_COLUMN_NAME;
                case 2 : return KMADEConstant.CONSTRAINTS_STATE_COLUMN_NAME;
            }
