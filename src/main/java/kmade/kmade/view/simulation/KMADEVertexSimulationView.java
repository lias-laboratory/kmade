...
public class KMADEVertexSimulationView extends KMADEVertexView {
    ...
    protected String getIterationName(KMADEDefaultGraphCell myGraphCell) {
        ...
        if (myGraphCell.getTask().getIteExpression().isErrorInExpression()) {
            ...
            if (myGraphCell.getTask().getIteExpression().isVariableExpressionNode()) {
                iteration += String.valueOf(myGraphCell.getTask().getIteExpression().getIterationVariant()) + "/" + myGraphCell.getTask().getIteExpression().getNodeExpression().getNodeValue();
            } else {
                if (myGraphCell.getTask().getStateSimulation().isActive() || myGraphCell.getTask().getStateSimulation().isPassive()) {
                    try {
                        myGraphCell.getTask().getIteExpression().getNodeExpression().evaluateNode(null);
	...
	class MyVertexSimulationRenderer extends MyVertexRenderer {
        ...
        protected void paintLabelCaract(Graphics2D f, KMADEDefaultGraphCell myCell) {
            if (GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getSimulationToolBar().isLabelColorSelected()) {
        ...
        /**
         * permet l'affichage du texte et de la couleur concernant l'�tat de la t�che ainsi que les informations d'it�ration
         * @param gr le graphix
         * @param widthString - n'est plus utilis�
         * @param numString - n'est plus utilis�
         * @param iterString - la chaine pour it�ration
         * @param num - la position horizontale du texte pour l'�tat
        ...
        public void paint(Graphics gr) {                
            ...
            //permet l'ajout de la couleur et du texte du � l'�tat de la tache, f le graph, 0 position selon X, "" n'est plus utile,"",-5 et -20 permettent de determiner la position verticale de l'affichage du texte
            ...
            if (selected) {
                ...
                f.drawRect(((int) this.getBounds().getWidth() / 2) - 40, 1, 80 - 1, (int) this.getBounds().getHeight() - 1);
            }

            if (!myGraphCell.isMotherPortNoEdged() && (myGraph == GraphicEditorAdaptator.getMainFrame().getSimulationDialog().getGraphSimulation())) {
